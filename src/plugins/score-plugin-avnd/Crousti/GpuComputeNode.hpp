...
using ComputeNodeBaseType =
  std::conditional_t<avnd::gpu_image_output_introspection<Node_T>::size != 0, CustomGpuNodeBase, CustomGpuOutputNodeBase>;
template<typename Node_T>
using ComputeRendererBaseType =
std::conditional_t<avnd::gpu_image_output_introspection<Node_T>::size != 0, score::gfx::NodeRenderer, score::gfx::OutputNodeRenderer>;
...
struct GpuComputeNode final : ComputeNodeBaseType<Node_T>
{
    ...
    , control_outs{std::move(ctls)}
  {
    ...
    {
      this->input.push_back(new score::gfx::Port{this, {}, score::gfx::Types::Image, {}});
    }
    for(std::size_t i = 0; i < texture_outputs::size; i++)
    {
      this->output.push_back(new score::gfx::Port{this, {}, score::gfx::Types::Image, {}});
    }
...
struct GpuComputeRenderer final : ComputeRendererBaseType<Node_T>
{
  ...
  QRhiShaderResourceBinding initBinding(score::gfx::RenderList& renderer, F field)
  {
    ...
    else if constexpr(requires { F::image2D; })
    {
      ...
      QRhiTexture* tex = tex_it != createdTexs.end() ? tex_it->second : &renderer.emptyTexture();

      if constexpr(requires { F::load; } && requires { F::store; })
        return QRhiShaderResourceBinding::imageLoadStore(F::binding(), bindingStages, tex, 0);
    ...
    else if constexpr(requires { F::buffer; })
    {
      ...
      if constexpr(requires { F::load; } && requires { F::store; })
        return QRhiShaderResourceBinding::bufferLoadStore(F::binding(), bindingStages, buf);
  ...
  void init_input(score::gfx::RenderList& renderer, avnd::field_reflection<Idx, F> field)
  {
    ...
    auto ubo = renderer.state.rhi->newBuffer(QRhiBuffer::Dynamic, QRhiBuffer::UniformBuffer, gpp::std140_size<ubo_type>());
    ubo->create();
  ...
  void update(score::gfx::RenderList& renderer, QRhiResourceUpdateBatch& res) override
  {
    ...
    avnd::gpu_uniform_introspection<Node_T>::for_all(
          ...
          [&] <avnd::uniform_port F> (const F& t) {
            using uniform_type = typename avnd::member_reflection<F::uniform()>::member_type;
            using ubo_type = typename avnd::member_reflection<F::uniform()>::class_type;
