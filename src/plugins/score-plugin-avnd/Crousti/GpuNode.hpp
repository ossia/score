    ...
    , GpuRendererFiles<Node_T>
{
  ...
  // removeOutputPass). Each pass gates its own srb->create() /
  // pipeline->create(): a single global flag cannot handle a pass added live
  // onto an update()-driven node, where the first frame would either recreate
  // the already-live passes or skip the new one entirely.
  ...
  void initState(score::gfx::RenderList& renderer, QRhiResourceUpdateBatch& res) override
  {
    ...
    // prepare()/processControlIn for graphics nodes is not invoked here:
    // `states` is empty at initState time (states are constructed per-edge in
    // addOutputPass), so there is nothing to prepare. The prepare/control-in
    // call happens in addOutputPass, right after each state is built.
      ...
      QRhiResourceUpdateBatch& res) override
  {
    ...
    if(rt.renderTarget)
    {
      ...
      // input and run prepare() on the freshly-constructed state here, not
      // in initState, where `states` is still empty. Detection uses
  ...
  void releaseState(score::gfx::RenderList& r) override
  {
    ...
    // Release the allocated pipelines. Each Pass::p.srb refers to the same
    // shared m_srb (see addOutputPass), so null it out per-pass before
    // Pipeline::release() and delete it exactly once below, covering the
    // m_p-empty case too. Mirrors removeOutputPass.
    for(auto& pass : m_p)
    {
