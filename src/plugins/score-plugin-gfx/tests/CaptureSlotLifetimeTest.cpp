...
//
// Both are pure logic -- atomics and a small array. A borrowed slot handed back
// one acquisition too early is a frame the producer's device overwrites while
// the GPU is still sampling it; one handed back too late, or never, starves the
// producer's queue instead. A format generation bumped when nothing changed
// makes the render thread rebuild every GPU resource it owns once per frame.
//
...
TEST_CASE("a reader never sees half of two geometries")
{
  ...
  // Wait for the reader to get through one iteration before racing it:
  // std::thread's constructor does not promise the body has begun, and on a
  // machine where thread start-up is slower than 100000 publishes the writer
  // would set stop before the reader ever looped.
