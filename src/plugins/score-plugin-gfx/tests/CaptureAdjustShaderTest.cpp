...
///
/// The comparison above uses a bespoke vertex shader with no material block,
/// which is exactly why it could not catch the bug this exists for: giving the
/// fragment stage the long material block while the vertex stage kept the short
/// one makes the program fail to link -- "struct type mismatch between shaders
/// for uniform (named mat)" -- and every capture went black with no shader
/// error, because the failure is at link time, not compile time.
...
/// The bug this guards against, reproduced: a vertex stage declaring the SHORT
