...
int runFlip(const Args& a, const DeviceInfo& di)
{
  ...
  // Drop the first interval: it spans the modeset, not a flip. It runs many
  // frames long while the mode settles, so counting it turns an otherwise
  // vblank-exact median into a FAIL(pacing) that says more about the harness
