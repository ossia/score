...
// imports as. The fallback is CPU staging, which at 3552x3556 copies 25.5 MB
// per frame out of uncached V4L2 mmap pages and does not hold 30 fps.
//
// This answers, on the hardware rather than by inference:
//   1. which DRM fourccs this EGL will import at all (EXT_image_dma_buf_import_modifiers)
//   2. whether a real V4L2-exported buffer imports under each candidate layout
//   3. whether what the GPU samples MATCHES what the CPU sees in the same buffer
//
// (3) is the one that cannot be skipped: on the desktop, V4L2 dma-bufs imported
// into NVIDIA read back partly as zeros because vb2 pages are CPU-cached and the
// GPU reads DRAM. If Tegra has the same gap, no import layout saves us and the
// answer is NvBufSurface allocation instead.
//
...
// same pages. Two things are under test and both are load-bearing:
//   - what an external sampler yields for a single-channel 16-bit image (for
//     YUV it converts to RGB; for R16 the channel layout is unspecified)
//   - whether the GPU sees what the CPU wrote. On the desktop, V4L2 dma-bufs
//     imported into NVIDIA read back partly as zeros: vb2 pages are CPU-cached
//     and the GPU reads DRAM. If that gap exists here, no import layout helps.
//
