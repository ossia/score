...
// our buffers (V4L2_MEMORY_DMABUF), instead of V4L2 allocating and us importing
// its CPU-cached ones. The capture device DMAs straight into GPU-allocated
// memory, so the cache-staleness failure cannot arise -- there is nothing
// CPU-cached in the path at all.
//
// This also exercises the constraint NVIDIA documents for the same pattern on
// Tegra: the allocator's pitch must equal the driver's bytesperline, or every
// row lands at the wrong offset.
// ---------------------------------------------------------------------------
