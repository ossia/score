...
// text handed to the GL backend is rewritten. score's uniform blocks end
// `} renderer;`, not `};`, so slicing to the first `};` cuts the block name off
// and the compile fails at token "renderer". Hence the assertions below are
// about what must survive the edit, not only about what it inserts.
