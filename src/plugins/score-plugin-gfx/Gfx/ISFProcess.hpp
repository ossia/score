  ...
  */
    ...
    struct input_vis
    {
      ...
      Process::Inlet* operator()(const long_input& v)
      {
        ...
        // ComboBox::init expects the value to be initially selected, not an
        // index, while libisf's `v.def` is the index into values for enum
        // mode: look up the alternative at v.def and forward its value.
        // CSF/Process.cpp and GeometryFilter/Process.cpp do the same.
      ...
      Process::Inlet* operator()(const storage_input& v)
      {
        ...
        // read inlet would give write buffers a phantom TextureInlet, shifting
        // every later port by one (positional routing) and never exposing the
        // TextureOutlet the renderer produces.
        ...
        // flexible-array member synthesize a "size" control -- the same
        // condition as CSF/Process.cpp setupCSF, the renderer and the
        // generated GLSL.
        if(!v.layout.empty()
      ...
      Process::Inlet* operator()(const texture_input& v)
      {
        ...
        // every texture_input, so the model must create the matching inlet:
        // returning nullptr here would shift every subsequent port.
        auto port = new Gfx::TextureInlet(
            QString::fromStdString(input.name), Id<Process::Port>(i), &self);
        self.m_inlets.push_back(port);
        return port;
      ...
      Process::Inlet* operator()(const csf_image_input& v)
      {
        ...
        // produced storage image. Always creating an inlet would give write
        // images a phantom inlet (port shift) and no outlet for downstream
        // connection.
        if(v.access == "read_only")
        {
          auto port = new Gfx::TextureInlet(
