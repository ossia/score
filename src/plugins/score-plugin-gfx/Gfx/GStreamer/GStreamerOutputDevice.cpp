...
struct GStreamerOutputNode : score::gfx::OutputNode
{
  ...
  // Non-blocking bus poll, once per rendered frame, surfacing otherwise-silent
  // encoder / filesink / muxer errors.
  //
  // Only GST_MESSAGE_ERROR is fatal. GStreamer routinely posts GST_MESSAGE_WARNING
  // during healthy encoding -- late or dropped buffers, missing PTS, encoder rate
  // warnings -- so the filter is on ERROR alone and bus_timed_pop_filtered drains
  // the warnings it passes over.
  //
  // A real error clears m_feeding but deliberately leaves m_started set:
  // stop_pipeline() must still run, emit EOS and drive the pipeline to
  // GST_STATE_NULL so the muxer finalizes the file.
