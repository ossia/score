...
void GfxContext::recomputeTimers()
{
  ...
  // readback of the same frame returns correct pixels. The timer-driven path
    ...
    const ossia::flat_set<EdgeSpec>& cur_edges)
{
  ...
  // Edges whose endpoint node is not present YET must NOT be treated as
  // applied: updateGraph already committed cur_edges to the authoritative
  // `edges` baseline, so unless we roll them back the next diff sees
  // old_edges == cur_edges for them and never re-emits them. We drop them
  // from the baseline and re-raise edges_changed so the next tick re-emits
  // and wires them.
...
void GfxContext::updateGraph()
{
  ...
  // dropped update. Clearing after the copy could clobber a set-after-copy, and
  // with prev_edges dedup on the producer side that update is never re-sent.
