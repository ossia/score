...
dxgiToQRhiFormat(DWORD dxgi, QRhi::Implementation backend) noexcept
{
  ...
  switch(static_cast<DXGI_FORMAT>(dxgi))
  {
    ...
    case DXGI_FORMAT_R16G16B16A16_TYPELESS:
      // RGBA16F is the only 4x16 format QRhi exposes, and dxgiToVulkanFormat() maps
      // the same DXGI formats to VK_FORMAT_R16G16B16A16_SFLOAT so the imported
      // VkImage and the QRhi-created view agree.
      //
      // Unfixed: for a _UNORM sender the raw 16-bit UNORM bits are reinterpreted as
      // IEEE half-float at sample time, and any value whose bit pattern falls in the
      // half NaN/Inf range (1.0 = 0xFFFF, everything >= 0x7C01) samples as NaN and
      // propagates through downstream blends. Correcting it needs a format-converting
      // import -- vkCmdBlitImage UNORM->SFLOAT, or a D3D shader copy -- since a
      // post-sample renormalize cannot recover collapsed data. Both _UNORM and _FLOAT
      // are 64 bits per pixel, so the import succeeds.
...
private:
  ...
  void initState(score::gfx::RenderList& renderer, QRhiResourceUpdateBatch& res) override
  {
    ...
    else
    {
      ...
      si.width = 1280;
      si.height = 720;
      // Default DXGI format: BGRA on D3D/Vulkan, RGBA on GL.
  ...
  void updateD3D11(QRhi& rhi, QRhiResourceUpdateBatch& res)
  {
    ...
    // Recreate the destination texture if anything changed. D3D11 CopyResource
    // requires source & destination formats to match, so the sender's DXGI
    // format has to be honored here.
  ...
  static VkFormat dxgiToVulkanFormat(DWORD dwFormat)
  {
    switch((DXGI_FORMAT)dwFormat)
    {
      ...
      case DXGI_FORMAT_R16G16B16A16_UNORM:
      case DXGI_FORMAT_R16G16B16A16_FLOAT:
        // The QRhi destination for both is RGBA16F, the only 4x16 format QRhi exposes.
        // The imported VkImage must use the same format as the QRhi-created view, or
        // QVkTexture::createFrom() builds an SFLOAT view over a non-MUTABLE_FORMAT
        // UNORM image, which violates VUID-VkImageViewCreateInfo-image-01762 and
        // samples garbage. Both _UNORM and _FLOAT are 64 bits per pixel, so both map to
        // SFLOAT to stay consistent with dxgiToQRhiFormat().
        //
        // Unfixed: for a _UNORM sender this reinterprets the UNORM bits as half-float,
        // so bright values sample as NaN -- see the fuller note in dxgiToQRhiFormat().
  ...
  bool linkVulkanImage(QRhi& rhi, HANDLE dxShareHandle, unsigned int w, unsigned int h, DWORD dwFormat)
  {
    ...
    // Resolve vkGetMemoryWin32HandlePropertiesKHR through vkGetDeviceProcAddr, not
    // inst->getInstanceProcAddr: Qt forwards that to vkGetInstanceProcAddr, which
    // for a device-level extension function can return a non-null trampoline that
    // crashes when called, since the instance loader has no per-device dispatch for
    // device extensions.
    //
    // vkGetDeviceProcAddr is itself core 1.0, so resolving it through
    // getInstanceProcAddr is safe.
    ...
    // Probe whether import for this format/handle type is supported. This is
    // informational; the real test is the memory-type intersection below.
