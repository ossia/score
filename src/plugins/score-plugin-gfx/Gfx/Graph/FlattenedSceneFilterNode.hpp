 ...
 * per MDI batch so mesh-level fields collapse to 0. Use a CSF compute
 * filter for per-draw cases; this node is for multi-mesh inputs
 * (per-object producers, pre-MDI composition).
 *
 * Outputs:
 *   - Port 0: Geometry (Types::Geometry)
 */
