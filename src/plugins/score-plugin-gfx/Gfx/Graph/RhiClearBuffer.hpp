 ...
 * idiom, which pays a per-call zero-vector allocation plus a CPU→GPU upload
 * of zero bytes. These entry points either issue a native GPU-side fill
...
/// zero-buffer pool — no per-call zero-vector allocation.
///
