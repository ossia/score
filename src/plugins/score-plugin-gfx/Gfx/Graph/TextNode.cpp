...
private:
  ...
  void rerender()
  {
    ...
    if(m_img.size().isNull())
    {
      ...
      // so the same project would render text at a different glyph size
      // depending on which platform plugin the process happens to be using,
      // and differently again headless.
      //
