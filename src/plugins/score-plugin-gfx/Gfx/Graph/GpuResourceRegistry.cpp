...
namespace
{
// Per-arena configuration: fixed-stride layout, buffer capacity = stride x
// slot_count, consumer shaders index arena.entries[slot_index] with std430
// stride == slot_stride. Capacities are a hard cap -- allocate() logs and
// returns an invalid Slot on overflow; growth in place is not implemented.
struct ArenaConfig
{
...
constexpr ArenaConfig kArenaConfigs[(std::size_t)GpuResourceRegistry::Arena::Count_]
    = {
        ...
        // RawLight: 64 B x 4096 = 256 KiB, SSBO static since QRhi forbids
        // StorageBuffer + Dynamic. Sized for VJ / particle workflows emitting
        // thousands of procedural lights; a typical file scene pays only for the
        // slots it uses, the rest is dormant device-local memory. Keep in sync with
        // ScenePreprocessor's lightIdxBytes floor, which must be slot_count * 4.
...
void GpuResourceRegistry::init(QRhi& rhi, QRhiResourceUpdateBatch& batch)
{
  ...
  for(std::size_t i = 0; i < m_arenas.size(); ++i)
  {
    ...
    // Zero-fill the arena: Vulkan does not initialise VkBuffer memory, and arenas
    // are sparse-uploaded -- each Light / Material / Transform / Camera node writes
    // only its own slot -- so on a fresh RenderList every consumer indexing past
    // the populated range would read whatever the device-memory page held.
    //
    // About 4 MiB total per RenderList init across all arenas, once per resize.
    // RhiClearBuffer routes Dynamic buffers through chunked updateDynamicBuffer
    // and Static ones through uploadStaticBuffer, both fed from a thread-local
    // zero pool.
...
{
  // OutputNode-side teardown. The registry persists across RenderList rebuilds,
  // so destroy(RenderList&)'s RL-routed releaseBuffer path is bypassed on a
  // normal rebuild. When the OutputNode's QRhi is about to go away there is no
  // live RenderList to plumb through, so tear the QRhi resources down directly.
  // Callers MUST invoke this BEFORE RenderState::destroy().
  //
  // `delete` on a QRhiBuffer / QRhiTexture / QRhiSampler destroys the GPU
  // resource and frees the wrapper, mirroring RenderList::release's direct
  // deletes -- same contract, QRhi still alive.
  for(auto& a : m_arenas)
  {
    delete a.buffer;
    a.buffer = nullptr;
    ...
    TextureChannel channel, void* native_handle) noexcept
{
  ...
  // Miss: first reuse a slot sweepStaleDynamicTextureSlots() has nulled,
  // otherwise a producer that keeps swapping its texture
...
{
  // A dynamic slot caches a non-owning raw QRhiTexture* belonging to an upstream
  // producer. When that producer changes resolution or format it destroys the
  // texture and creates one with a fresh globalResourceId, so resolveDynamicSlot
  // returns a different slot and the old one is left holding a freed pointer.
  // There is no teardown callback, so staleness is detected structurally:
  // resolveDynamicSlot stamps every slot it resolves with a fresh
  // dynamicSlotCounter, and a slot stamped at or before the previous sweep's
  // checkpoint was not resolved by any live material since.
  //
    ...
    uint32_t current_frame) noexcept
{
  ...
  if(it != m_meshSlabs.end())
  {
    ...
    // Count mismatch -- the same mesh primitive re-emitting with different counts.
    // Defer the free to the grace queue so an in-flight draw referencing the old
    // offset cannot read freed-and-reused bytes.
    //
    // released_frame = current_frame so the next sweep waits `grace` frames from
    // this enqueue, matching QRhi's deferred-release contract, which keys on the
    // submission frame slot.
    //
    // The *Used trackers are decremented eagerly here so the allocation below sees
    // an accurate live-slab footprint while the old slot sits pending. The actual
    // OffsetAllocator::free runs in sweepMeshSlabs phase 2 and does not decrement
    // again: one decrement per slab, at logical-release time.
  ...
  // Drain pending releases that have served their grace BEFORE the fresh
  // allocate. Otherwise an immediate count-mismatch plus a grace-elapsed queued
  // release would force the allocator to find space for new + old bytes, which
  // reads as a spurious "pool exhausted" under live edit on a near-capacity
  // scene. Same grace=2 invariant sweepMeshSlabs uses.
    ...
    uint32_t current_frame, uint32_t grace) noexcept
{
  ...
  // Two phases: move slabs past their grace into m_pendingReleases carrying
  // their vertex and index Allocations, then free the already-pending releases
  // whose grace has elapsed from the OffsetAllocators.
  //
  // The grace period guards against use-after-free: an indirect_draw_cmds entry
  // from last frame may still reference the slab's byte offset through an
  // in-flight draw, so grace >= FramesInFlight + 1.
    ...
    uint64_t stable_id, uint32_t current_frame) noexcept
{
  ...
  // sub-allocation at once: the backing QRhiBuffer is long-lived, but an
  // immediately freed offset could be handed out again this frame while an
  // in-flight draw still references it. sweepMeshSlabs drains the queue once
  // released_frame + grace <= current_frame.
  //
  // The *Used trackers are decremented eagerly here; the phase-2 drain does not
  // re-decrement.
  if(m_vertexAllocator
     && it->second.vertex_slot.metadata
            != OffsetAllocator::Allocation::NO_SPACE)
