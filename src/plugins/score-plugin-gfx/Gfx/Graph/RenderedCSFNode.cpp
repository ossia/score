  ...
  for(const auto& input : n.m_descriptor.inputs)
  {
    ...
    else
    {
              ...
              && binding_upstream->indirect_count.handle)
      {
        ...
        // this CSF produces none. Needed for any passthrough CSF between
        // ScenePreprocessor and an MDI rasterizer, since the fast path would otherwise
        // keep the previously published handle.
