...
thread_local std::unordered_set<std::size_t> g_warnedUnalignedOffsets;

void warnUnalignedOffset(
    const char* kind, int binding, const QRhiBuffer* buf, int64_t offset,
    int64_t alignment)
{
  std::size_t key = std::hash<const void*>{}(buf);
  ossia::hash_combine(key, binding);
  ossia::hash_combine(key, offset);
  if(!g_warnedUnalignedOffsets.insert(key).second)
    return;
  qWarning().nospace() << "score.gfx: CSF " << kind << " binding " << binding << " ("
                       << (buf ? buf->name() : QByteArray{})
                       << "): the upstream publishes its data at byte offset " << offset
                       << ", not a multiple of the " << alignment
                       << "-byte offset alignment; the whole buffer is bound from byte "
                          "0, so the shader sees the published data "
                       << offset << " bytes in";
}
    ...
    int64_t offset, int64_t size, std::size_t& rangeHash) noexcept
{
  ...
  const bool aligned = offset % kStorageOffsetAlignment == 0;
  const bool ranged = size > 0 && offset >= 0 && offset < capacity && aligned
                      && (offset > 0 || size < capacity);
  if(!aligned && size > 0 && offset > 0 && offset < capacity)
    warnUnalignedOffset("storage", binding, buf, offset, kStorageOffsetAlignment);
  if(!ranged)
  {
    switch(type)
    {
...
QRhiShaderResourceBinding uniformBufferBinding(
    QRhi& rhi, int binding, QRhiBuffer* buf, int64_t offset, int64_t size,
    std::size_t& rangeHash) noexcept
{
  constexpr auto stage = QRhiShaderResourceBinding::ComputeStage;
  const int64_t capacity = buf ? (int64_t)buf->size() : 0;
  const int64_t alignment = rhi.ubufAlignment();
  const bool aligned = offset % alignment == 0;
  if(!aligned && size > 0 && offset > 0 && offset < capacity)
    warnUnalignedOffset("uniform", binding, buf, offset, alignment);
  if(offset <= 0 || offset >= capacity || !aligned)
    return QRhiShaderResourceBinding::uniformBuffer(binding, stage, buf);

  const auto off = (quint32)offset;
  const auto len = (quint32)(capacity - offset);
  ossia::hash_combine(rangeHash, binding);
  ossia::hash_combine(rangeHash, off);
  ossia::hash_combine(rangeHash, len);
  return QRhiShaderResourceBinding::uniformBuffer(binding, stage, buf, off, len);
}
}
...
{
  // Map CSF format strings to Qt RHI texture formats. Compared case-insensitively:
  // libisf lowercases the FORMAT layout qualifier into the GLSL while the CSF JSON
  // parser stores image->format verbatim, so both spellings must resolve here.
  ...
  // Integer formats, required for atomic image ops. Atomics in Vulkan, D3D12 and
  // Metal 3.1+ work on the R{8,32}{UI,SI} family; the wider {RG,RGBA}32{UI,SI}
  // variants are sample-only on most desktop GPUs but still legal as storage
  // images, and are exposed so several counters can be packed per voxel.
  //
  // Added to QRhiTexture::Format in Qt 6.10; on older Qt the request falls
  // through to RGBA8 rather than failing the build.
...
static bool inputClosesCycle(const score::gfx::Node& self, const score::gfx::Port& input)
{
  std::vector<const score::gfx::Node*> sources;
  for(auto* e : input.edges)
    if(e->source && e->source->node)
      sources.push_back(e->source->node);
  if(sources.empty())
    return false;
  if(ossia::contains(sources, &self))
    return true;

  std::vector<const score::gfx::Node*> stack{&self};
  std::unordered_set<const score::gfx::Node*> seen{&self};
  while(!stack.empty())
  {
    ...
    for(auto* out : n->output)
    {
      ...
      {
        if(!e->sink || !e->sink->node)
          continue;
        const auto* next = e->sink->node;
        if(ossia::contains(sources, next))
          return true;
...
{
  // registerCommonExpressionVariables emplaces up to 4 doubles per image-type
  // input plus 4 one-time unsuffixed, 1 per scalar input, 2 per geometry binding
  // plus 2 one-time, and 2 ($COUNT/$BYTESIZE) per addressable SSBO or UBO.
  // Callers add a couple of $USER constants. Each descriptor input falls in
    ...
    ossia::math_expression& e, ossia::small_pod_vector<double, 16>& data) const
{
  ...
  // Register the full geometry of each input image: $WIDTH_<name>,
  // $HEIGHT_<name>, $DEPTH_<name>, $LAYERS_<name>.
  //
  // DEPTH and LAYERS come from the live QRhiTexture when available and fall back
  // to 1, so "$DEPTH_vol" stays defined whether or not the bound texture is
  // volumetric. The first input image also exposes the unsuffixed forms for the
  // common "filter inherits its input's size" case.
  ...
  // Named geometry vertex/instance counts: $VERTEX_COUNT_<name>,
  // $INSTANCE_COUNT_<name>, and the first one unsuffixed.
  //
  // Always registered so the expression parses even before any upstream geometry
  // has flowed, falling back to the descriptor's static counts and then to 1.
  // Otherwise every dispatch evaluated before updateGeometryBindings populates
  // geo_bind raises ERR232 Undefined symbol.
  ...
  // Register $COUNT_<name> and $BYTESIZE_<name> for every addressable SSBO / UBO
  // the node binds, input or output, so SIZE / TARGET / WIDTH / HEIGHT
  // expressions can size themselves to upstream extents by name.
  //
  // On a name collision the nested input-side binding is registered first and the
  // top-level output-side re-registration is suppressed: `$COUNT_scene_lights`
  // means the upstream count, not the size of the buffer about to be overwritten.
  //
  // For a UBO, COUNT is 1 and BYTESIZE the struct size. For an SSBO with a
  // flexible array, stride comes from calculateStorageBufferSize(layout, 1) minus
  // calculateStorageBufferSize(layout, 0) and COUNT is the element count;
  // without one, COUNT is 1.
  {
...
BufferView RenderedCSFNode::bufferForOutput(const Port& output)
{
  for(auto& [port, index] : this->m_outStorageBuffers) {
    if(&output == port) {
      ...
      BufferView bv{
          sb.buffer, sb.owned ? 0 : sb.offset,
          !sb.buffer ? 0 : sb.owned ? (int64_t)sb.buffer->size() : sb.size};
        ...
        if(aux->buffer >= 0 && aux->buffer < (int)mesh.buffers.size())
        {
          ...
          if(auto* gpu = ossia::get_if<ossia::geometry::gpu_buffer>(&geo_buf.data))
          {
            if(gpu->handle)
            {
              ...
              const int64_t published = aux->byte_size > 0
                                            ? aux->byte_size
                                            : gpu->byte_size - aux->byte_offset;
              if(storageBuffer.buffer != rhi_buf || storageBuffer.size != published
                 || storageBuffer.offset != aux->byte_offset)
              {
                adoptIntoSlot(renderer, storageBuffer, rhi_buf, published);
                storageBuffer.offset = aux->byte_offset;
    ...
    RenderList& renderer, QRhiResourceUpdateBatch& res)
{
  ...
  for(const auto& input : n.m_descriptor.inputs)
  {
    ...
    if(binding_has_upstream && upstream_mesh)
    {
      ...
      for(int attr_idx = 0; attr_idx < (int)geo_input->attributes.size(); attr_idx++)
      {
        ...
        if(auto* gpu = ossia::get_if<ossia::geometry::gpu_buffer>(&geo_buf.data))
        {
             ...
             && (attr_size == elem_size || stride == (int)csf_elem_stride))
          {
            ...
            if(req.access == "read_write" && !in_place && rhi_buf != ssbo.read_buffer
               && (!ssbo.owned || rhi_buf != ssbo.buffer))
            {
          ...
          if(geo_aux->buffer >= 0 && geo_aux->buffer < (int)mesh.buffers.size())
          {
            ...
            if(auto* gpu = ossia::get_if<ossia::geometry::gpu_buffer>(&geo_buf.data))
            {
              if(gpu->handle)
              {
                ...
                if(!in_place && !aux.is_uniform && aux.access == "read_write"
                   && published > 0 && rhi_buf != aux.read_buffer
                   && (!aux.owned || rhi_buf != aux.buffer))
                {
      ...
      // Auxiliary textures: matched by name against the mesh's auxiliary_textures,
      // falling back to the shape-matched placeholder rather than keeping a stale
      // upstream handle. Only the cached texture pointer is updated here; the SRB
      // rebuild is driven by initComputeSRBAndPasses / recreateSRB.
  ...
  for(const auto& input : n.m_descriptor.inputs)
  {
    ...
    for(int aux_idx = 0; aux_idx < (int)binding.auxiliary_ssbos.size(); aux_idx++)
    {
      ...
      if(aux.size_expr.empty() || !aux.owned)
        continue;
  ...
  for(const auto& input : n.m_descriptor.inputs)
  {
    ...
    if(structure_changed)
    {
      ...
      if(binding_upstream)
      {
        ...
        // metadata from upstream for filter-type nodes.
      ...
      if(binding_upstream)
      {
        ...
        // buffer forward: an aux texture this CSF declares itself keeps its binding.
      ...
      {
        // Forward upstream's indirect-draw buffer when this CSF produces none.
    ...
    else
    {
      ...
      if(binding_upstream)
      {
        ...
        // Publish this CSF's own writable storage images (write_only and read_write
        // csf_image_input declarations) into the geometry cable's auxiliary_textures so
        // downstream consumers can resolve them by name -- the mirror of how
        // m_storageBuffers is forwarded into out_geo.buffers above.
        for(const auto& si : m_storageImages)
        {
          if(si.access == "read_only" || !si.texture)
            continue;
...
void RenderedCSFNode::createComputePipeline(RenderList& renderer)
{
  ...
  try
  {
    ...
    warnStorageImageUnits(rhi, "CSF", {&m_computeShaderSource});
    ...
    QList<QRhiShaderResourceBinding>& bindings)
{
  ...
  for(const auto& input : n.m_descriptor.inputs)
  {
    ...
    if(auto* storage_in = ossia::get_if<isf::storage_input>(&input.data))
    {
      ...
      if(it != m_storageBuffers.end() && it->buffer)
      {
        if(it->access == "read_only")
        {
          ...
          int64_t buf_offset = it->owned ? 0 : it->offset;
          int64_t buf_size = it->owned ? 0 : it->size;
        ...
        {
          bindings.append(storageBufferBinding(
              QRhiShaderResourceBinding::BufferStore, bindingIndex++, it->buffer,
        ...
        {
          bindings.append(storageBufferBinding(
              QRhiShaderResourceBinding::BufferLoadStore, bindingIndex++, it->buffer,
              it->owned ? 0 : it->offset, it->owned ? 0 : it->size, m_srbRangeHash));
          output_port_index++;
        }
      }
      ...
      {
        // Missing storage buffer: warn, and bump bindingIndex so the rest of
        // the layout stays in sync with the shader's expected slots.
      ...
      // synthesized "size" INPUT port on the model (setupCSF / isf_input_port_vis).
      // The write branches above only advance output_port_index, so advance the input
      // cursor here under the same flex-array condition used everywhere else; the
      // geometry branch does the equivalent for its $USER ports.
    ...
    else if(auto* geo_input = ossia::get_if<isf::geometry_input>(&input.data))
    {
      if(geo_binding_index < (int)m_geometryBindings.size())
      {
        ...
        for(int attr_idx = 0; attr_idx < (int)geo_input->attributes.size(); attr_idx++)
        {
          ...
          else // read_write -> 2 bindings: _in (readonly) + _out (read-write)
          {
            ...
            // The snapshot must MATCH the buffer it shadows. Allocating it once
            // behind a `!read_buffer` guard sized it from whatever the buffer
            // happened to be at first bind -- one element, before the vertex
        ...
        for(auto& aux : binding.auxiliary_ssbos)
        {
          ...
          if(!aux.buffer)
          {
            ...
            // match the aux kind: binding a StorageBuffer-only buffer as a UBO, or the
            // reverse, is rejected by the Vulkan validation layer.
            //
            // Sized from the shader's own LAYOUT rather than a fixed guess -- aux.size is 0
            // here, since it is only assigned where a buffer already exists.
          ...
          if(aux.is_uniform)
          {
            ...
            bindings.append(uniformBufferBinding(
                rhi, bindingIndex++, aux.buffer, aux.owned ? 0 : aux.offset,
                aux.owned ? 0 : aux.size, m_srbRangeHash));
          }
          else if(aux.owned)
          {
      ...
      // Inlet port for upstream geometry, created either by empty ATTRIBUTES (pure
      // pass-through) or by at least one read_only / read_write attribute. Either way
      // the geometry input owns exactly one entry in node.input, which the following
      // storage_input / texture_input entries address by position, so the cursor must
      // advance past it.
  ...
  for(const auto& input : n.m_descriptor.inputs)
  {
    ...
    else if(auto* geo = ossia::get_if<isf::geometry_input>(&input.data))
    {
      ...
      for(const auto& aux : geo->auxiliary)
      {
        ...
        int arrayCount = 0;
        if(!aux.size.empty())
          arrayCount = std::max(1, resolveCountExpression(aux.size, *geo, aux.name));
      ...
      // Auxiliary textures: one entry per geometry_input AUXILIARY texture
      // declaration. The sampler is allocated now, or skipped for storage-image
      // entries, and a placeholder from the RenderList keeps the SRB valid before the
      // first upstream resolution; per-frame resolution happens in
      // updateGeometryBindings.
      //
      // write_only / read_write storage-image entries also allocate the texture here,
      // the analogue of the m_storageImages allocation top-level csf_image_input
      // entries get. The RGBA8 sample-only placeholder cannot serve an integer-format
      // imageStore or imageAtomicOr.
      for(const auto& atx : geo->auxiliary_textures)
      {
      ...
      // empty (pass-through) or at least one is writable, not when it merely has a
      // writable AUXILIARY (see isf_input_port_count_vis). binding.has_output is
      // broader -- it is also set for writable aux SSBOs, which publish no geometry
      // outlet -- so outlet_index must not advance on it.
...
// Hash the bindings list to detect frame-to-frame drift: two lists hash equal
// iff every entry's descriptor identity matches, which lets
// recreateShaderResourceBindings skip the destroy+setBindings+create cycle in
// the steady state. Uses Qt's own qHash(QRhiShaderResourceBinding) so the
// equivalence matches QRhi's canonical representation rather than a hand-packed
// copy of its private union. Per-binding hashes are seeded by the binding index
// so equal bindings at different slots differ, then combined with
// ossia::hash_bytes.
namespace
{
...
void RenderedCSFNode::recreateShaderResourceBindings(RenderList& renderer, QRhiResourceUpdateBatch& res)
{
  ...
  // frame: identical → skip the destroy+setBindings+create cycle.
  for(auto& [edge, pass] : m_computePasses)
  {
    ...
    Edge& edge)
{
  ...
  {
    // Step 1: update all scatter params UBOs and SRBs (needs live res batch)
    ...
    // Step 2: dispatch all scatters inside a single compute pass
    if(anyScatter)
    {
...

  {
    ...
    for(auto& binding : m_geometryBindings)
    {
      for(auto& ssbo : binding.attribute_ssbos)
        if(ssbo.restore_source && ssbo.owned && ssbo.buffer && ssbo.restore_size > 0)
          anyRestore = true;
      for(auto& aux : binding.auxiliary_ssbos)
        if(aux.restore_source && aux.owned && aux.buffer && aux.restore_size > 0)
          anyRestore = true;
    }
    ...
    if(anyRestore)
    {
      ...
      auto restore = [&](auto& slot) {
        if(!slot.restore_source || !slot.owned || !slot.buffer)
          return;
        const int64_t sz = std::min<int64_t>(slot.restore_size, slot.buffer->size());
        if(sz > 0)
          copyBuffer(
              *renderer.state.rhi, commands, slot.restore_source, slot.buffer, (int)sz,
              (int)slot.restore_offset, 0, BufferCopyBarrier::None);
      };
      for(auto& binding : m_geometryBindings)
      {
        for(auto& ssbo : binding.attribute_ssbos)
  ...
  for(std::size_t passIndex = 0; passIndex < n.m_descriptor.csf_passes.size(); passIndex++)
  {
        ...
        || passDesc.execution_type == "PER_INSTANCE")
    {
      ...
      {
        // 1D_BUFFER resolution takes three forms, chosen by what TARGET holds:
        //
        //   an expression ("$expr", "literal * literal", "literal"): evaluated through
        //   the common resolver, with the same variables as SIZE / WIDTH / HEIGHT /
        //   STRIDE_*, and the result is the total thread count spread across x/y/z
        //   workgroups below -- MANUAL without picking an axis split by hand.
        //
        //   a bare identifier ("bufferName"): dispatch over that buffer's element
        //   count, shorthand for "$COUNT_bufferName".
        //
        //   empty: size by the output storage buffer matching the current edge, in
        //   BYTES rather than elements, then by the first geometry's vertex_count.
      ...
      {
        // Workgroup count overflow: skip this pass only. No compute pass is open at
        // this point -- the begin/end is hoisted after the size calculation -- so there
        // is nothing to close, and `return` would desync the ping-pong swaps.
        continue;
      }
    ...
    if(dispatchX <= 0 || dispatchY <= 0 || dispatchZ <= 0 || m_auxSizeUnresolved)
      continue;

    // Publish the workgroup count to the per-pass ProcessUBO so the shader can read
    // gl_NumWorkGroups through the libisf-injected alias: SPIRV-Cross's HLSL backend
    // cannot emit the GLSL built-in, and the D3D11/D3D12 bake fails outright.
    //
    // Must happen before beginComputePass: updateDynamicBuffer is applied as part of
    // the resource-update batch that beginComputePass consumes.
    if(pass.processUBO)
    {
      if(!res)
        res = renderer.state.rhi->nextResourceUpdateBatch();
  ...
  // that holds these bindings via the indices recorded at build time.
  {
    ...
    if(any_swap)
    {
      ...
      // Graphics passes that visualize the persistent image bake the pre-swap
      // si.texture pointer at construction, so after the ping-pong that handle names
      // the stale slot. Patch every graphics SRB to sample the post-swap writable
      // target.
      for(auto& [e, gp] : m_graphicsPasses)
      {
        if(!gp.pipeline.srb || !gp.outputSampler)
      ...
      // m_outputTexture is the fallback returned by textureForOutput() for
      // default-port queries, captured from the first persistent storage image's
      // primary texture at build time. After the swap that pointer is the stale slot,
      // so find the entry whose post-swap read_texture matches it and refresh.
      if(m_outputTexture)
      {
        for(const auto& si : m_storageImages)
        {
  ...
  // GENERATE_MIPS: regenerate the mip chain so downstream samplers with a mipmap
  // filter see a valid level > 0, queued on the same per-frame resource-update
  // batch as the rest of update().
  //
  // Gated on FRAMEINDEX > 0: textures are created PREINITIALIZED and Qt RHI's
  // GenMips path transitions from a transfer layout back to whatever the texture
  // was stored as, which trips VUID-VkImageMemoryBarrier-newLayout-01198 until the
  // compute dispatch has moved the image to GENERAL.
