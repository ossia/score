    ...
    QRhiResourceUpdateBatch& res)
{
  ...
  // triple. Comparing `curSz.w * curSz.h` against `audio.data.size()` only
  // holds for Waveform, whose W=samples x H=channels layout has
  // pixel_count == raw_sample_count; FFT has half the pixels and Histogram
  // hard-codes H at 240, so both would read as "size changed" every frame and
  // force a destroy+recreate plus a full SRB rebuild in the caller.
