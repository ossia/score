...
// The Light producer owns a RawLight
// arena slot and writes RawLightData directly in its own update() hook
// (see Threedim/Light.cpp). Consumer shaders compose direction / position
// on the fly from world_transforms[RawLight.transform_slot].

// ---- mesh_primitive -> ossia::geometry ------------------------------------
//
// Builds a transient ossia::geometry on the heap wrapping a mesh_primitive's
// buffers and attribute layout. The downstream preprocessor copies those
// handles into its own output, so it only needs to survive the flatten pass.
// CPU-backed buffer_data flows through as cpu_buffer and GPU handles as
// gpu_buffer.
...
// Pack the CPU-side material_component into the 64-byte GPU struct. Only factor
// fields are packed; textureRefs[] stay at tex_ref_none().
// ScenePreprocessorNode runs rebuildChannel(ch) per channel after the scene
// walk, which calls patchMaterialRefsFromCache to fill in the assigned
// texture-array layer per material per channel.
MaterialGPU packMaterial(const ossia::material_component& mc)
{
  ...
  // Occlusion: set the flag whenever the material has an occlusionTexture at all.
  // The shader samples through mat.occlusion_textureRef unconditionally in the
  // "separate" branch, which covers both distinct-source and ORM packings.
  // Falling back to mr.r when there is no occlusion texture is unsafe: the glTF
  // spec leaves pbrMetallicRoughness.R undefined and most tools leave it at 0.
...
// Pack the OpenPBR / KHR extension fields into MaterialExtensionsGPU (272 B).
// Field order matches the struct declaration.
//
// textureRefs[] stay at tex_ref_none() here; the encoded refs are written by
// ScenePreprocessor::patchMaterialRefsFromCache in lockstep with the base
// channels, with the kExtTextureSlots table routing each slot through one of
// the 5 existing channel pools by format expectation. There is no separate
// ext-channel pool: the same bucket samplers serve both.
...
// Dedup key combining a payload identity pointer with the accumulated world
// transform on the walk path that reached it. Plain pointer dedup collapses
// every instance of a shared prototype into one, so an upstream SceneDuplicator
// referencing a single prototype under N transforms would lose N-1 instances.
// Keying by (pointer, world matrix) keeps genuinely distinct instances apart
// while still deduping DAG re-references reached through a bit-identical
// transform. Mesh GPU uploads are deduped downstream by DrawCall::stable_id, so
// N draws still upload the prototype's bytes once.
struct InstanceKey
{
  const void* ptr{};
...
struct FlattenVisitor
{
  ...
  // genuinely shared sub-tree references.
  //
  // Nodes, meshes and clouds dedup by (pointer, accumulated world transform) so
  // distinct instances of a shared prototype survive -- see InstanceKey. Lights,
  // cameras, scene_data and instances keep plain pointer dedup: the duplicator
  // path does not multiply them.
  ossia::hash_set<InstanceKey, InstanceKeyHash> seenNodes;
  ossia::hash_set<InstanceKey, InstanceKeyHash> seenClouds;
  // Secondary dedup key for clouds: the raw_data pointer. FormatOverride clones
  // the primitive_cloud_component to rewrite format_id but keeps the underlying
  // raw_data shared, so two components pointing at the same raw_data are one
  // upload's worth of GPU bytes. Falls back to the component pointer when
  // raw_data is null, and is still combined with the world transform.
  ...
  void visitPayload(const ossia::scene_payload& payload)
  {
    ...
    {
      // Format-agnostic point cloud / splat: collected here, then bucketed by
      // format_id in the ScenePreprocessor's primitive-cloud branch, which emits one
      // indirect-draw geometry per bucket alongside the mesh MDI. The data lives in
      // raw_data + format_params, forwarded to the format's CSF chain through the
      // bucket geometry's "raw_splats" auxiliary.
      //
      // Deduped by raw_data pointer rather than component pointer, so FormatOverride
      // -- which clones the component but shares the heavy raw_data -- does not
      // defeat dedup.
      if(*pc)
      {
