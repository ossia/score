...
/// Records the texture→dst copy on `cb`. Must be called outside a render
/// pass. The copy is tightly packed rows, texture storage order (top-left
/// origin in score). Returns false when the texture's format/size cannot
/// target this backend's path — nothing is recorded, fall back.
SCORE_PLUGIN_GFX_EXPORT
