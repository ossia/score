...
{
/**
 * @brief Named-bucket GPU timing collector.
 *
 ...
 * returns the delta in milliseconds. `RenderList::renderInternal` feeds
 * that value in under the `"frame"` bucket; other buckets (`"pso_compile"`)
 * are recorded by whoever measures them.
 *
 ...
 * the last N frames and exposes a copy via `snapshot()` for the
 * observability panel.
 *
