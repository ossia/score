    ...
    RenderList& renderer, QRhiResourceUpdateBatch& res, Edge* edge)
{
  ...
  if(mustRecreatePasses)
  {
    ...
    if(geometry.meshes && !geometry.meshes->meshes.empty())
    {
      ...
      // The geometry-buffer rebind above only patches m_storage entries. Plain
      // image_input INPUTS live in m_inputSamplers and are refreshed only by
      // rebindAuxTextures Path A, which is gated on geometryChanged and runs
      // once earlier in update(); if geometry.meshes was null then, or a
      // sibling renderer republishes a mesh_list afterwards, the sampler keeps
      // its placeholder or a deleteLater'd pointer.
      //
