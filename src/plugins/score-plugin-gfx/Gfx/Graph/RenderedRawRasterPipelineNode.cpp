...
void RenderedRawRasterPipelineNode::updateInputTexture(const Port& input, QRhiTexture* tex, QRhiTexture* depthTex)
{
  ...
  // down — see the comment there. Without this updateInputTexture
  // silently no-ops on every override-bound entry.
    ...
    RenderList& renderer, QRhiResourceUpdateBatch& res, Edge* edge)
{
  ...
  if(mustRecreatePasses)
  {
    ...
    if(geometry.meshes && !geometry.meshes->meshes.empty())
    {
      ...
      // Sampler refresh: the geometry-buffer rebind above only patches
      // m_storage entries (csf_image_input / storage_input / uniform_input). Plain
    ...
    Edge& edge)
{
  ...
  // The MRT pass is the one initMRTPass registered with a null edge; the
  // blit passes that follow each carry their own edge. Index 0 is NOT a
  // reliable stand-in: initMRTPass only registers its pass if the pipeline
  // was created, so a shader the driver rejects leaves a blit pass at index
  // 0, and pairing that pass's non-layered output target with the MRT
  // pipeline state segfaults inside QRhi::beginPass.
  ...
  for(int i = 0; i < invocationCount; ++i)
  {
    ...
    // per-mip / per-face / per-layer one above. Every one of those branches is
    // guarded on the RT existing, so when none matches -- a PerLayer colour pass
    // whose mip RT was never created, a PerCubeFace pass with a null face RT --
    // it can still be null here, and Qt segfaults inside beginPass rather than
    // rejecting it. Skip the pass instead: the same choice
    // createRenderTarget already makes when it degrades instead of aborting.
    if(!rtForPass)
    {
...
{
  // ScenePreprocessor's output geometry is now ALWAYS a single
  // sub-mesh (regular meshes + instance groups all ride through one
  // drawIndexedIndirect / one cpu_draw_commands iteration). There is
  // no per-sub-mesh SRB rebind to do — the SRB is bound once and the
  // draw fans out via the indirect cmd list. The legacy name is
  // preserved for now to avoid churning every call-site.
