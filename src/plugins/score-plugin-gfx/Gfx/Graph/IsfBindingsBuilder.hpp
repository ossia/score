...
struct GraphicsStorageResources
{
  ...
  // over ssbos/images alone: that omits uniform_input UBOs and collides the
  // multiview binding with the last UBO's slot. -1 until the first
  // collectGraphicsStorageResources() call.
  int nextBinding{-1};
  ...
  // observed (sentinelSize). Single buffer reused for both SSBO and UBO
  ...
  QRhiBuffer* sentinelBuffer{};        ///< StorageBuffer usage only
...
void collectGraphicsStorageResources(
    ...
    int firstInlet = 0, int firstImageBinding = -1);

/**
 * @brief Number of bindings the graphics-visible csf_image_input's of `desc`
 *        take, `_prev` slots included.
 */
SCORE_PLUGIN_GFX_EXPORT
 ...
 * thrashing the SRB pool slot every frame on a static scene.
 */
SCORE_PLUGIN_GFX_EXPORT
 ...
 * `score-plugin-gfx`; do not introduce private copies.
 *
 * The vertex-attribute format → byte-size mapping
 * (`ossia::geometry::attribute` enum) is a different domain -- binary
 * attribute formats, not GLSL type strings -- and lives in its own helper
 * inside `RenderedCSFNode.cpp`.
 */
SCORE_PLUGIN_GFX_EXPORT
 ...
 * the SSBO's std430 layout: mixing the two corrupts vec3 data silently.
 */
SCORE_PLUGIN_GFX_EXPORT
