 ...
 *
 * WHY NOT QT_VERSION_CHECK(6, 13, 0): the ossia SDK pins the Qt *6.12
 * branch* and cherry-picks the dev (6.13) indirect changes on top
 * (sdk common/clone-qt.sh: refs/changes/11/738611, 12/738612, 69/761469).
 * On those builds QT_VERSION says 6.12 while the 6.13 API EXISTS, so a
 * version gate would silently compile the features OUT of the reference
 * builds. A stock 6.12 lacks the API entirely, and CI's floor is 6.4.
 * Member/enumerator detection is right on all three tiers; version macros
 * are right on only two.
 *
 * SCORE_GFX_SIMULATE_QT612: defining this (compile flag) forces every
 * 6.13-era detection below to FALSE even when the API exists. It exists to
 * compile-and-run the exact "Qt 6.12: multi-draw indirect but no count, no
 * indirect dispatch" tier against a newer Qt, since no stock 6.12 build may
 * be reachable. The runtime equivalents are the per-capability environment
 * kill switches in RenderState::Caps::populate (SCORE_GFX_NO_GPU_*).
 */
