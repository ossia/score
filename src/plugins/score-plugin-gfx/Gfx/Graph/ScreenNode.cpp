...
void ScreenNode::createOutput(score::gfx::OutputConfiguration conf)
{
  ...
  // destroyOutput() resets it. Returning early here left the callbacks below
  // bound to the graph of whichever execution created the window first, so a
  // window that was closed and re-shown called into a graph that no longer
  // existed and never came back -- not on the next play either, because that
  // re-entry returned here too. Recreating the window is still what must not
