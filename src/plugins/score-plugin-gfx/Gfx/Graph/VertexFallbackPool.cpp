...
{
  // RenderList owns us and must call release() before tearing down the QRhi.
  // Anything left in the map here leaks, but deleting a QRhiBuffer* without
  // knowing the QRhi is still alive is worse, and an assert would fire during
  // OOM teardown -- so the leak is left quiet.
}
