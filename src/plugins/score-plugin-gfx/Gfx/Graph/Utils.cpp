...
namespace
{
// A create() here failing is the backend refusing a configuration -- an MSAA
// depth attachment it does not implement, a format it will not render to, a
// size past its limits. That is a runtime condition, not a broken caller, so
// the node that asked has to be able to render nothing and let the rest of the
// graph carry on. Aborting takes the whole process down, which on a headless
// sweep means the run dies on the first unsupported case instead of reporting
// it.
//
// Releases only what the failing function allocated: `texture` belongs to the
// caller in the overload that is handed one, so it is never touched here.
