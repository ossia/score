...
namespace
{
// A create() failure here is the backend refusing a configuration -- an MSAA
// depth attachment it does not implement, a format it will not render to, a
// size past its limits. That is a runtime condition, so the node that asked
// renders nothing and the rest of the graph carries on.
//
// Releases only what the failing function allocated: in the overload handed a
// `texture`, that texture belongs to the caller.
TextureRenderTarget renderTargetFailed(TextureRenderTarget& ret, const char* what)
{
  qWarning() << "createRenderTarget: the backend would not create" << what
             << "- this render target is unavailable";

  if(ret.renderTarget)
    ret.renderTarget->deleteLater();
  if(ret.renderPass)
    ret.renderPass->deleteLater();
  if(ret.msDepthTexture)
    ret.msDepthTexture->deleteLater();
  if(ret.depthTexture)
    ret.depthTexture->deleteLater();
  if(ret.depthRenderBuffer)
...
createRenderTarget(const RenderState& state, QRhiTexture* tex, int samples, bool depth, bool samplableDepth)
{
  ...
  else
  {
    ...
    if(!ret.colorRenderBuffer->create())
      return renderTargetFailed(ret, "the multisample color buffer");
  ...
  if(samplableDepth)
  {
    ...
    if(!ret.depthTexture->create())
      return renderTargetFailed(ret, "the depth texture");
    ...
    if(useDepthResolve)
    {
      ...
      if(!ret.msDepthTexture->create())
      return renderTargetFailed(ret, "the multisample depth texture");
  ...
  if(!renderTarget)
    return renderTargetFailed(ret, "the render target object");
  ...
  if(!renderPass)
    return renderTargetFailed(ret, "the render pass descriptor");

  renderTarget->setRenderPassDescriptor(renderPass);
  if(!renderTarget->create())
    return renderTargetFailed(ret, "the render target");
    ...
    bool samplableDepth, QRhiTexture::Flags flags)
{
  ...
  if(!texture->create())
  {
    // This overload owns nothing but the texture it just tried to allocate.
    qWarning() << "createRenderTarget: the backend would not create the color"
               << "texture - this render target is unavailable";
    texture->deleteLater();
    return {};
  }
    ...
    int samples)
{
  ...
  if(depthTex)
  {
    ...
    if(!ret.depthRenderBuffer->create())
      return renderTargetFailed(ret, "the depth buffer");
...
// The reinterpret_cast<QRhiShaderResourceBinding::Data*> below, and in
// replaceSampler / replaceTexture, exists because QRhiShaderResourceBinding
// keeps its payload in a private nested ::Data whose only public accessor is
// the const data(). Rebinding in place relies on the class being a thin wrapper
// whose only data member is that struct -- stable across Qt 6.4..dev, but not a
// guaranteed ABI. Revisit if QRhi ever exposes a public mutator.
void replaceBuffer(
...
{
  // Writing a null texture into a sampledTexture / ImageLoad binding crashes the
  // next vkUpdateDescriptorSets. A caller that wants to detach should pass a
  // RenderList empty-fallback matching the sampler's kind; reaching here with
  // null leaves the existing binding in place.
  if(!newTexture)
    return;
  for(QRhiShaderResourceBinding& b : tmp)
  {
...
// layout and pool slot, bump the generation, rewrite only the changed
// descriptors. destroy()+create() would free the pool slot on every live edit.
// All five backends implement the virtual.
...
// Unified geometry-attribute lookup, used by raw raster and CSF alike. Matches
// the request (name plus optional semantic key) to an upstream
    ...
    QRhiShaderResourceBindings* srb)
{
  ...
  // An empty mesh has no vertex-input layout and cannot satisfy a vertex
  // shader that declares inputs; creating the pipeline anyway is a
  // VUID-VkGraphicsPipelineCreateInfo-Input-07904 violation.
  if(!mesh.hasGeometry())
  {
    delete ps;
    return {nullptr, srb};
  ...
  // An invalid render target reaches here when createRenderTarget could not
  // build one; the pipeline cannot be created without its render pass, so the
  // node renders nothing rather than the process dying.
  if(!rt.renderPass)
  {
    qWarning() << "buildPipeline: no render pass - the render target was not created";
    ps->deleteLater();
    return {nullptr, srb};
  }
    ...
    std::span<QRhiShaderResourceBinding> additionalBindings)
{
  ...
  // Bind samplers, substituting the view-type-matched empty texture carried by
  // Sampler::fallback for a null source. This keeps the SRB valid so
  // vkUpdateDescriptorSets cannot crash when an optional shader input has no
  // upstream producer; the pass samples the fallback and renders a neutral value.
  //
  // A null `sampler.fallback` is assumed to mean sampler2D and gets
    ...
    bool useShadingRate)
{
  ...
  // VRS opt-in. Only applies when the backend supports
...
makeShaders(const RenderState& v, QString vert, QString frag, int multiViewCount)
{
  auto [vertexS, vertexError]
      = ShaderCache::get(v, vert.toUtf8(), QShader::VertexStage, multiViewCount);
  ...
  auto [fragmentS, fragmentError]
      = ShaderCache::get(v, frag.toUtf8(), QShader::FragmentStage, multiViewCount);
    ...
    switch(in->type)
    {
      case Types::Image: {
        ...
        else
        {
          ...
          // View-type-matched fallback when the render target has no texture yet. Same
          // reasoning as the GrabsFromSource branch above: binding a sampler2D view into
          // a sampler2DArray / samplerCube / sampler3D slot trips
          // VUID-...-viewType-07752 every frame and crashes some drivers.
          QRhiTexture* fallback = nullptr;
          if((in->flags & Flag::Cubemap) == Flag::Cubemap)
            fallback = &renderer.emptyTextureCube();
          else if((in->flags & Flag::ThreeDimensional) == Flag::ThreeDimensional)
    ...
    QRhiTexture::Format depthFmt)
{
  ...
  // Some backends, notably GL ES, require a colour attachment, so allocate a
  // dummy that is never written; the depth-only RT owns it and releases it with
  // the RT. Desktop Vulkan, Metal and D3D usually accept a depth-only RT, but the
  // dummy is always allocated for portability.
  //
  // The dummy MUST match the depth extent, not be 1x1: the Vulkan backend derives
  // the framebuffer and renderArea from the FIRST colour attachment whenever
  // colorAttCount > 0, and only falls back to the depth texture's size at
  // colorAttCount == 0. A 1x1 dummy would clamp the render area to one pixel.
  ...
  if(!ret.dummyColorTexture->create())
    return renderTargetFailed(ret, "the placeholder color texture");
    ...
    bool samplableDepth)
{
  ...
  // caller-supplied depth texture, which may be a plain 2D depth texture or a
  // TextureArray; QRhi attaches layer 0 by default for a depth-only pass.
  //
  // Ownership: `externalDepthTexture` becomes ret.depthTexture and is released
  // with the RT.
  TextureRenderTarget ret;
