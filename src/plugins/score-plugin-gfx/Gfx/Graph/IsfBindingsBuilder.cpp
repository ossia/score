...
// Whether an INPUTS storage/uniform entry consumes a GRAPHICS-pipeline binding.
// Must match libisf's is_graphics_visibility() verbatim: the GLSL codegen emits
// a layout(binding=N) declaration, and advances its counter, only for these
// visibility strings. A value the codegen skips must not consume a runtime
// binding, or every subsequent storage resource drifts one slot from the binding
// the shader was compiled with -- and on Vulkan and D3D12 the SRB is the
// pipeline layout. Deliberately not visibilityToStages(), whose lenient
// "all"/unknown -> fragment mapping is a different set.
  ...
  walk_descriptor_inputs(
      desc, [&](const isf::input& inp, const port_counts& cur, const port_counts&) {
        ...
        else if(auto* img = ossia::get_if<isf::csf_image_input>(&inp.data))
        {
          ...
          // declaration, triggering Vulkan
          // VUID-VkGraphicsPipelineCreateInfo-layout-07990 at pipeline build.
...
//  if(f == "rg32f")   return QRhiTexture::RG32F;
  ...
  // Integer storage image formats, required for atomic image ops. Aliasing an
  // integer SPIR-V OpTypeImage Format operand onto a float QRhiTexture::Format
  // violates VUID-RuntimeSpirv-OpTypeImage-07752 and
  // VUID-RuntimeSpirv-OpImageWrite-04469. Gated on Qt 6.10+, when QRhiTexture
  // exposed the R{8,32}{UI,SI} and {RG,RGBA}32{UI,SI} formats.
...
// Sentinel zero-buffer for an upstream SSBO/UBO port that disconnects
// mid-session: Vulkan requires every SRB binding to point at a valid resource,
// and a disconnect would otherwise leave it on the prior upstream's
// deleteLater'd QRhiBuffer.
//
// 64 KiB covers any realistic layout a graphics shader reads; the fallback
// returns zeros regardless of size, only descriptor validity matters.
//
// One storage buffer and one uniform buffer, not one buffer with both usages:
// QGles2Buffer::create() refuses a UniformBuffer combined with any other usage,
// so on GL a shared sentinel would silently not exist. Vulkan, D3D11, D3D12 and
// Metal have no such restriction.
    ...
    QRhi& rhi, QRhiResourceUpdateBatch& res, GraphicsStorageResources& store)
{
  ...
  const auto make = [&](QRhiBuffer::UsageFlags usage,
    ...
    if(!buf->create())
    {
      qWarning() << "Failed to create sentinel disconnect buffer" << name;
      delete buf;
      return nullptr;
    }
  ...
  if(!store.sentinelBuffer)
    store.sentinelBuffer = make(QRhiBuffer::StorageBuffer,
                                "ISF_SentinelDisconnectBuffer_SSBO");
  if(!store.sentinelUniformBuffer)
    store.sentinelUniformBuffer = make(QRhiBuffer::UniformBuffer,
                                       "ISF_SentinelDisconnectBuffer_UBO");
    ...
    QSize renderSize)
{
  ...
  for(auto& e : store.ubos)
  {
    ...
    // rationale as the SSBO placeholder above.
    ...
    QRhiShaderResourceBindings* srb)
{
  ...
  for(auto& e : store.ubos)
  {
    ...
    else if(!e.owned && store.sentinelUniformBuffer && !port->edges.empty())
    {
      ...
      // stays valid; reads return predictable zeros.
      //
...
void bindUpstreamBuffersFromGeometry(
    ...
    QRhiShaderResourceBindings* srb)
{
  // SSBO/UBO sibling of bindUpstreamImagesFromGeometry. An INPUTS-declared
  // storage_input / uniform_input may carry its upstream buffer either through a
  // dedicated Buffer port edge, handled by bindUpstreamBuffers, or name-matched
  // against the upstream geometry's auxiliary_buffers -- the pattern
  // ScenePreprocessor uses to publish scene_lights, world_transforms, per_draws,
  // scene_materials, scene_counts, camera and env into a flattened-scene shader.
  //
  // `geometry` is a single ossia::geometry: the caller unwraps from
  // geometry.meshes->meshes[0], same convention as bindUpstreamImagesFromGeometry.
