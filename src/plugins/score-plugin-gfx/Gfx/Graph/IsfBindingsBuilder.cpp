...
// storage and one uniform buffer, not one buffer with both usages: the
// OpenGL backend refuses that combination outright --
//
//   qrhigles2.cpp, QGles2Buffer::create()
//     if (m_usage.testFlag(QRhiBuffer::UniformBuffer)) {
//         if (int(m_usage) != QRhiBuffer::UniformBuffer) {
//             qWarning("Uniform buffer: multiple usages specified, this is
//                       not supported by the OpenGL backend");
//             return false;
//
// -- so on GL the sentinel silently did not exist and every disconnect left
// the SRB binding on the freed upstream buffer, which is the exact
// use-after-free this sentinel is here to prevent. Vulkan, D3D11, D3D12 and
