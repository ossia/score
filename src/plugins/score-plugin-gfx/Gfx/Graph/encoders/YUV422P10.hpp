 ...
 *     the 16-bit plane bytes are packed into RGBA8 here (two samples per
 ...
 * rows, U/V = w bytes/row over h rows), so consumers do not care which one
 * compiled. The < 6.10 path requires width % 4 == 0; the >= 6.10 path only
 * width % 2 == 0.
 */
