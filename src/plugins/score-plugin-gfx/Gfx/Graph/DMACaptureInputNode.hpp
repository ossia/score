...
struct SCORE_PLUGIN_GFX_EXPORT DMACaptureInputNode : ProcessNode
{
  ...
  /// renderer has resolved it. Harnesses must report this: a result row that
  /// does not say which path ran cannot be trusted, and four rounds of results
  /// in one session were invalidated by exactly that.
  ...
  /// them. A harness that reports only its consumption rate cannot tell a card
  /// that failed to capture from one whose frames it was too slow to collect.
