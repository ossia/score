...
struct AudioTextureUpload
{
      ...
      QRhiTexture* rhiTexture)
  {
    ...
    {
      scratchpad[i] = 0.5f + audio.data[i] / 2.f;
    }

    // Copy it
    QRhiTextureSubresourceUploadDescription subdesc(
        scratchpad.data(), audio.data.size() * sizeof(float));
      ...
      QRhiTexture* rhiTexture)
  {
    ...
    for (int i = 0; i < audio.channels; i++)
    {
      ...
      auto spectrum = fft.execute(inputData, bufferSize);

      float* outputSpectrum = scratchpad.data() + i * fftSize;
...
struct RenderedISFNode : score::gfx::NodeRenderer
{
  ...
  void update(Renderer& renderer, QRhiResourceUpdateBatch& res) override
  {
    ...
    // Clear the non-persistent passes
    /*
    int pass_i = 0;
    for(auto& pass : n.descriptor.passes)
    {
      // OPTIMIZEME
      if(!pass.target.empty() && !pass.persistent)
      {
        auto tex = m_passes[pass_i].renderTarget.texture;
        SCORE_ASSERT(tex);
        int bytes = tex->pixelSize().width() * tex->pixelSize().height() * (tex->format() == QRhiTexture::RGBA8 ? 4 : 16); // TODO if we have more formats later

        thread_local QByteArray empty_buffer;
        empty_buffer.resize(bytes);
        QRhiTextureSubresourceUploadDescription desc;
        desc.setData(empty_buffer);

        res.uploadTexture(tex, QRhiTextureUploadDescription(QRhiTextureUploadEntry(0, 0, desc)));
      }
      pass_i++;
    }
    */
