...
struct RenderedScenePreprocessorNode final : NodeRenderer
{
      ...
      QRhiResourceUpdateBatch*& res, Edge& edge) override
  {
    ...
    // post-snapshot resource-update batch:
    //
    //   command stream : ... [updateBatch_N applied] [copyBuffer current->prev]
    //   res            :                             [uploadStaticBuffer/slot]
    //   RenderList     :                             ^ submitted next iteration
    //
    // So the copy reads m_worldTransformsBuffer at its frame N-1 contents, and
    // the next beginPass sees current = frame N, prev = frame N-1.
    //
    ...
    //
    // Fires every frame, not only when writes are pending: on a static scene
    // the copy is what keeps prev == current and motion vectors at zero.
    //
    // Frame 0 sees prev = zeroes, so the first frame's motion is large;
    // consumer shaders handle that through temporal accumulation. The auto
    // barrier covers the compute/transfer hazards around the copy.
