...
QRhiTexture* SimpleRenderedISFNode::textureForOutput(const Port& output)
{
  ...
  // Map an OUTPUT Port to its MRT colour/depth texture.
  //
  // n.output may interleave non-image output ports among the image ones: a
  // write / read_write storage_input pushes a Types::Buffer port and a writable
  // geometry_input a Types::Geometry port during ISFNode's desc.inputs walk,
  // before ISFNode appends one Types::Image port per desc.outputs entry.
  // initMRTPass, meanwhile, builds its attachments from descriptor().outputs,
  // which lists only image and depth outputs.
  //
  // So the descriptor index of an image port is its position among IMAGE PORTS
  // ONLY, not its raw index in n.output: skip the non-image ports.
    ...
    QRhiResourceUpdateBatch& res)
{
  ...
  // Allocate the storage resources (SSBOs and images) declared in the shader,
  // reusing the caller's `res` batch. A fresh rhi.nextResourceUpdateBatch() here
  // would leak a pool slot per addOutputPass call -- QRhi does not reclaim an
  // unreleased batch -- and the 64-slot pool empties after roughly 60 resize
  // cycles under X11 async resize, where each tick rebuilds the RenderList with
...
void SimpleRenderedISFNode::initMRTPass(RenderList& renderer, QRhiResourceUpdateBatch& res)
{
  ...
  // targets. createRenderTarget(mrtSamples) allocates multi-sample colour
  ...
  if(colorTextures.empty() && depthTex)
  {
    ...
    // TextureArray when maxLayers > 1. Letting createDepthOnlyRenderTarget
    // allocate its own would leave textureForOutput() naming a texture nothing
    // renders into.
  ...
  // Allocate the shader-declared storage resources (SSBOs and images) and borrow
  // any upstream buffers BEFORE building the extra bindings, exactly as the
  // non-MRT initPass does. buildExtraBindings only emits an SRB entry for a
  // storage resource whose GPU buffer or texture is already allocated, and on
  // Vulkan the SRB is the pipeline layout, so a missing binding makes
  // vkCreateGraphicsPipelines reject the pipeline the codegen declared.
  // ensureStorageResources is idempotent, so this runs once when the shared MRT
