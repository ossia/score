...
/// rebuilding that by hand gets it wrong: the blocks end `} renderer;` rather
/// than `};`, so slicing to the first `};` produces malformed source and a
/// compile error at token "renderer".
