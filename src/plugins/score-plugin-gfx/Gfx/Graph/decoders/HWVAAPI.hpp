...
#define SCORE_HAS_DRM_HWCONTEXT 1
#endif
}

#if QT_HAS_VULKAN && defined(SCORE_HAS_DRM_HWCONTEXT) && QT_VERSION >= QT_VERSION_CHECK(6, 6, 0)
...
/**
 * @brief Zero-copy VAAPI decoder for the Vulkan RHI backend.
 *
 * Maps VAAPI surfaces to DRM PRIME (DMA-BUF), then imports the per-plane
 * file descriptors into Vulkan VkImages via VK_KHR_external_memory_fd and
 * VK_EXT_image_drm_format_modifier.  The QRhiTextures are rewrapped each
 * frame with createFrom(), so the GPU samples directly from the decoder's
 * output memory — no pixel copy at all.
 *
 ...
 */
struct HWVaapiVulkanDecoder : GPUVideoDecoder
{
  ...
  // Ring buffer: slot N was last used 2 frames ago -> GPU is done with it.
  static constexpr int NumSlots = 2;
  struct ImportSlot
  {
    PlaneImport planes[2]{};
    AVFrame* vaapiRef{}; // prevents VAAPI surface reuse while GPU reads
  };
  ...
  void cleanupSlot(ImportSlot& slot)
  {
    ...
    if(slot.vaapiRef)
    {
      av_frame_free(&slot.vaapiRef);
      slot.vaapiRef = nullptr;
    }
  }
  ...
  // ------------------------------------------------------------------
  //  exec — import DMA-BUF per-plane into Vulkan, rewrap QRhiTextures
  // ------------------------------------------------------------------
  ...
  void exec(RenderList& r, QRhiResourceUpdateBatch& res, AVFrame& frame) override
  {
    ...
    {
      // Already a software frame — shouldn't happen with this decoder
      return;
    }
    ...
    // Hold a reference to the VAAPI surface to prevent content reuse
    slot.vaapiRef = av_frame_alloc();
    if(av_frame_ref(slot.vaapiRef, &frame) < 0)
    {
      av_frame_free(&slot.vaapiRef);
      slot.vaapiRef = nullptr;
      return;
    ...
    // Map VAAPI → DRM PRIME to obtain DMA-BUF file descriptors
    AVFrame* drmFrame = av_frame_alloc();
    drmFrame->format = AV_PIX_FMT_DRM_PRIME;

    int ret = av_hwframe_map(drmFrame, &frame,
                             AV_HWFRAME_MAP_READ | AV_HWFRAME_MAP_DIRECT);
    if(ret < 0)
    {
      qDebug() << "HWVaapiVulkanDecoder: av_hwframe_map failed:" << ret;
    ...
    if(!planesOk)
    {
      qDebug() << "HWVaapiVulkanDecoder: unexpected DRM layout, layers:"
               << desc->nb_layers;
      av_frame_unref(drmFrame);
      av_frame_free(&drmFrame);
      return;
    ...
    for(int i = 0; i < 2; ++i)
    {
      ...
      if(!m_importer.importPlane(slot.planes[i], obj.fd, obj.format_modifier,
                                pi.offset, pi.pitch, pi.format, pi.w, pi.h))
      {
        qDebug() << "HWVaapiVulkanDecoder: importPlane failed, plane" << i;
        av_frame_unref(drmFrame);
        av_frame_free(&drmFrame);
        return;
      }
...
#endif // QT_HAS_VULKAN && SCORE_HAS_DRM_HWCONTEXT
#endif // __linux__
