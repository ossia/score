...
/**
 * @brief Decodes YUVA444P12 videos.
 *
 * Full chroma resolution planar YUV (4:4:4) with alpha at 12-bit depth.
 * All four planes are at full resolution, stored as 16-bit words with
 * 12 bits of data in the low bits.
 */
struct YUVA444P12Decoder : GPUVideoDecoder
{
  static const constexpr auto frag = R"_(#version 450
...
void main ()
{
  ...
  float a = 16. * texture(a_tex, v_texcoord).r;

  vec4 rgb = processTexture(vec4(y,u,v, 1.));
  fragColor = vec4(rgb.rgb, a);
})_";

  explicit YUVA444P12Decoder(Video::ImageFormat& d)
      : decoder{d}
  {
  }
