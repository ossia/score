 ...
 * accept for an imported dma-buf: eglCreateImage accepts R16, R8 and ABGR8888,
 * binding any of them to GL_TEXTURE_2D fails, and GL_TEXTURE_EXTERNAL_OES
 * succeeds for all three. Without this decoder the zero-copy rung has no
 * consumer and capture falls back to staging whole frames out of uncached
 * pages.
 *
 ...
 *  - the sample arrives in `.r`: an R16 external image reads back
...
struct SCORE_PLUGIN_GFX_EXPORT BayerExternalOESDecoder : GPUVideoDecoder
{
