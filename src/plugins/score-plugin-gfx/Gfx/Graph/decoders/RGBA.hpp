...
struct PackedDecoder : GPUVideoDecoder
{
  ...
  /// Formats whose texel is reassembled from several bytes (big-endian 16-bit,
  /// sub-byte bit fields) MUST NOT be filtered: linear interpolation blends the
  /// bytes of adjacent texels before the filter can recombine them, so the
  /// reconstructed value is meaningless.
