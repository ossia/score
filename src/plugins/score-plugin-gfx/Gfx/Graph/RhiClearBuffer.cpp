...
// allocation across every clearBuffer call site: the vector grows once to the
// largest size requested and is reused for every subsequent call.
//
// Pattern != 0 hits a side path that materialises the requested 4-byte pattern
// into a separate vector. Every current call site uses the 0 path.
...
// Returns true if the backend's native path took the clear, false to request
// the shared fallback.
    ...
    quint32 pattern)
{
  ...
  // is the zero pool. A future revision could record a pending native
  // fill and apply it in the next CB-recording op.
  (void)rhi;
  clearViaBatch(batch, buf, offset, size, pattern);
}
