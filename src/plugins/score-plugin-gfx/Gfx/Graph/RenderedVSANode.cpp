    ...
    QRhiResourceUpdateBatch& res)
{
  ...
  try
  {
    ...
    // VSA draws are never face-culled: CullMode::None on every backend. This must
    // run after m_mesh->preparePipeline(), which unconditionally calls setCullMode()
    // and setFrontFace().
    //
    // Face culling is decided from the triangle's window-space winding sign, which
    // QRhi does not normalise across backends. It stays consistent only for shaders
    // following the QRhi convention -- multiply gl_Position by clipSpaceCorrMatrix()
    // and do not flip Y on SPIRV/Vulkan -- which is what ISF blit_vs,
    // RenderedRawRasterPipelineNode and the RGBA decoder do, and they all cull with
    // a single CullMode::Back.
    //
    // VSA does the opposite: it skips clipSpaceCorrMatrix and applies
    // `gl_Position.y = -y` on SPIRV/HLSL/MSL. That keeps the image orientation
    // consistent but inverts the window-space winding sign on Vulkan relative to
    // OpenGL, so for any single triangle winding exactly one of the two keeps the
    // face and the other culls it: no per-backend CullMode + FrontFace pair makes
    // one front-facing VSA triangle visible on both.
    //
    // VSA art is 2-D procedural geometry driven by gl_VertexIndex, where front
    ...
    RenderList& renderer, QRhiResourceUpdateBatch& res, Edge* edge)
{
  ...
  for(auto& audio : n.m_audio_textures)
  {
       ...
       = m_audioTex->updateAudioTexture(audio, renderer, n.m_material_data.get(), res))
    {
      ...
      // Keep m_audioSamplers[i].texture in sync with the live GPU texture: when a pass
      // is torn down and rebuilt, allSamplers() must hand buildPipeline the current
      // texture. Otherwise the fresh SRB binds &renderer.emptyTexture(), since
      // initAudioTextures leaves texture null, and no later updateAudioTexture
      // re-triggers replaceTexture -- its no-change path returns {} -- so the shader
