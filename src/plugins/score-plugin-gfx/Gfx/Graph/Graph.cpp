...
Graph::createRenderList(OutputNode* output, std::shared_ptr<RenderState> state)
{
  ...
  output->onRendererChange();
  {
    ...
    // Mark built. Skips the wasteful and previously-dangerous mid-frame
    ...
    // once on the next frame in maybeRebuild), causing multi-second
    // resizes for non-trivial scenes. That mid-frame teardown was also
    // the root of a command-buffer cascade bug. The safety net (a
    // synchronous drain in maybeRebuild) stays in place for forced
    // rebuilds and the actual size-change cycle in maybeRebuild on
    // subsequent frames.
    //
    // Null processUBO in MRT blit passes, feedback ISF persistent
    // textures, and surgical rt_changed handling are all handled
    // correctly here. The two missing pieces vs maybeRebuild's
    // release+init (m_requiresDepth recompute, markBuilt) are done here.
    r.markBuilt();
  }
