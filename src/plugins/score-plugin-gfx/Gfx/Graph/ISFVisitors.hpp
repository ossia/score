...
{
// ---------------------------------------------------------------------------
// Descriptor port walker
// ---------------------------------------------------------------------------
//
// The single source of truth for "how many input ports / output ports /
// samplers does each desc.inputs entry produce?", shared by CSF port_indices,
// RawRaster port_idx, RawRaster bindAuxTexturesInit and IsfBindingsBuilder.
// Mirrors `isf_input_port_vis` in ISFNode.cpp, which is the actual
// port-creation code.
//
// When a new isf::*_input variant is added, update isf_input_port_vis and the
// matching `operator()` here -- keep them in lockstep.
struct port_counts
{
...
struct isf_input_port_count_vis
{
  ...
  {
    // GrabsFromSource means no own render target, so the matching depth
    // sampler (image_input.depth==true) is not created in initInputSamplers
    // either.
...
struct isf_input_size_vis
{
  ...
  {
    // Must match what isf_input_port_vis (ISFNode.cpp) writes into the blob,
    // and the synthesized "size" int it creates: only a writable buffer whose
    // layout ends in a flexible-array member. Reserving for every write buffer
    // would desync this from the port visitor and the generated GLSL
    // Params/material_t block.
  ...
  {
    // isf_input_port_vis writes nothing into the material blob for write
    // csf_image inputs (its point2d/long synthesis is commented out), so
    // reserve nothing here: the size visitor, the port visitor and the
    // generated uniform block must agree.
  }
