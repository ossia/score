...
public:
   ...
   *
   * We `delete` the QRhiBuffer / QRhiTexture / QRhiSampler wrappers
   * directly: the QRhi is still alive at this call site (callers MUST
   * invoke this BEFORE RenderState::destroy() / setSwapchainFormat-style
   * teardown), so the destructors free both the wrapper and the
   * underlying GPU resource cleanly. After this call the registry is
   * back to its pre-init() state and can be re-init()'d against a new
   * QRhi.
   */
  void destroyOwned();
