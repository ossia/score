 ...
 * on an older driver this class refuses to construct, leaving the
 * consumer to fall back to `cuMemAlloc` plus host-staged transfers.
 */
...
public:
   ...
   *  cuMemExportToShareableHandle. The caller owns the fd; the Vulkan import
   *  consumes it on success. Returns -1 on failure. */
   ...
   *  (the export returns CUDA_ERROR_NOT_SUPPORTED). Where the attribute is
   *  set and GPUDirect RDMA is available, the export itself succeeds and
   ...
   *  the CUDA-exported dma-buf is still unsupported on NVIDIA, though:
...
public:
   ...
   *                     sharing or Vulkan import). Costs a little more,
   *                     so set it only when the allocation is exported.
