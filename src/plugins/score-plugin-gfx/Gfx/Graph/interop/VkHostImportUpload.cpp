...
{
  // Report which check failed rather than falling back silently. qWarning,
  // not qDebug: some targets strip debug output.
    ...
    int height, std::size_t srcOffset, std::size_t rowPitchBytes) noexcept
{
  ...
  // beginExternal flushes QRhi's pending work and hands back the live
  // VkCommandBuffer.
  cb.beginExternal();
  const auto* nh
