...
public:
  ...
  void endProducerFrame(QRhiCommandBuffer* cb) override
  {
    ...
    // beginExternal flushes QRhi's command buffer to the D3D11 context, so
    // the render commands execute before the CopyResource below.
    cb->beginExternal();

    // The consumer may skip a slot under triple buffering: one released with
    // key 1 and never acquired stays there, so reset it to key 0 first.
    ...
    // Must check for S_OK specifically, not SUCCEEDED():
    // SUCCEEDED(WAIT_TIMEOUT) is true even though the mutex was not acquired.
...
public:
  ...
  void endProducerFrame(QRhiCommandBuffer* cb) override
  {
    ...
    // Signal the fence for any slot left pending by the PREVIOUS frame, so
    // the signal happens after QRhi submitted that frame's commands.
...
public:
  bool setupProducer(QRhi* producer, QSize size, QRhiTexture::Format format) override
  {
    ...
    // Semaphore functions are optional.
#endif
    ...
    // External semaphores are optional: without them the sharing falls back
    // to triple buffering alone.
  ...
  QRhiTexture* acquireConsumerTexture(QRhiCommandBuffer* cb) override
  {
    ...
    // Without semaphores, triple buffering alone keeps the two sides apart:
    // it works, at the cost of occasional visual artifacts.
    ...
    QRhi* producer, QRhi* consumer, QSize size, QRhiTexture::Format format)
{
  ...
  // Publish the backend as initialized and safe to access from other threads.
  // MUST come after setup() completes, or other threads race on its members.
  if(result)
...
bool TextureShare::setupProducer(QRhi* producer, QSize size, QRhiTexture::Format format)
{
  ...
  // Publish the backend as initialized and safe to access from other threads.
  // MUST come after setupProducer() completes, or other threads race on its
  // members.
  if(result)
    m_impl->backendCreated.store(true, std::memory_order_release);
