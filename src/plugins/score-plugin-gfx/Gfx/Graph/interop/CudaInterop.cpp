...
struct CudaInteropContext_t
{
  ...
  std::mutex mutex;
  // Aligned-pointer -> real cuMemAlloc base, for over-aligned allocations.
  std::unordered_map<void*, CUdeviceptr> alignedAllocs;
};
...
/// has been built so far itself. Otherwise a failure after
/// cuDevicePrimaryCtxRetain leaks both the object and a permanent reference on
    ...
    CudaInteropResourceHandle* out_handle)
{
  ...
  *out_handle = nullptr;
  ...
  // yield a flat device pointer via cuGraphicsResourceGetMappedPointer,
  // which is what AJA's DMABufferLock(inRDMA=true) accepts.
...
CudaInteropError cuda_interop_gl_read_buffer(
    CudaInteropContextHandle ctx, CudaInteropResourceHandle h, void* dst_device_ptr,
    uint32_t size)
{
  if(!ctx || !h || !h->graphicsResource || !dst_device_ptr || size == 0)
    return CUDA_INTEROP_ERROR_INVALID_PARAMETER;
  std::lock_guard<std::mutex> lock(ctx->mutex);
  CU_CHECK(
      ctx->cu.ctxSetCurrent(ctx->cuContext), ctx, CUDA_INTEROP_ERROR_COPY_FAILED,
  ...
  CU_CHECK(
      ctx->cu.graphicsMap(1, &h->graphicsResource, ctx->stream), ctx,
      CUDA_INTEROP_ERROR_INTEROP_FAILED, "cuGraphicsMapResources(GL read)");

  CUdeviceptr srcPtr{};
  size_t mappedSize = 0;
  if(ctx->cu.graphicsGetMappedPointer(&srcPtr, &mappedSize, h->graphicsResource)
         != CUDA_SUCCESS
     || !srcPtr || mappedSize < size)
  {
    ctx->cu.graphicsUnmap(1, &h->graphicsResource, ctx->stream);
    ctx->lastError = "cuGraphicsResourceGetMappedPointer(GL read) failed";
    return CUDA_INTEROP_ERROR_INTEROP_FAILED;
  }

  CUresult cr = ctx->cu.memcpyDtoDAsync(
      reinterpret_cast<CUdeviceptr>(dst_device_ptr), srcPtr, size, ctx->stream);
  if(cr == CUDA_SUCCESS)
    cr = ctx->cu.streamSync(ctx->stream);
  ...
  if(cr != CUDA_SUCCESS)
    return reportCuError(
        ctx, cr, CUDA_INTEROP_ERROR_COPY_FAILED, "cuMemcpyDtoDAsync(GL read)");
  return CUDA_INTEROP_SUCCESS;
}
    ...
    CudaInteropContextHandle ctx, uint64_t size, void** out_device_ptr)
{
  ...
  *out_device_ptr = nullptr;
  ...
  CU_CHECK(
      ...
      "cuMemAlloc");
  // Third-party DMA engines (AJA DMABufferLock inRDMA=true via
  // nvidia_p2p_get_pages) need GPU-page-aligned VAs. cuMemAlloc suballocates
  // requests under ~1 MB from a pool at 512-byte alignment, which can land a
  // frame slot part-way into a page and make every AutoCirculateTransfer
  // against it fail, so re-allocate over-sized and hand out an aligned
  // interior pointer. (CUDA VMM memory is NOT a substitute
  // here: this driver pins it fine but the card->GPU writes never land.)
  constexpr uint64_t kGpuPage = 64 * 1024;
  if(dptr % kGpuPage != 0)
  {
    ctx->cu.memFree(dptr);
    CUdeviceptr raw{};
    CU_CHECK(
        ctx->cu.memAlloc(&raw, size + kGpuPage), ctx,
        CUDA_INTEROP_ERROR_ALLOC_FAILED, "cuMemAlloc(aligned)");
    dptr = (raw + kGpuPage - 1) & ~(kGpuPage - 1);
    ctx->alignedAllocs.emplace(
        reinterpret_cast<void*>(static_cast<uintptr_t>(dptr)), raw);
  }
  ...
  // in cuda_interop_copy_dtod is correct either way.
...
void cuda_interop_free_buffer(CudaInteropContextHandle ctx, void* device_ptr)
{
  ...
  if(ctx->cu.ctxSetCurrent(ctx->cuContext) != CUDA_SUCCESS)
    return;
  if(auto it = ctx->alignedAllocs.find(device_ptr);
     it != ctx->alignedAllocs.end())
  {
    ctx->cu.memFree(it->second);
    ctx->alignedAllocs.erase(it);
    return;
  }
    ...
    uint64_t height)
{
  ...
  m.WidthInBytes = width_bytes;
  m.Height = height;
  // cuMemcpy2D rejects device pitches cuMemAllocPitch would not produce
  // (SD UYVY rows are 1440 bytes); cuMemcpy2DUnaligned accepts any pitch.
  if(const CUresult r = ctx->cu.memcpy2DAsync(&m, ctx->stream);
     r != CUDA_SUCCESS)
  {
    if(!ctx->cu.memcpy2DUnaligned)
      return reportCuError(
          ctx, r, CUDA_INTEROP_ERROR_COPY_FAILED, "cuMemcpy2DAsync(dtod2d)");
    // cuMemcpy2DUnaligned takes no stream, so pending stream work has to be
    // drained here rather than by the caller's stream ordering.
    CU_CHECK(
        ctx->cu.streamSync(ctx->stream), ctx, CUDA_INTEROP_ERROR_COPY_FAILED,
        "cuStreamSynchronize");
    CU_CHECK(
        ctx->cu.memcpy2DUnaligned(&m), ctx, CUDA_INTEROP_ERROR_COPY_FAILED,
        "cuMemcpy2DUnaligned(dtod2d)");
    return CUDA_INTEROP_SUCCESS;
  }
  CU_CHECK(
      ctx->cu.streamSync(ctx->stream), ctx, CUDA_INTEROP_ERROR_COPY_FAILED,
