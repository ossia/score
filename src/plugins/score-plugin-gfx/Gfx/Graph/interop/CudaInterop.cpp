    ...
    CudaInteropContextHandle ctx, uint64_t size, void** out_device_ptr)
{
  ...
  *out_device_ptr = nullptr;
  ...
  // requests under ~1 MB from a pool at 512-byte alignment (measured: an SD
  // frame slot landed at offset 0xE00 into a page, and every
  // AutoCirculateTransfer against it failed), so re-allocate over-sized and
  // hand out an aligned interior pointer. (CUDA VMM memory is NOT a substitute
