 ...
 * `HostPinnedRing` and (where supported) tier-0 P2P backend can use:
 *
 ...
 *     exercises this for tier-0 SDI capture/output.
 *
 ...
 * planned AMD tier-0 strategy); other backends — Vulkan, D3D11, Metal
...
struct SCORE_PLUGIN_GFX_EXPORT AmdPinnedBuffers
{
  ...
  /** GL_AMD_bus_addressable_memory — tier-0 P2P (AJA-AMD path). */
   ...
   *  third-party DMA engines via its bus address. Tier-0 path.
   *
