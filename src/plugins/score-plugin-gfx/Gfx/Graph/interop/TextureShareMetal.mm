...
public:
  ...
  bool setupConsumer(QRhi* consumer) override
  {
    @autoreleasepool {
      ...
      for (int i = 0; i < 3; ++i)
      {
        // Consumer Metal texture backed by the same IOSurface: zero-copy.
  ...
  {
    // Both sides, set up from the calling thread.
  ...
  void endProducerFrame(QRhiCommandBuffer* cb) override
  {
    @autoreleasepool {
      ...
      // With IOSurface there is no GPU copy: producer and consumer already
      // share the same memory. The shared event still has to be signalled on
      // the native command buffer so the consumer knows when rendering is
      // complete.
  ...
  QRhiTexture* acquireConsumerTexture(QRhiCommandBuffer* cb) override
  {
    @autoreleasepool {
      ...
      // Wait for the producer to finish rendering into this slot.
      auto* cbHandles = static_cast<const QRhiMetalCommandBufferNativeHandles*>(
          cb->nativeHandles());
      if (cbHandles && cbHandles->commandBuffer)
      {
      ...
      // No copy: the consumer texture is backed by the same IOSurface.
