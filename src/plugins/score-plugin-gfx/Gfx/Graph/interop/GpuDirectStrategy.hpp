 ...
 * and the supporting helpers (`GpuRingBuffer`, `InteropFence`,
 * `ComputeRingDispatcher`, `GpuDirectOutput`) are in score-plugin-gfx so
 * the per-vendor file shrinks to ~50 lines of "construct a
 * `GpuDirectOutput`, fill in the pin/unpin callbacks".
 *
 * Lifecycle:
 ...
 *      ...) as constructor args.
 *   2. Addon calls `init(GpuDirectStrategyConfig{rhi, state, sourceTex,
...
/**
 * @brief Generic config passed to `GpuDirectStrategy::init`. Vendor-
 *        specific bits (card handle, pixel-format enum, ...) live on
 *        the strategy class itself.
 */
struct GpuDirectStrategyConfig
{
  QRhi* rhi{};
 ...
 */
struct GpuDirectStrategy
{
  virtual ~GpuDirectStrategy() = default;
  ...
  virtual bool init(const GpuDirectStrategyConfig& cfg) = 0;
