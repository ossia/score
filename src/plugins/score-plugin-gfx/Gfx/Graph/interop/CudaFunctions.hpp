 ...
 *   - Gfx/Graph/interop/ — vendor-neutral CUDA interop (CudaInterop,
 *     RdmaGpuBuffer, HostPinnedRing, CudaVmmAllocator, GpuCapabilities):
 *     AJA RDMA, planned Magewell + Rivermax
 *
 * Each embeds a `CudaFunctions` and calls `load()` once. The function
...
typedef enum CUdevice_attribute_enum
{
  ...
  // this device. 0 on Turing/Ada consumer and Quadro RTX parts, so
  // CUDA→dma-buf export to Vulkan/GL is unavailable there. Non-zero on
...
struct CudaFunctions
{
  ...
  // Synchronous variant without cuMemcpy2D's device-pitch restriction
  // (pitches cuMemAllocPitch would not produce, e.g. SD UYVY's 1440-byte
  // rows, make cuMemcpy2DAsync return CUDA_ERROR_INVALID_VALUE).
  ...
  bool load()
  {
    ...
    memcpy2DUnaligned = (FN_cuMemcpy2DUnaligned)sym("cuMemcpy2DUnaligned_v2");
    ...
    // these unversioned on both Linux and Windows in 10.2+.
   ...
   *  on Turing/Ada Quadro and GeForce parts, so
