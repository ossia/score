...
/**
 * @file VideoOutputStrategySelect.hpp
 * @brief Generic "try GPU-direct output strategies in order" picker.
 *
 * Every output addon walks the same DVP -> RDMA -> (CPU) candidate chain:
...
inline std::unique_ptr<VideoOutputStrategy> selectVideoOutputStrategy(
    const VideoOutputStrategyConfig& cfg,
