...
bool ImportedGpuBufferRing::createOpenGL()
{
  ...
  for(auto& slot : m_slots)
  {
    ...
    {
      // Register only; gpuDevicePtr stays null. The consumer maps and unmaps
      // around each frame's copy (cuda_interop_gl_write_buffer /
      // cuda_interop_gl_read_buffer) so the buffer is GL-owned whenever GL
      // touches it.
...
bool ImportedGpuBufferRing::createD3D12Stub()
{
  qDebug() << "ImportedGpuBufferRing(D3D12): unsupported — QRhi D3D12 backend "
              ...
              "doesn't expose inRDMA either (so D3D12 zero-copy is moot "
