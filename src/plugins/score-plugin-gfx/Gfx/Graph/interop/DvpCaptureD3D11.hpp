...
struct DvpCaptureD3D11 : score::gfx::interop::VideoCaptureStrategy
{
  ...
  bool init(const score::gfx::interop::VideoCaptureStrategyConfig& c) override
  {
    ...
    // proves DVP accepted the descriptors, not that a transfer can complete --
    // a device can fail every dvpMapBufferEndAPI(tex) with -1 while the rung
    // reports itself engaged and produces no frames. Same reasoning as
    // VendorDmaRegistrar::verifyTransfer on the output side: fail init so the
    // chain falls back cleanly instead of degrading into a per-frame failure
    // the renderer cannot recover from. The probe is unproven against a working
    // DVP setup, so SCORE_GFX_DVP_NO_PROBE=1 skips it without a rebuild.
  ...
  bool ingestFrame(std::size_t i) override
  {
    ...
    if(nv_dvp_copy_buffer_to_texture(m_dvpCtx, slot.dvpBuf, m_dvpTex)
       != NV_DVP_SUCCESS)
    {
      // A failed transfer is not transient: every frame takes the same path
      // with the same buffers. Retrying forever spins the capture thread at
      // full CPU and starves the consumer, which presents as a hang rather
      // than as a degraded rung, so give up after a few strikes and let the
      // caller fall back.
      if(++m_failures <= 3)
