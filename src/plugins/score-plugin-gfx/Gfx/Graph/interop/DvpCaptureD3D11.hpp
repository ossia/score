...
struct DvpCaptureD3D11 : score::gfx::interop::VideoCaptureStrategy
{
  ...
  bool init(const score::gfx::interop::VideoCaptureStrategyConfig& c) override
  {
    ...
    // One real transfer before claiming the rung. Registering the buffers only
    // proves DVP accepted the descriptors; it does not prove a transfer can
    // complete -- on this Quadro every dvpMapBufferEndAPI(tex) returns -1 and
    // the rung produced no frames at all while reporting itself engaged. Same
    // reasoning as VendorDmaRegistrar::verifyTransfer on the output side:
    // fail init so the chain falls back cleanly, instead of degrading to a
    // per-frame failure the renderer cannot recover from.
    // Escape hatch: the probe is a safety net against an unrecoverable
    // per-frame failure, but no working DVP setup exists on any machine here
    // to prove it never false-positives on a healthy one. If it ever refuses a
    // rung that actually works, SCORE_GFX_DVP_NO_PROBE=1 restores the old
    // behaviour without a rebuild.
    if(!qEnvironmentVariableIsSet("SCORE_GFX_DVP_NO_PROBE"))
    {
    if(m_slots[0].dvpBuf && m_dvpTex
       && nv_dvp_copy_buffer_to_texture(m_dvpCtx, m_slots[0].dvpBuf, m_dvpTex)
              != NV_DVP_SUCCESS)
    {
      qWarning() << "DVP-IN(D3D11): probe transfer failed:"
                 << nv_dvp_get_error_string(m_dvpCtx)
                 << "- DVP capture unusable here, falling back";
      release();
      return false;
    }
    }
  ...
  /// False once the rung has failed enough consecutive transfers to be
  /// considered unusable; the caller should fall back rather than keep
  /// feeding it.
  bool healthy() const noexcept { return !m_dead; }
  ...
  bool ingestFrame(std::size_t i) override
  {
    ...
    if(nv_dvp_copy_buffer_to_texture(m_dvpCtx, slot.dvpBuf, m_dvpTex)
       != NV_DVP_SUCCESS)
    {
      // A transfer that fails is not transient here: every frame takes the
      // same path with the same buffers. Retrying forever spins the capture
      // thread at full CPU and starves the consumer of frames it will never
      // get, which presents as a hang rather than as a degraded rung. Mirror
      // the output side and give up after a few strikes so the caller can
      // fall back.
      if(++m_failures <= 3)
      ...
      {
        m_dead = true;
        qWarning() << "DVP-IN(D3D11): giving up after" << kMaxFailures
                   << "consecutive failures; the DVP capture rung is unusable "
                      "on this setup";
      }
      return false;
    }
