...
/**
 * @brief Register a GL Shader Storage Buffer Object for P2P and keep it mapped
 * for the resource's lifetime, yielding a stable device pointer.
 * GL <-> CUDA buffer interop is direct. The GL context must be current
 * on the calling thread.
 *
 * @warning Only valid when GL never touches the buffer again after the import:
 * cuGraphicsMapResources transfers ownership to CUDA, and GL access to a mapped
 * resource is undefined. If GL writes the buffer per frame, use
 * cuda_interop_register_gl_buffer plus cuda_interop_gl_read_buffer instead.
 */
 ...
 * mapped for its lifetime), this registers the buffer and immediately returns it
 * to GL ownership, so GL may keep using it. Every CUDA access then goes through
 * cuda_interop_gl_write_buffer / cuda_interop_gl_read_buffer, which map, copy
 * and unmap — the map/unmap pair is both the ownership transfer and the
 * coherency point. A GL buffer that stays mapped while CUDA writes it is never
 * seen coherently by GL, and one that stays mapped while GL writes it is
 * undefined by the same contract.
 */
...
/**
 * @brief Map a GL buffer registered with cuda_interop_register_gl_buffer, copy
 *        @p size bytes out of it into @p dst_device_ptr, then unmap. The
 *        read-direction mirror of cuda_interop_gl_write_buffer: the map is what
 *        transfers ownership of the GL-written bytes to CUDA, and the unmap
 *        hands the buffer back so the next GL dispatch may write it again.
 *        Stream-synchronised on return.
 */
CUDA_INTEROP_API CudaInteropError cuda_interop_gl_read_buffer(
    CudaInteropContextHandle ctx,
    CudaInteropResourceHandle h,
    void* dst_device_ptr,
    uint32_t size);
