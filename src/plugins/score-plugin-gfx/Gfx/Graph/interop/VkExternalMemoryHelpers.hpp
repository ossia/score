 ...
 *   - the RDMA buffer allocator (export VkBuffer for CUDA P2P)
 *
 * Design notes:
 ...
 *
 * Symmetric with createExportableImage.
 */
...
// =============================================================================
// Cleanup — symmetric helpers. Out-of-line because they dispatch through
// QVulkanInstance::deviceFunctions(), which would otherwise drag
 ...
 *
 * Used during PipeWire EnumFormat construction: only importable modifiers are
 * advertised, so producers can fixate on a supported choice.
 *
 * Returns false without calling into Vulkan if
 * VK_EXT_image_drm_format_modifier isn't loaded; the shared layer's
 * format_negotiation then falls back to LINEAR, which is always supported.
 */
