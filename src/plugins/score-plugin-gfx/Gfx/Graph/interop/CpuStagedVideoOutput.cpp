    ...
    std::unique_ptr<score::gfx::GPUVideoEncoder> enc1)
{
  ...
  // which is where it is faster. The copy-based backends are a regression
  // instead, because their fence wait lands on the render thread -- so they
  // keep the plain QRhi staging path unless asked otherwise.
  // SCORE_GFX_DIRECT_READBACK=always|import|never overrides ("always" also
  // takes the copy-based backends; "never" is the kill switch).
  ...
  // fall through to the unchanged CPU staging ring below.
     ...
     && s->enc[0]->outputTexture() && s->enc[1]->outputTexture())
  {
    ...
    if(ts.width() > 0 && ts.height() > 0 && slotBytes == s->cfg.frameByteSize)
    {
         ...
         && s->pinnedRing.backend() != HostPinnedRingBackend::None)
      {
        ...
        // Entering DVP mode DISABLES the encoder readback, so if downloads
        // then fail there is nothing left to produce a frame --
        // prepareNextFrame returns nullptr forever and the card is handed
        // nothing.
...
void* CpuStagedVideoOutput::prepareNextFrame()
{
  ...
  if(s.dvpMode)
  {
    ...
    if(!s.pinnedRing.downloadTextureToSlot(tex, slotIdx))
    {
      ...
      // Log the first few rather than dropping frames silently.
