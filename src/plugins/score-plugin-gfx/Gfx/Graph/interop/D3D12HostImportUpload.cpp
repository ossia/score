...
/// the source footprint's format to match the destination resource's, so a
/// hardcoded RGBA8 fails every BGRA8 capture: the input stops locking on D3D12
/// while the same format passes on Vulkan.
...
bool D3D12HostImportUpload::pitchUsable(std::size_t rowPitch) noexcept
{
  ...
  if(rowPitch % D3D12_TEXTURE_DATA_PITCH_ALIGNMENT == 0)
    return true;
  // Some drivers handle an unaligned pitch, but it is undocumented: the runtime
  // is free to reject or mis-stride it, so it stays behind an explicit opt-in.
