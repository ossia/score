...
HostPinnedRingBackend pickBackend(const HostPinnedRingConfig& cfg) noexcept
{
  ...
  // Checked before DVP: DVP is NVIDIA-only, but `dvpLoaded` merely says libdvp is
  // installed, which it can be on a box whose GPU is a Radeon.
  //
  ...
  // been rejected ... (-14)"), aborting the process from its own CS thread, so no
  // in-process check can demote out of it. AMD's Windows driver runs the same rung
  // correctly, measured on a W6800, and the driver string is what separates them:
  // Mesa puts "Mesa" in it, the Windows driver reports a bare version.
...
struct HostPinnedRing::Impl
{
  ...
  //
  // One persistent QRhiReadbackResult per slot, allocated once in createSlots()
  // and reused; the completion lambda captures `this` plus the slot index and
  // writes into slots[i]. Per-slot atomics synchronise: pending[i] is set between
  // enqueue and completion, and a re-enqueue on a pending slot returns false;
  // ready[i] is set once the completion lambda has copied bytes into slot.host,
  // which the vendor strategy polls and then clears.
  //
  // Teardown contract: the caller must drain until nothing is pending before
  // destroying the ring. QRhi holds a raw pointer to the QRhiReadbackResult
  // between batch submit and completion. destroy() warns if it sees pending
  // readbacks.
  ...
  // -- CudaHostReg backend --------------------------------------------
  //
  // Page-locks each slot with cuMemHostRegister. The QRhi-side transfer stays
  // per-backend: a CUDA-imported destination gives a CUarray to cuMemcpy2D into,
  // while every other QRhi backend has no cheap path from registered host memory
  // to a texture without an intermediate copy, so upload demotes to CPU staging
  // and the pinning is a perf-only benefit.
...
bool HostPinnedRing::create(const HostPinnedRingConfig& cfg)
{
  ...
  const auto picked = m_impl->backend;
  bool ok = false;
  ...
  // Which rung actually engaged, and whether it is the one that was picked.
  // Without this a silent demotion is indistinguishable from never having
  // tried, which is exactly the question a harness row has to answer.
  if(m_impl->backend != picked)
    qWarning() << "HostPinnedRing" << cfg.debugName << ": picked"
               << hostPinnedBackendName(picked) << "but demoted to"
               << hostPinnedBackendName(m_impl->backend);
  else
    qDebug() << "HostPinnedRing" << cfg.debugName << ": engaged"
             << hostPinnedBackendName(m_impl->backend);
