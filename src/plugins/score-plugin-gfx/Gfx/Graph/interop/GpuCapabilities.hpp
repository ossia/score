 ...
 * needs to decide at init time which GPU interop mechanism to use:
 *
 *   - true GPU-direct (CUDA VMM + nvidia_peermem, or AMD
...
{
  Unknown = 0,
  NvidiaConsumer,   /**< GeForce. GPU-direct RDMA is unavailable (driver-gated). */
  NvidiaProQuadro,  /**< Quadro / RTX A-series / Tesla. GPU-direct RDMA enabled by driver. */
...
struct AmdGlExtensions
{
  ...
  bool externalVirtualMemory{};  /**< GL_EXTERNAL_VIRTUAL_MEMORY_BUFFER_AMD / GL_EXTERNAL_VIRTUAL_MEMORY_AMD — sysmem virtual pinning. */
  bool externalPhysicalMemory{}; /**< GL_EXTERNAL_PHYSICAL_MEMORY_AMD — sysmem physical pinning. */
...
struct SCORE_PLUGIN_GFX_EXPORT GpuCapabilities
{
   ...
   *  cuMemSetAccess) available. Required for direct NIC→VRAM
   *  Rivermax-style allocation. */
  bool cudaVmmSupported{};
  /** nvidia_peermem kernel module loaded (Linux). Gates AJA-RDMA and
   *  Rivermax/Ximea direct-VRAM paths. Always false on non-Linux. */
  bool nvidiaPeermem{};
  ...
  /** Running on macOS with Metal-capable GPU. Implies Metal-IOS
