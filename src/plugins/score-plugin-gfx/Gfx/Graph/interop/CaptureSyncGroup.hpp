 ...
 * it, so a rig that is quietly tearing shows up as a number.
 *
 ...
 * crosses threads.
 *
 * Vendor-neutral: it serves any backend that can hand over N slots belonging to
 * one capture.
 */
...
public:
  ...
  /// slot can be stranded by the producer outrunning the renderer.
