 ...
 *   - Vulkan  : a ring of exportable binary VkSemaphores (OPAQUE_FD /
 *               OPAQUE_WIN32) imported into CUDA via
 ...
 *
 * Keeps the `glFinish` / D3D11-immediate-context-flush handling in one place
 * instead of open-coded in each per-backend output path, and gives a stable
 * hook for the D3D12 path when it lands.
 */
