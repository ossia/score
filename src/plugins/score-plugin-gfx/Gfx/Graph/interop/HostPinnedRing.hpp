 ...
 * @brief Vendor-neutral N-slot sysmem ring for DVP / AMD-pinned / host-copy
 *        transfers.
 *
 * Counterpart to `ImportedGpuBufferRing` (which holds GPU-side buffers
 * for true direct P2P). `HostPinnedRing` holds host-side page-locked buffers
 ...
 * host RAM on the hot path: DeckLink (all variants), AJA's DVP path,
...
SCORE_PLUGIN_GFX_EXPORT const char*
hostPinnedBackendName(HostPinnedRingBackend) noexcept;
...
public:
   ...
   *  the host buffer is then perf-neutral, since the QRhi upload does
   *  not see the pinning. The zero-copy path requires the binding
   *  above. */
