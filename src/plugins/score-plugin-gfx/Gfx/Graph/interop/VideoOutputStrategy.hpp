 ...
 *
 * Any peer device that can "give me a GPU pointer I can DMA from" (capture
 * cards via a vendor pin/register call, plus NVIDIA DVP and SMPTE 2110
 * variants) can hang strategies off the same dispatch chain inside its addon.
 *
 * Concrete implementations live in each vendor's addon; only the interface
 * and the supporting helpers (`ImportedGpuBufferRing`, `InteropFence`,
 * `ComputeRingDispatcher`, `RdmaVideoOutput`) live here, so a per-vendor file
 * shrinks to ~50 lines: construct a `RdmaVideoOutput`, fill in the pin/unpin
 * callbacks.
 *
 * Lifecycle:
...
struct VideoOutputStrategy
{
  ...
  /// "RDMA-D3D11" / "Magewell-NVRDMA/GL". Dispatch sites print this
