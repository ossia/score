 ...
 * exportable-image / CUDA-import machinery, so the acquisition lives here
 * rather than once per shim.
 *
 * This is the common denominator of the AJA and Deltacast Vulkan RDMA paths.
 * Their transfer machinery does not converge -- AJA uses
 * per-slot LINEAR images imported as flat CUDA buffers (cuda_interop copy
 ...
 * (cuda_interop copy buffer<->array) backed by RdmaGpuBuffer. These are
 * deliberate, correctness-driven choices (AJA notes an OPTIMAL-image-as-array
 * "scrambles per-tile on this driver"), so a shared transfer base is a
 * redesign, not a mechanical move.
 */
