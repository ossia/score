...
/**
 * @file RdmaVideoOutput.hpp
 * @brief Vendor-neutral GPU-direct output strategy.
 *
 * Generalises the AJA RDMA output path so any peer device with a
 ...
 *   - A `registerSlot(gpuPtr, size)` callback the helper invokes once per
 *     slot at init time. Typical impl: `card->DMABufferLock(...)`.
 *   - A `releaseSlot(gpuPtr, size)` callback invoked at shutdown.
 *
...
// adapter) lives in VendorDmaRegistrar.hpp, shared with CpuStagedVideoOutput.

struct RdmaVideoOutputConfig
{
  QRhi* rhi{};
 ...
 */
class SCORE_PLUGIN_GFX_EXPORT RdmaVideoOutput
{
public:
  RdmaVideoOutput() = default;
  ~RdmaVideoOutput();

  RdmaVideoOutput(const RdmaVideoOutput&) = delete;
  RdmaVideoOutput& operator=(const RdmaVideoOutput&) = delete;
   ...
   */
  bool init(const RdmaVideoOutputConfig& cfg);
