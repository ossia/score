...
struct GbmDmaBufExport
{
  ...
  bool init() noexcept
  {
    ...
    // _with_modifiers2 is newer (Mesa 22+); fall back to _bo_create.
    ...
    // Open the DRM render node: the privilege-free entry point, which lets
    // unprivileged processes render without master access to the display.
    // /dev/dri/renderD128 is the first card; multi-GPU selection would go
    // through eglQueryDevicesEXT.
