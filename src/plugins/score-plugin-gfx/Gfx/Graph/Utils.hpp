 ...
 * named/owned by the node (so textureForOutput() can return it).
 *
 ...
 */
SCORE_PLUGIN_GFX_EXPORT
std::pair<QShader, QShader> makeShaders(
    const RenderState& v, QString vert, QString frag, int multiViewCount = 0);
