...
bool RenderList::resizeSwapchainSizedTargets(QSize newSize)
{
  ...
  // release+init cycle. With the persistent GpuResourceRegistry and
  // the rt_changed downstream-size propagation
  // (createAllInputRenderTargets), maybeRebuild is now cheap enough
  // to be the correct way to handle resize.
  //
...
void RenderList::render(QRhiCommandBuffer& commands, bool force)
{
  ...
  if(state.rhi)
  {
    ...
    // or (b) race a shared thread_local cooldown. That would require adding
    // fields to RenderList.hpp. frameNumber comes from this->frame (not a
    // process-/thread-global counter), and the decrement now ticks every
    // frame below rather than only inside the stall branch.
