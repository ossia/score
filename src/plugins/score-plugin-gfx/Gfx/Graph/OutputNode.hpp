...
public:
   ...
   *
   * Persist-across-rebuild contract: ownership is on the OutputNode, not
   * the RenderList, so the texture-array data, the mesh slabs and the
   * producer arena slot indices — all of which describe scene content,
   * not framebuffer state — survive across
