...
public:
  ...
  /// operator()() once during reconciliation and immediately push the result
  /// into each downstream sink's per-port scene cache. Otherwise a
  /// live-inserted producer's output only reaches `m_portScenes` when the next
  /// render frame's upstream scan fires its runInitialPasses, which can arrive
