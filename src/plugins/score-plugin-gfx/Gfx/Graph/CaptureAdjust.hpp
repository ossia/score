 ...
 *
 * Defaults are the identity. A pipeline that sets nothing renders the raw
 * demosaiced frame -- including the linear transfer curve, which looks dark.
 * Changing that default would silently restyle every existing project, so the
 * correction is offered rather than imposed.
 *
...
struct CaptureAdjust
{
  ...
  ///
  /// Stretch, not Original, because Stretch is what a scale of (1,1) means.
  /// Original scales the quad by texture/viewport, so defaulting to it would
  /// zoom and crop every capture already in use.
