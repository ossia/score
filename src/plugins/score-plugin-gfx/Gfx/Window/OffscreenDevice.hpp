...
class offscreen_device : public ossia::net::device_base
{
  ...
  // parent in this class (it inherits NodeModel, not QObject), so nothing
  // else would delete it -- and every offscreen device cycle would leak it
  // along with the rhi resources its ~BackgroundNode → destroyOutput
  // releases.
