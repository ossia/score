...
// WindowCapture does not use format_negotiation: the portal source defines
// its own width/height/format and whatever it offers is accepted. Advertising
// GPU-supported modifiers to the portal compositor would go here.
...
public:
  ...
  void stop() override
  {
    ...
    // destroyed while the loop is still alive.
    m_pwContext.reset();
  ...
  static void onProcess(void* data)
  {
    ...
    else if(d.type == SPA_DATA_DmaBuf)
    {
      if(d.fd >= 0)
      {
        ...
        // unmapped formats fall back to XRGB8888.
        const uint32_t fcc
