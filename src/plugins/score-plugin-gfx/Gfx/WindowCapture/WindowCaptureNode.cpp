...
public:
  ...
  void initState(score::gfx::RenderList& renderer, QRhiResourceUpdateBatch& res) override
  {
    ...
    // -- the texture is recreated in QRhiTexture::RGBA8 the first time a
    // CPU_RGBA frame arrives; without that branch, RGBA bytes would upload as
    // BGRA and display with R and B swapped.
    ...
    else if(score::gfx::EglDmaBufImporter::isAvailable(rhi))
    {
      ...
      if(m_eglDmaBufImporter->init(rhi))
      {
        ...
        // each frame, so the id is stable and createFrom runs once.
      ...
      case CapturedFrame::DMA_BUF_FD:
      {
        ...
        else if(m_eglDmaBufImporter && m_eglGlTexture != 0)
        {
                 ...
                 frame.drmFormat, frame.width, frame.height))
          {
            ...
            // has not changed, only its storage (via the EGLImage
