...
class gphoto2_decoder : public ::Video::ExternalInput
{
  ...
  // queue->drain() below read freed memory (ASan heap-use-after-free in
  // test_integration_gfx_protocol_settings, GPhoto2Device.cpp:972 reaching
  // FrameQueue.cpp:212 on a block freed by ~gphoto2_protocol at :999).
  //
  // The queue really is shared -- the camera's preview thread produces into
  // it, the decoder consumes from it -- so the fix is to share-own the queue,
  // not to skip the drain.
