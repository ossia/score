...
//
// PipeWire video input device: connects to a PipeWire node providing video
// frames and surfaces them as AVFrames through
// Gfx::video_texture_input_protocol, the same path libav, V4L2 and camera inputs
// use.
//
// Consumer shape follows pipewire's src/examples/video-play.c: stream with state
// callbacks, format negotiated in on_stream_param_changed through
// spa_format_video_raw_parse, frames pulled in on_stream_process. score-side:
// the pipewire main loop runs on a dedicated per-stream std::thread, frames land
// in a mutex-guarded AVFrame pool the renderer pulls with dequeue_frame(), and a
// renegotiation rebuilds the pool for the new geometry and pixel format.
//
// Gated on OSSIA_ENABLE_PIPEWIRE; the factory is registered only when the
...
// sends DmaBuf with a fourcc DRMPrimeDecoder doesn't recognise), the
// memcpy path takes over.
...
private:
  struct PipeWireData
  {
    ...
    // registry; this only attaches a pw_stream to its loop.
...
bool InputStream::start() noexcept
{
  ...
  try
  {
    ...
    // per process.
    m_data->shared = acquireSharedContext("PipeWire input start");
    if(!m_data->shared)
...
void InputStream::stop() noexcept
{
  ...
  m_pw_subs.clear();
  // The shared context's thread_loop keeps running; this only unhooks and
  // destroys our stream. cleanup() takes the lock internally.
  m_data->cleanup();
...
bool PipeWireDevice::reconnect()
{
  ...
  // refreshes its connection indicator.
