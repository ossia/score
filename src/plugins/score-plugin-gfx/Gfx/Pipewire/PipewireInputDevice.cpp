    ...
    void* data, uint32_t id, const struct spa_pod* param)
{
  ...
  if(pc.kind == ng::result::needs_fixation)
  {
    if(pc.candidate_modifiers.empty())
      return;
    // Phase 5 baseline policy: prefer LINEAR when offered, otherwise
    // take the producer's first choice. Phase 6 follow-up: probe each
    // candidate against the GPU import path and pick the cheapest
    // supported tiled modifier instead of always LINEAR.
