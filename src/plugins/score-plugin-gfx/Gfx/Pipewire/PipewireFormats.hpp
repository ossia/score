...
inline uint64_t frameBytes(Tag t, int w, int h) noexcept
{
  ...
  // A subsampled plane rounds up: 641 luma columns carry 321 chroma columns.
  const uint64_t cw = uint64_t(w + 1) / 2;
  const uint64_t ch = uint64_t(h + 1) / 2;
  switch(t)
  {
    case Tag::NV12:
    case Tag::YV12:
    case Tag::YUV420P: return wh + 2 * cw * ch;              // 8-bit 4:2:0
    case Tag::P010:    return 2 * (wh + 2 * cw * ch);        // 16-bit, 4:2:0
    case Tag::P210:    return 2 * (wh + 2 * cw * uint64_t(h)); // 16-bit, 4:2:2
    case Tag::YUYV422:
    case Tag::UYVY422: return 4 * cw * uint64_t(h);          // packed 4:2:2
 ...
 *  RGB10A2 (SPA xRGB_210LE, R in bits 29-20) is DRM_FORMAT_ARGB2101010 'AR30'
 *  per the DRM spec; 'AB30' is the mirrored layout and would be wrong. */
