...
public:
  /** Buffers actually handed to pipewire, over every publish path. A
   *  round-trip whose consumer saw nothing needs to know whether the
   *  producer had anything to give it: no frames out of a producer that
   *  never published is an environment, no frames out of one that did is
   *  a defect. */
