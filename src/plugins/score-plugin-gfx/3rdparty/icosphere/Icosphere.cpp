...
void Icosphere::subdivideVerticesFlat()
{
  ...
  for (i = 1; i <= subdivision; ++i)
  {
    ...
    for (j = 0; j < indexCount; j += 3)
    {
      ...
      addSubLineIndices(index, index + 1, index + 4, index + 5, index + 11,
                        index + 9); // CCW
...
unsigned int Icosphere::addSubVertexAttribs(const float v[3], const float n[3], const float t[2])
{
  ...
  if (Icosphere::isSharedTexCoord(t))
  {
    ...
    // if not in the list, add the vertex attribs to arrays and return its
    // index if exists, return the current index
...
///////////////////////////////////////////////////////////////////////////////
void Icosphere::computeFaceNormal(
    const float v1[3],
    const float v2[3],
    const float v3[3],
    float n[3])
...
///////////////////////////////////////////////////////////////////////////////
void Icosphere::computeHalfVertex(
    const float v1[3],
    const float v2[3],
    float length,
    float newV[3])
...
// texture coordinate is shared or no. If it is on the line segments, it is
// also non-shared point
//   00  01  02  03  04         //
//   /\  /\  /\  /\  /\         //
//  /  \/  \/  \/  \/  \        //
// 05  06  07  08  09   \       //
