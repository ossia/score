    ...
    geometry_input::auxiliary_texture_request& out)
{
  ...
  // anything other than the sampled-texture default (rgba8). ONLY uses
  // FORMAT — NOT ACCESS — because `access` defaults to "read_write" in
  // the struct, so FORMAT is the only usable discriminator.
  if(!out.is_storage)
  {
...
// TYPES are parsed (TYPES may appear in any order in the JSON).
    ...
    if(v.get_type() == sajson::TYPE_ARRAY)
    {
      ...
      for(std::size_t i = 0; i < n; i++)
      {
        ...
        if(obj.get_type() == sajson::TYPE_OBJECT)
        {
          ...
          // consecutive locations under GLSL 4.50 §4.4.1).
          //
          // For mixed explicit / auto layouts the cumulative-sum above
          // can collide with a user-pinned LOCATION.
...
// included the struct. The compute path has its own copy of this logic at
    ...
    bool alias_prev = false)
{
  ...
  // bound texture's QRhi flags must agree with the GLSL declaration, or
  // Vulkan rejects the pipeline with
...
#define CAMERA_POSITION camera.data[VIEW_INDEX].cameraPosition_.xyz
  ...
  m_vertex += "#undef main\n";
  m_vertex += "void main()\n{\n";
  if(m_desc.multiview_count < 2 || m_desc.default_state.topology == "points")
  {
...
void parser::parse_csf()
{
  ...
  for(const auto& inp : m_desc.inputs)
  {
    ...
    else if(auto* geo_ptr = ossia::get_if<geometry_input>(&inp.data))
    {
      ...
      // macro because the GLSL type differs (samplerXY vs imageXY).
