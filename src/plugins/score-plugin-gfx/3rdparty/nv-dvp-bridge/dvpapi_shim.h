...
/* Resolved from dvpMemcpy2D, which takes ELEVEN parameters -- the mangled
 * names the shim looks up decode to (m,m,j,m,m,m,j,j,j,j,j) on Linux and the
 * same count on Windows. This typedef previously declared ten, so any call
 * through it would have passed one argument short and let the callee read
 * garbage. Nothing in the tree calls it (only dvpMemcpyLined is used), so the
 * mismatch never fired, but an exported typedef is a loaded gun. */
typedef DVPStatus (*PFN_dvpMemcpy)(
    DVPBufferHandle src, DVPSyncObjectHandle srcSync, uint32_t srcAcq,
    uint64_t timeout, DVPBufferHandle dst, DVPSyncObjectHandle dstSync,
    uint32_t dstRel, uint32_t startingY, uint32_t startingX, uint32_t height,
    uint32_t width);
