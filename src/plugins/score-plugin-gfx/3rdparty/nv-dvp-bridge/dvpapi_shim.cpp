...
void doLoad()
{
  ...
  if(!g_module)
  {
    ...
    // ERROR_MOD_NOT_FOUND is reported both when dvp.dll is absent and when it
    // is present but one of ITS imports cannot be resolved. NVIDIA ship DVP
    // v1.70 linked against the Visual C++ 2010 runtime, so on a machine
    // without that redistributable the library is right there and still fails
    // to load -- a distinction the bare error number hides.
    if(err == ERROR_MOD_NOT_FOUND)
      std::snprintf(
          g_error, sizeof(g_error),
          "LoadLibraryA(\"dvp.dll\") failed (Win32 error 126, "
          "ERROR_MOD_NOT_FOUND): dvp.dll is missing, or it was found but a "
          "dependency was not. DVP v1.70 imports MSVCR100.dll -- install the "
          "Microsoft Visual C++ 2010 x64 Redistributable if dvp.dll exists.");
    else
      std::snprintf(g_error, sizeof(g_error),
                    "LoadLibraryA(\"dvp.dll\") failed (Win32 error %lu)", err);
    return;
  }
#else
