 ...
 *        Video" (DVP) SDK so score's AJA GPU-direct output path can DMA
 *        encoder output from GPU memory to AJA-DMA-locked sysmem on Windows.
 *
...
/* True when the current GL context is a GLX one.
 *
 * Every libdvp GL entry point binds to the *current* context and walks a
 * dispatch table that exists only for GLX; given EGL (score's default, since
 * EGL is what makes dma-buf import zero-copy) or no context at all, those
 * pointers are null and calling them segfaults inside libdvp. Qt knows which
 * backend it created, so ask it -- same approach EglDmaBufImport uses in the
 * other direction with QEGLContext. A Qt without GLX support cannot have a
 * GLX context current, hence the false. */
bool hasCurrentGlxContext()
{
#if QT_CONFIG(xcb_glx_plugin)
  auto* ctx = QOpenGLContext::currentContext();
  return ctx && ctx->nativeInterface<QNativeInterface::QGLXContext>();
#else
  return false;
#endif
}
...
NV_DVP_API NvDvpError nv_dvp_init_gl(NvDvpContextHandle* out_ctx)
{
  ...
  *out_ctx = nullptr;
  ...
  /* libdvp reports success even when it has silently degraded to CUDA-only
   * (it returns DVP_STATUS_OK from dvpInitGLContext and
   * dvpGetRequiredConstantsGLCtx alike), so there is nothing to test after
   * the fact -- the context type is the only reliable signal. */
  if(!hasCurrentGlxContext())
  {
    fprintf(
        stderr,
        "[nv-dvp] GPUDirect-for-Video disabled: no GLX context.\n"
    ...
    setInitError(
        "no GLX context: set QT_XCB_GL_INTEGRATION=xcb_glx",
...
NV_DVP_API void nv_dvp_shutdown(NvDvpContextHandle ctx)
{
  ...
  const bool glCurrent = hasCurrentGlxContext();
  ...
  if(ctx->initialized)
  {
    switch(ctx->backend)
    {
      case NvDvpContext_t::Backend::D3D11:
        ...
        break;
      case NvDvpContext_t::Backend::GL:
        /* dvpCloseGLContext unbinds from the *current* GL context and
         * segfaults inside libdvp when there is none -- which is what
         * teardown looks like once the QRhi has dropped the context, seen at
         * Graph::~Graph on the Quadro box. Leaking libdvp's context in that
         * window is better than taking the process down at exit. */
        if(glCurrent)
          dvpCloseGLContext();
        break;
      case NvDvpContext_t::Backend::CUDA:
  ...
  if(r->kind == ResourceState::Kind::Buffer)
  {
    switch(ctx->backend)
    {
      case NvDvpContext_t::Backend::D3D11:
        ...
        break;
      case NvDvpContext_t::Backend::GL:
        // Same gate as nv_dvp_shutdown: unbinding needs the GLX context the
        // handle was bound to. This runs FIRST in every teardown -- callers
        // unregister each resource before shutting the context down -- so
        // leaving it ungated made the gate in nv_dvp_shutdown unreachable.
        if(hasCurrentGlxContext())
          dvpUnbindFromGLCtx(r->dvp);
        break;
      case NvDvpContext_t::Backend::CUDA:
