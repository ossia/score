...
TEST_CASE("a slot is only returned once the GPU can no longer be reading it")
{
  ...
  // corrupts the frame being scanned out, and the symptom reads as a sensor
  // glitch rather than as a lifetime bug.
  CaptureSyncGroup g{2};
  g.setRetireDepth(2);
          ...
          "[syncgroup][slots]")
{
  ...
  // takeReturned. Ageing the retire queue used to require a *fresh* pin, and a
  // fresh pin requires a free slot -- so a producer no deeper than the retire
  // depth published its whole ring once and then stalled forever. Backpressure
  // is fine; never recovering is not.
