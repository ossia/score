...
//
// Each of them shipped wrong once, and none of them fails loudly: a derived
// chroma offset on an NvBufSurface points at the wrong address, a derived pitch
// on a padded producer shears the frame progressively, a nominal-format pitch
// check rejects a perfectly good buffer, a zero plane count is refused before
// EGL is ever called, and a driver-only Vulkan gate refuses the two allocators
// that measured byte-exact.
//
// They live inside init(), which wants a live QRhi, so each was lifted into a
// free function in the same header and init() now calls it -- a mechanical
// extraction with no behaviour change, and the only way to reach this logic
// without a GPU, since the state it reads is private.
...
// importExternal's first guard is planeCount == 0, so passing the "derive it"
// sentinel straight through refused the import before EGL was ever called --
// for exactly the V4L2-style producer this path exists to serve, since those
// never fill planes[].
