...
TEST_CASE("a borrowed slot is not returned before the GPU is done with it")
{
  ...
  // Each acquisition displaces the slot bound before it. Slot 0 is displaced
  // by the acquisition of slot 1 and becomes returnable only `retireDepth`
  // acquisitions later -- returning it at any of the two in between is the bug
  // that corrupts the frame on screen.
  CHECK(t.ingest(1) == -1);
  CHECK(t.acquire() == 1);
  CHECK(t.takeReturned() == 0u);
