...
ChangeSpline::ChangeSpline(const ProcessModel& autom, const ossia::nodes::spline3d_data& newval)
...
void ChangeSpline::update(const ProcessModel&, const ossia::nodes::spline3d_data& newval)
{
  m_new = newval;
}

void ChangeSpline::update(const ProcessModel&, ossia::nodes::spline3d_data&& newval)
{
  using namespace std;
  swap(m_new, newval);
}
