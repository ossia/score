...
//
// Blending between scenes does not happen at the scene-graph level:
// scene-level blending has no meaningful semantics for arbitrarily
// different scene trees. Render each scene to its own texture
// (ScenePreprocessor → classic_pbr → BackgroundNode with a texture
// output) and ISF-crossfade the textures instead.
class SceneSwitch
{
public:
