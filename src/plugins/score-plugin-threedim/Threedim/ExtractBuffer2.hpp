...
namespace Threedim
{
// Name-based version of Threedim::ExtractBuffer, which enumerates a fixed list
// of attribute slots and selects one through a combobox. This one takes a Mode
// enum -- Attribute or Buffer -- and a free-form name interpreted per mode.
//
// Mode == Attribute extracts a single per-vertex attribute:
//   * "position" / "normal" / "tangent" / "bitangent" / "texcoord" or
//     "texcoord0".."texcoord7" / "uv" / "color" or "color0".."color3" match
//     against halp::attribute_semantic
//   * "<integer>" is the Nth entry in mesh.attributes[]
//   * anything else is a custom-name lookup in mesh.attributes[].name
//   The output uses the same Direct / Compute / Indexed strategies.
//
// Mode == Buffer extracts a whole raw buffer:
//   * "<integer>" is the Nth entry in mesh.buffers[]
//   * "index" is the buffer mesh.index points at
//   * a name matching mesh.auxiliary[].name, checked first since aux names may
//     shadow attribute names, returns that auxiliary's backing buffer with its
//     byte_offset and byte_size. This is how ScenePreprocessor's per-frame
//     auxiliaries reach a standalone gpu_buffer outlet for consumers that do
//     not want try_bind_from_geometry
//   * anything else looks up an attribute by semantic or custom name and
//     returns the buffer it lives in
//
// The source buffer handle is re-fetched from the mesh on every update(), so an
// upstream that rebuilds its QRhiBuffer is reflected on the next frame.
