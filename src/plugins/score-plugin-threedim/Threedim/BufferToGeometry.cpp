...
#undef CASE
  ...
  // Compute the TRS matrix from the position/rotation/scale controls. Returns
  // true only when the matrix actually changed, so an untouched knob does not
  // fire a downstream transform rebuild every frame.
