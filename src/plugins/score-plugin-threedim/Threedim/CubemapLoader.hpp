...
public:
  ...
  struct ins
  {
    ...
    // onto the node from the execution thread.
    struct image_t : halp::file_port<"Image", halp::mmap_file_view>
    {
      // Only LDR formats are advertised: decode goes through QImage, which has
      // no stock Radiance-HDR / OpenEXR handler, and both the equirect source
      // and the cube textures are RGBA8. Listing .hdr/.exr would decode to null
      // or truncate to 8 bits, since no float decoder is vendored here.
  ...
  struct
  {
    ...
    // directly (e.g. a bare-skybox rendering shader). Tagged via
    // halp::gpu_cubemap_output so sinks know to grab-from-source
...
private:
  ...
  // already been destroyed), they go straight to deleteLater.
