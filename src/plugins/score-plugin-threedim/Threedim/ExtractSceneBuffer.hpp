...
// Scene-level buffer extractor: takes a scene_spec, picks one of the GPU arena
// slots stamped on the scene's components, and republishes the backing
// {QRhiBuffer*, byte_offset, byte_size} triple on a halp::gpu_buffer outlet.
//
// Unlike Threedim::ExtractBuffer2, which extracts from a flattened geometry's
// aux list downstream of a ScenePreprocessor, this works directly on a raw
// scene_spec -- for a compute shader that wants a producer's Raw arena slot
// without a preprocessor flatten, or a pipeline that has no preprocessor at all.
//
// Source resolution uses each component's `raw_slot`:
...
//
// The registry's isLive() check guards every read; a stale ref clears the outlet
// rather than handing a dangling QRhiBuffer* downstream.
//
// Lights are not exposed because the light tree is not a flat scene_state.lights
// vector -- lights live as scene_payload children. Extract them downstream of a
// ScenePreprocessor with ExtractBuffer2("scene_lights").
