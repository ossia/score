...
//
// Core use case is IBL wiring: an irradiance-convolution, GGX-prefilter
// or BRDF-LUT shader's output plugs in here and lands on
// `scene_environment.{irradiance_map, prefiltered_map, brdf_lut}` with
// no per-target glue code. Shadow-map generation passes will target
// `scene_state.shadow_cascades.shadow_map_array` the same way.
//
