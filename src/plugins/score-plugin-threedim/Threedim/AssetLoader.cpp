...
AssetLoader::ins::asset_t::process(file_type tv)
{
  ...
  if(!loaded)
  {
    // Say so: the node cannot observe its own failure afterwards. An empty
    // function makes the avnd runtime apply nothing, so operator()() only ever
    // sees "I have no scene", which is also what a brand-new instance sees.
    // A rejected asset is a normal outcome -- assets are addressed through
    // several path syntaxes, including C:/Users/... paths opened on Linux --
    // and without this it is a black frame with no message anywhere.
    if(dispatched)
