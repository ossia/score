...
static ossia::scene_transform to_transform(const fastgltf::Node& n)
{
  ...
  {
    // Matrix form -- full TRS decomposition. Options::DecomposeNodeMatrices
    // makes fastgltf convert to TRS upfront, but this branch still fires for
    ...
    const std::filesystem::path& dir)
{
  ...
  auto fill_tex = [&](ossia::texture_ref& tr, const fastgltf::TextureInfo& ti) {
    ...
    std::visit(
        [&](const auto& data) {
          ...
          // sources::Vector / sources::Fallback / sources::CustomBuffer are
          // not handled -- files use one of the three above.
        },
        img.data);

    // Content-hash for cross-output / cross-reload decode dedup. Prefer
    // hashing the embedded bytes — it's the decoded payload contents that
    // matter, not the file path (two different files can embed the same
    // JPEG). Fall back to hashing the path string when no embedded data
    // (URI → we'll read the file on demand inside the preprocessor,
    // hashing the path is a stable proxy for session-scope dedup).
  ...
  // at parse time (see loadFromFile() below). Each KHR_* that sets
  // material_component fields gets a matching block below.
  //
    ...
    const std::vector<int>& material_index_remap)
{
  ...
  if(auto* a = get_accessor("POSITION"))
  {
    ...
    // Local-space AABB. glTF requires min/max on the POSITION accessor, but
    // walking the decoded float stream costs one extra pass on load and keeps
    // this uniform with the FBX / procedural code paths.
