...
#undef CASE
  ...
  // true iff the matrix actually changed.
  ...
  for(int i = 0; i < 8; ++i)
  {
    ...
    // Add attribute with sequential location and resolved semantic, carrying
    // the authored text as well. halp::geometry_attribute documents `name` as
    // "For custom semantics; empty = use semantic name", and
    // resolveSemanticFromName maps anything it does not recognise onto
    // `custom`, so without the string every custom attribute would be an
    // anonymous `custom` with nothing for a shader asking for `temperature`
    // to match against.
    const bool isCustom
