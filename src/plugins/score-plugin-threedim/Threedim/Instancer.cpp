...
void Instancer::operator()()
{
  ...
  // go through update().
  //
  // The Points-input cache also compares the current vertex count and primary
  // buffer handle against m_cached_points_vertices / m_cached_points_buf: an
  // upstream CSF regenerating its cloud with a different count but the same
  // persistent QRhiBuffer leaves dirty_mesh clear, and the stale instance_count
  // would keep the downstream preprocessor's persistent m_pendingGpuCopies queue
  // firing the old count.
  const auto& in = inputs.scene_in.scene;
  const ossia::scene_state* in_state = in.state.get();
  void* points_primary
      = !inputs.points.mesh.buffers.empty()
