...
public:
  ...
  // a bytes-pointer compare: pointer identity does not detect in-place
  // buffer updates (ring-buffer video readback reuses the address).
  ...
  void update(
      ...
      score::gfx::Edge* e)
  {
    // Face size comes from the largest input; content changes come from the
    // producer's `changed` flag (set by halp::texture's update() — see
    // avendish texture_formats.hpp), cleared here on consumption. Size is
    // tracked separately so a producer that resizes a face still triggers a
    // texture recreation even when it forgot to toggle `changed`.
    int maxSize = 0;
    int faceIdx = 0;
