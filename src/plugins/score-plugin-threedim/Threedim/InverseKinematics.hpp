...
//
// Given a 3-joint chain (root -> mid -> end), a target world-space position and a
// pole vector to disambiguate the elbow plane, produces the joint rotations that
// make the end effector reach, or come as close as possible to, the target.
// Law-of-cosines closed form, about 50 floating-point ops, no iteration.
//
// The solver reads the input skeleton's TRS, finds the named end joint, walks two
// parents up to identify the chain, and emits a scene_spec with only those three
// joints' local rotations modified; the rest of the skeleton and the mesh and
// material data pass through unchanged.
//
// This is "reach for that door handle" IK. Full articulated rigs with more than
// two bones, spine chains or pole-axis constraints want a sequence of these per
// limb, or a FABRIK/CCD successor on N-joint chains; the interface is
// deliberately narrow so swapping one in later does not break patches.
//
// Limitations: no joint limits or rotation constraints, no twist decomposition,
// the chain must be a direct parent line rather than a branch, and an unreachable
// target extends the chain fully toward it, the natural straight-arm stretch.
