...
// GPU-instancing authoring node. Takes a scene containing a mesh and a GPU buffer
// of per-instance transforms, plus optional colors and custom data, and emits a
// scene_spec wrapping an instance_component that ScenePreprocessor forwards
// downstream as the standard instance_transforms / instance_colors /
// instance_custom auxiliary buffers.
//
// Consumer shaders read the per-instance attributes through the VERTEX_INPUTS
// location 3..5 convention already in GeometryToBufferStrategies.hpp:
//   3 = translation / rotation / transform_matrix, 4 = color0, 5 = scale/custom.
//
// Transform formats, packed floats per instance:
//   mat4        16 floats, column-major
//   trs         10 floats: 3 translation + 4 quaternion + 3 scale
//   translation 3 floats, rotation and scale identity
class Instancer
{
public:
  ...
  struct ins
  {
    ...
    // Optional point-cloud geometry input. When wired, its semantic attributes
    // override the raw buffer inputs above: translation / position and
    // transform_matrix feed the Transforms buffer in Translation and Mat4 mode,
    // color0 feeds the Colors buffer. The `count` inlet is then overridden by the
    // geometry's vertex_count, so shaderlib presets can feed Instancer directly
    // without a glue repack.
    struct
    {
  ...
  // below depends on the size. Comparing handles alone would let a shrink go
  // unnoticed and the cloud would draw past the end of its transforms.
  struct CachedView
  {
    void* handle{};
    int64_t byte_offset{};
