...
// Converts the two source-data shapes HumanoidRetarget accepts --
// keypoint_stream from an ONNX PoseDetector, tracker_bundle_6 from a mocap
// device -- into the canonical humanoid_pose. Both paths produce PARENT-LOCAL
// quaternions, which is the invariant HumanoidRetarget depends on.
...
// undefined by just two direction endpoints): a hard limit of single-camera
// keypoint mocap.
...
//
//   1. For each bone with a valid (parent_idx, child_idx) edge and both
//      keypoints above the confidence threshold:
//        d_world[b] = normalize(kp[child] - kp[parent])
//        q_world[b] = shortestArc(kRestAxis[b], d_world[b])
//   2. Walk bones in enum order, which is already topological since a bone's
//      parent has a lower index. No world rotation means validity = 0; a parent
//      without one makes q_world[b] the local rotation, i.e. root-relative;
//      otherwise q_local[b] = inv(q_world[parent(b)]) * q_world[b].
//   3. Copy the Hips world position from the landmark that best represents it,
//      the hip midpoint for BlazePose and COCO, approximated as left_hip. Used
//      only by the root-motion toggle downstream.
// ---------------------------------------------------------------------------
    ...
    float confidence_threshold = 0.5f) noexcept
{
  ...
  // root motion; the tracker path below carries a real pelvis position.
...
//
// With 6 trackers (head, hips, 2 hands, 2 feet) only those 6 bones are driven;
// spine, shoulders, elbows and knees stay at their retarget rest. Making them
// follow realistically needs either more trackers (10-point Vive Full-Body) or a
// downstream 2-bone IK chain keyed on shoulder and wrist positions.
//
// Tracker quaternions are world-space by convention -- PSN, OSC and VRPN all
// report world transforms -- so parent-local is produced by inverting the parent
// bone's tracker rotation when that parent also has a tracker, and inherited
// directly otherwise.
// ---------------------------------------------------------------------------
