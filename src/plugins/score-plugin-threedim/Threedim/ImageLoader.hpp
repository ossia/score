...
public:
  ...
  struct ins
  {
    ...
    struct image_t : halp::file_port<"Image", halp::mmap_file_view>
    {
      ...
      {
        // Decode through the shared gfx loader: RGBA8888 canonicalization and
        // the footer-less TGA fallback in one place.
        QImage img;
        if(!data.bytes.empty())
        {
          if(auto dec = score::gfx::decodeImageFromMemory(
                 QByteArray::fromRawData(data.bytes.data(), data.bytes.size()), {}))
            img = std::move(dec->image);
        }
        if(img.isNull() && !data.filename.empty())
        {
          if(auto dec = score::gfx::decodeImageFromPath(
                 QString::fromUtf8(data.filename.data())))
            img = std::move(dec->image);
        }
  ...
  // user to re-trigger the file-port. Without it, release() drops m_tex and
  // update() clears m_pendingImage, so the next init() has nothing to upload
  // and the texture port stays bound to the empty placeholder for the rest of
  // the session.
  QImage m_keptImage;
  QRhiTexture* m_tex{};
  bool m_changed{};
