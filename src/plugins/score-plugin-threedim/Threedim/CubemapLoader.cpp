...
bool CubemapLoader::createCubemapTexture(QRhi& rhi, int faceSize)
{
  ...
  // TextureSizeMax is a *dimension* limit — 32768 on desktop NVIDIA — and a
  // mipped RGBA8 cube at that size is ~32 GB, far past maxMemoryAllocationSize.
  // Bound the allocation by the resolution the UI actually offers as well.
  constexpr int max_ui_face_size = 8192;
  faceSize = std::min(faceSize, max_ui_face_size);
...
bool CubemapLoader::setupEquirectPipeline(score::gfx::RenderList& renderer)
{
  ...
  // faceIndex, but all 6 are recorded into one command buffer and executed at
  // endFrame, so a single Dynamic UBO region at offset 0 would hold the LAST
  // face's value by the time any draw runs on Vulkan/Metal/D3D. Give each face
  // its own slot, spaced by the device's uniform-buffer offset alignment, and
  // bind per-pass with a QRhiCommandBuffer::DynamicOffset.
