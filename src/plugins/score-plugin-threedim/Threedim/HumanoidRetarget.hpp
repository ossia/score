...
// Threedim::HumanoidRetarget: live scene filter that drives a rigged model's
// skeleton from a humanoid_pose stream.
//
// Inputs are scene_in, an ossia::scene_spec carrying at least one
// skeleton_component, and pose_in, an optional humanoid_pose from a source
// adapter. Controls select the target rig's bone-name convention (Mixamo, VRM,
// Unreal Mannequin), capture the rest pose -- required before any motion
// transfers -- and optionally drive Hips translation from the source's
// hip_position delta.
//
// scene_out is the incoming scene_spec with only the mapped joints' rotations,
// and optionally Hips translation, replaced; everything else passes through.
//
// Offset / delta-from-rest math, the correct choice when source and target rigs
// have different axis conventions:
//
//     q_tgt_cur = q_tgt_rest * ( inverse(q_src_rest) * q_src_cur )
//
// Calibration captures q_src_rest per canonical bone and q_tgt_rest per resolved
// target joint, so the delta is a parent-relative quaternion that transfers even
// between a BlazePose landmark graph and a Mixamo FBX, as long as the adapter
// produces parent-relative rotations. Per-bone axis correction matrices, needed
// for some exotic rigs, are a follow-up.
//
// No smoothing -- that belongs in the adapter, pre-pose_spec. No IK -- chain
// InverseKinematics after this process; the two compose on scene_spec.
