...
// 4-way camera switch and weighted blender.
//
// Operates at the scene_spec level like SceneSwitch, specialised to selecting or
// blending up to 4 Camera producers. Each input is expected to be the output of
// a Threedim::Camera, or any scene_spec whose first root carries a
// scene_transform + camera_component pair.
//
// Select mode: `index` picks one input and the other three are ignored.
// Equivalent to a SceneSwitch in front of a camera, without the caveat that
// non-camera scene data from the unselected inputs would be dropped too.
//
// Blend mode: `weights` (x,y,z,w) linearly blends positions, FOV and near/far,
// and nlerps the orientation quaternions. Weights are normalised to sum 1
// internally, so raw envelope or LFO outputs can be passed directly. nlerp is
// fine for small angular deltas; great-circle blending across wide angles would
// take two slerps.
//
// An unwired input contributes zero weight, and an output with no wired input
// carrying effective weight is empty.
class CameraSwitch
{
public:
  ...
  // changes from within operator()()'s republish path.
  ...
  void rebuild()
  {
    ...
    if(wsum <= 1e-6f)
    {
      ...
      // Bump dirty so consumers detect the empty-state transition.
      m_pending_dirty = 0xFF;
      return;
    }
    ...
    // Accumulators must start at zero: value-initialisation APPLIES the
    // default member initialisers (scale 1,1,1), so a weighted sum would land
    // on mean + default.
    ossia::scene_transform outX{};
    outX.scale[0] = outX.scale[1] = outX.scale[2] = 0.f;
    ...
    // Same here: camera_component defaults yfov to 45 degrees, zfar to 1000,
    // aspect/xmag/ymag to 1 and focal_length to 50.
    ossia::camera_component outCam{};
    outCam.yfov = 0.f;
    outCam.aspect_ratio = 0.f;
    outCam.xmag = 0.f;
    outCam.ymag = 0.f;
    outCam.znear = 0.f;
    outCam.zfar = 0.f;
    ...
    // contributes NOTHING and is very likely still at its defaults, which
    // would make an orthographic camera 1 come out perspective. Take the mode
    // from the camera that actually dominates the blend.
    {
      int dominant = 0;
      float best = -1.f;
      for(int i = 0; i < 4; ++i)
