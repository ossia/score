...
// =============================================================================
// Legacy extractor (used to drive the removed halp dynamic_geometry output).
// Keeping it around as dead code: referenced only by its declaration and
// never instantiated now that FbxParser publishes only scene_spec. Feel
// free to delete once you're sure no consumer needs it.
// =============================================================================
#if 0
struct FbxMeshExtractor
{
  float_vec& buf;
  std::vector<mesh>& meshes;
  std::vector<uint32_t> tri_indices;

  void extract_mesh_part(
      const ufbx_mesh* umesh, const ufbx_mesh_part& part,
      const ufbx_matrix& world_matrix)
  {
    ...
    mesh m{};
    m.vertices = num_verts;
    m.normals = has_normals;
    m.texcoord = has_uv;
    m.colors = has_colors;
    m.tangents = has_tangents;

    int64_t total_floats = num_verts * 3;
    if(has_uv) total_floats += num_verts * 2;
    if(has_normals) total_floats += num_verts * 3;
    if(has_colors) total_floats += num_verts * 3;
    if(has_tangents) total_floats += num_verts * 4;

    const int64_t base = int64_t(buf.size());
    buf.resize(buf.size() + total_floats);

    int64_t offset = base;
    m.pos_offset = offset;
    {
      float* dst = buf.data() + offset;
      for(size_t fi = 0; fi < part.num_faces; fi++)
      {
        ...
        uint32_t num_tris
            = ufbx_triangulate_face(tri_indices.data(), tri_indices.size(), umesh, face);
        for(uint32_t ti = 0; ti < num_tris; ti++)
        {
          for(int vi = 0; vi < 3; vi++)
          {
            ...
            ufbx_vec3 pos
                = umesh->vertex_position.values.data[umesh->vertex_position.indices.data[idx]];
            transform_point(world_matrix, pos, dst[0], dst[1], dst[2]);
            dst += 3;
          }
        }
      }
      ...
      for(size_t fi = 0; fi < part.num_faces; fi++)
      {
        ...
        for(uint32_t ti = 0; ti < num_tris; ti++)
        {
          for(int vi = 0; vi < 3; vi++)
          {
            uint32_t idx = tri_indices[ti * 3 + vi];
            ufbx_vec2 uv
                = umesh->vertex_uv.values.data[umesh->vertex_uv.indices.data[idx]];
      ...
      for(size_t fi = 0; fi < part.num_faces; fi++)
      {
        ...
        for(uint32_t ti = 0; ti < num_tris; ti++)
        {
          for(int vi = 0; vi < 3; vi++)
          {
            uint32_t idx = tri_indices[ti * 3 + vi];
            ufbx_vec3 nrm
                = umesh->vertex_normal.values.data[umesh->vertex_normal.indices.data[idx]];
            transform_normal(world_matrix, nrm, dst[0], dst[1], dst[2]);
            dst += 3;
          }
        }
      }
      ...
      for(size_t fi = 0; fi < part.num_faces; fi++)
      {
        ...
        for(uint32_t ti = 0; ti < num_tris; ti++)
        {
          for(int vi = 0; vi < 3; vi++)
          {
            uint32_t idx = tri_indices[ti * 3 + vi];
            ufbx_vec4 col
                = umesh->vertex_color.values.data[umesh->vertex_color.indices.data[idx]];
            dst[0] = float(col.x); dst[1] = float(col.y); dst[2] = float(col.z);
            dst += 3;
          }
        }
      }
      ...
      for(size_t fi = 0; fi < part.num_faces; fi++)
      {
        ...
        for(uint32_t ti = 0; ti < num_tris; ti++)
        {
          for(int vi = 0; vi < 3; vi++)
          {
            uint32_t idx = tri_indices[ti * 3 + vi];
            ufbx_vec3 tan
                = umesh->vertex_tangent.values.data[umesh->vertex_tangent.indices.data[idx]];
            float tx, ty, tz;
            transform_normal(world_matrix, tan, tx, ty, tz);
            ...
            if(umesh->vertex_bitangent.exists)
            {
              ufbx_vec3 nrm
                  = umesh->vertex_normal.values.data[umesh->vertex_normal.indices.data[idx]];
              ufbx_vec3 bitan
                  = umesh->vertex_bitangent.values.data[umesh->vertex_bitangent.indices.data[idx]];
              float nx, ny, nz; transform_normal(world_matrix, nrm, nx, ny, nz);
              float bx, by, bz; transform_normal(world_matrix, bitan, bx, by, bz);
              float cx = ny * tz - nz * ty;
              float cy = nz * tx - nx * tz;
              float cz = nx * ty - ny * tx;
              float d = cx * bx + cy * by + cz * bz;
              dst[3] = d < 0.0f ? -1.0f : 1.0f;
            }
            else
            {
              dst[3] = 1.0f;
    ...
    for(size_t ni = 0; ni < scene->nodes.count; ni++)
    {
      ...
      if(umesh->material_parts.count > 0)
      {
        for(size_t pi = 0; pi < umesh->material_parts.count; pi++)
          extract_mesh_part(umesh, umesh->material_parts.data[pi], world);
      }
      else
      {
        ...
        extract_mesh_part(umesh, whole, world);
      }
    }
  }
};
