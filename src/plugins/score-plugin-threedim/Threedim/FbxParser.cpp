...
struct FbxSceneExtractor
{
  ...
  // Convert a ufbx_material to a material_component. Returns the index in
  // `materials`, registering it on first sight.
  int register_material(const ufbx_material* m)
  {
    ...
    auto fill_texture
        = [](ossia::texture_ref& tr, const ufbx_material_map& map) {
            ...
            // Stamp the content hash so the preprocessor's
    ...
    // which we don't carry yet on material_component.
