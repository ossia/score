...
// Same shape as every other JIT process (Bytebeat, Texgen, JitModel, AvndJit):
// no external-UI view. DefaultEffectItem cannot serve as ExtView_T -- it is a
// QGraphicsItem, while makeExternalUI() builds ExtView_T{proc, ctx, parent} and
// returns a QWidget*. (It also gained a leading `onlyShowUndisplayedPorts` ctor
// argument since this branch was written.)
using Expr2LayerFactory = Process::ScriptLayerFactory_T<
    Expr2Model,
