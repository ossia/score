  ...
  static void update(Execution::SoundComponent& component)
  {
    ...
    struct
    {
      ...
      void operator()(const Media::AudioFile::MmapReader& r) const noexcept
      {
        ...
        if (auto n
            = std::dynamic_pointer_cast<ossia::nodes::sound_mmap>(old_node))
        {
    ...
    , m_recomputer{*this}
{
  ...
  connect(
      &element,
      &Media::Sound::ProcessModel::fileChanged,
      this,
      &SoundComponent::on_fileChanged);

  auto node_action = [this](auto&& f) {
    if (auto n_ref
        = std::dynamic_pointer_cast<ossia::nodes::sound_ref>(this->node))
      in_exec([n_ref, ff = std::move(f)]() mutable { ff(*n_ref); });
    else if (
        auto n_mmap
        = std::dynamic_pointer_cast<ossia::nodes::sound_mmap>(this->node))
      in_exec([n_mmap, ff = std::move(f)]() mutable { ff(*n_mmap); });
  };

  con(element,
      &Media::Sound::ProcessModel::startChannelChanged,
      this,
      [=, &element] {
        node_action([start = element.startChannel()](auto& node) {
          node.set_start(start);
        });
      });
  con(element,
      &Media::Sound::ProcessModel::upmixChannelsChanged,
      this,
      [=, &element] {
        node_action([start = element.upmixChannels()](auto& node) {
          node.set_upmix(start);
        });
      });
  con(element,
      &Media::Sound::ProcessModel::nativeTempoChanged,
      this,
      [=, &element] {
        node_action([start = element.nativeTempo()](auto& node) {
          node.set_native_tempo(start);
        });
      });
  con(element,
      &Media::Sound::ProcessModel::stretchModeChanged,
      this,
      [=, &element] {
