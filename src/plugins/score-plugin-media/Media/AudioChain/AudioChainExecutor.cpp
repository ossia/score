#include "AudioChainExecutor.hpp"
...
namespace Media
{
AudioChainComponentBase::AudioChainComponentBase(
    Media::AudioChain::ProcessModel& element,
    const Execution::Context& ctx,
    const Id<score::Component>& id,
    QObject* parent)
    : Execution::ProcessComponent_T<Media::AudioChain::ProcessModel, ossia::node_chain_process>{
        element,
        ctx,
        id,
        ...
        parent}
{
  ...
  element.effects().orderChanged.connect<&AudioChainComponentBase::on_orderChanged>(*this);
}

AudioChainComponentBase::~AudioChainComponentBase()
{
  process().effects().orderChanged.disconnect<&AudioChainComponentBase::on_orderChanged>(*this);
}

void AudioChainComponentBase::unregister_old_first_node(
    std::pair<Id<Process::ProcessModel>, AudioChainComponentBase::RegisteredEffect>& old_first,
...
void AudioChainComponentBase::register_new_first_node(
...
void AudioChainComponentBase::unregister_old_last_node(
    std::pair<Id<Process::ProcessModel>, AudioChainComponentBase::RegisteredEffect>& old_last,
    Execution::Transaction& commands)
{
  if (old_last.second)
  {
...
void AudioChainComponentBase::register_new_last_node(
    std::pair<Id<Process::ProcessModel>, AudioChainComponentBase::RegisteredEffect>& new_last,
    Execution::Transaction& commands)
{
  if (new_last.second)
  {
...
Execution::ProcessComponent* AudioChainComponentBase::make(
    const Id<score::Component>& id,
...
void AudioChainComponentBase::added(ProcessComponent& e) { }

std::function<void()>
AudioChainComponentBase::removing(const Process::ProcessModel& e, ProcessComponent& c)
{
  if (process().badChaining())
    return {};
...
void AudioChainComponentBase::on_orderChanged()
{
  if (process().badChaining())
  {
...
void AudioChainComponentBase::unreg(const AudioChainComponentBase::RegisteredEffect& fx)
{
  system().setup.unregister_node_soft(fx.registeredInlets, fx.registeredOutlets, fx.node());
}

void AudioChainComponentBase::reg(const RegisteredEffect& fx, Execution::Transaction& vec)
{
  system().setup.register_node(fx.registeredInlets, fx.registeredOutlets, fx.node(), vec);
}

AudioChainComponent::AudioChainComponent(
    AudioChain::ProcessModel& element,
    const Execution::Context& ctx,
    const Id<score::Component>& id,
    QObject* parent)
    : score::PolymorphicComponentHierarchy<AudioChainComponentBase, false>{
        score::lazy_init_t{},
        element,
        ctx,
        id,
        parent}
{
  ...
  connect(&element, &Media::AudioChain::ProcessModel::badChainingChanged, this, [&](bool b) {
    if (b)
    {
      clear();
    }
...
void AudioChainComponent::cleanup()
{
#if !defined(NDEBUG)
  m_clearing = true;
#endif
