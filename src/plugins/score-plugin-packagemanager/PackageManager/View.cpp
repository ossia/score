...
namespace PM
{
  ...
  PluginSettingsView::PluginSettingsView()
  {
    ...
    m_addonsOnSystem->setSortingEnabled(true);

    m_remoteAddons->setSortingEnabled(true);
  ...
  void PluginSettingsView::handleAddon(const QJsonObject& obj)
  {
    ...
    if(m_addonsToRetrieve == std::ssize(m_remoteModel->m_vec))
      reset_progress();
    else
    ...
    m_remoteModel->addAddon(std::move(add));
  }
  ...
  void PluginSettingsView::uninstall()
  {
    ...
    {
      success
          = QDir{library.getPackagesPath() + '/' + addon.raw_name}.removeRecursively();
    }
    else if(addon.kind == "sdk")
    {
    ...
    if(success)
    {
      m_localModel->removeAddon(addon);
      set_info();
    }
  }
  ...
  void PluginSettingsView::update()
  {
    ...
    auto it = ossia::find_if(
        m_remoteModel->addons(), [&](auto& pkg) { return pkg.key == addon.key; });
    if(it == m_remoteModel->addons().end())
    {
      ...
      const Package& addon, const QDir& destination, const std::vector<QString>& res)
  {
    ...
    m_localModel->registerAddon(dir.absolutePath());
    set_info();
  }
