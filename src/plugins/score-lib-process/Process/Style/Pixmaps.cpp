...
void Pixmaps::reloadIcons()
{
  show_ui_off = score::get_pixmap(":/icons/undock_on.png");
  show_ui_on = score::get_pixmap(":/icons/undock_off.png");

  show_script_off = score::get_pixmap(":/icons/process_script_on_16.png");
  show_script_on = score::get_pixmap(":/icons/process_script_off_16.png");
...
const Pixmaps& Pixmaps::instance() noexcept
{
  ...
  // Skin::load() and setGlobalScaleFactor() both bump LoadIndex, which are
  // the only two things that invalidate any of this.
  if(const int idx = score::Skin::instance().LoadIndex; p.m_loadIndex != idx)
  {
    p.m_loadIndex = idx;
    if(const double dpr = qApp->devicePixelRatio(); p.m_dpr != dpr)
    {
      p.m_dpr = dpr;
      p.reloadIcons();
    }
