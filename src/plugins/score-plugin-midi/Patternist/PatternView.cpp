  ...
  , m_model{model}
{
  con(model, &Patternist::ProcessModel::patternsChanged,
      this, [=] { updateLanes(); });
  con(model, &Patternist::ProcessModel::currentPatternChanged,
      this, [=] { updateLanes(); });
}
...
void View::updateLanes()
{
  ...
  if(cur_p.lanes.size() != m_lanes.size())
  {
    ...
    for(int lane = 0; lane < cur_p.lanes.size(); lane++)
    {
      ...
      connect(sl, &score::QGraphicsIntSlider::sliderMoved,
              this, [this, sl, lane]  {
        noteChanged(lane, sl->value());
      });
      connect(sl, &score::QGraphicsIntSlider::sliderReleased,
              this, [this] {
        noteChangeFinished();
      });
