    ...
    , m_view{view}
{
  connect(
      ...
      this,
      [this, &layer](State::MessageList lst) {
        if (lst.empty())
          return;
    ...
    const Process::Context& doc)
{
  ...
  // Remove button
  {
    ...
    auto rm_item = new score::QGraphicsPixmapButton{
        pixmaps.close_on, pixmaps.close_off, item};
    connect(
        rm_item, &score::QGraphicsPixmapButton::clicked, &port, [this, &port] {
          QTimer::singleShot(0, &port, [this, &port] {
            CommandDispatcher<> disp{m_context.context.commandStack};
            disp.submit<RemoveControl>(
                static_cast<const Model&>(m_process), port);
          });
        });
...
{
  auto& portFactory
      = m_context.context.app.interfaces<Process::PortFactoryList>();
