...
//!
//! This has to be checked, not asserted: midi_in_configuration_for() only
//! fills the API configuration variant for a compiled-in backend
//! (libremidi.cpp: midi_any::for_backend), so on a build without ALSA the
//! variant holds unspecified_configuration, a get_if on
//! alsa_seq::input_configuration returns null, and asserting on it would abort
//! the application while opening the document. An absent backend is external
