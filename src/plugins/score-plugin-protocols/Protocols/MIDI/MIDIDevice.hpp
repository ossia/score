 ...
 *
 * Exported rather than file-static so MidiMissingApiTest can call it directly:
 * its per-API branches fetch the backend-specific configuration out of a
 * variant that libremidi::midi_in_configuration_for() leaves empty for a
 * backend that is not compiled in, and it must degrade to the generic
 * configuration instead of asserting, whatever API is asked for.
 */
SCORE_PLUGIN_PROTOCOLS_EXPORT
