...
static Process::MessageNode* try_getNodeFromString_impl(
    ...
    QStringList::const_iterator end, const State::DestinationQualifiers& qual)
{
  if(begin == end)
  {
    return &n;
...
static void updateNode(
    std::vector<Process::ProcessStateData>& vec, const ossia::value& val,
...
static void rec_delete(Process::MessageNode& node)
{
  if(node.childCount() == 0)
  {
    ...
    if(parent)
    {
      auto it = std::find_if(parent->begin(), parent->end(), [&](const auto& other) {
        return &node == &other;
      });
      if(it != parent->end())
      {
    ...
    const std::vector<Process::ProcessStateData>& other_vec)
{
  ...
  if(vec_size > 1)
  {
    ...
    auto it
        = ossia::find_if(vec, [&](const auto& data) { return data.process == proc; });
...
static void merge_impl(
    Process::MessageNode& base, const State::AddressAccessor& addr, MergeFun merge)
{
...
static void rename_impl(
    Process::MessageNode& base, const State::AddressAccessor& old_addr,
    ...
    Process::MessageNode& rootNode, const State::AddressAccessor& addr)
{
  ...
  if(node)
  {
    ...
    if(node->values.previousProcessValues.empty()
       && node->values.followingProcessValues.empty() && node->childCount() == 0)
    {
      rec_delete(*node);
    }
  }
...
static void
rec_getChildIndex(Process::MessageNode& rootNode, Process::MessageNode* n, int& idx)
{
  for(auto& child : rootNode)
  {
    if(child.hasValue())
