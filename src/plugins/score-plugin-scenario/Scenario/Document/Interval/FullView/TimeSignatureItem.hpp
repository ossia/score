...
struct Timebars
{
  ...
  // by-value members that would be a `delete` on an address interior to
  // Timebars. ~FullViewIntervalPresenter can order its own teardown around
  // that, but the ordering is bypassed when the scene is destroyed first
