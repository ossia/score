...
TemporalIntervalPresenter::TemporalIntervalPresenter(
    ...
    bool handles, QGraphicsItem* parentitem, QObject* parent)
    ...
    , m_handles{handles}
{
  ...
  con(metadata, &score::ModelMetadata::LabelChanged, m_header,
      &IntervalHeader::on_textChanged);
  ...
  connect(
      head, &TemporalIntervalHeader::doubleClicked, this,
  ...
  m_model.processes.removed.connect<&TemporalIntervalPresenter::on_processesChanged>(
      this);
...
{
  qreal height = 1.
                 + m_model.smallView().size()
                       * (SlotHeader::headerHeight() + SlotFooter::footerHeight());
...
void TemporalIntervalPresenter::createSlot(int pos, const Slot& aSlt)
{
  if(m_model.smallViewVisible())
  {
    ...
    else
    {
      ...
      p.footer
          = new AmovibleSlotFooter{*this, (int)m_model.smallView().size(), this->view()};
      ...
      const auto def_width = m_model.duration.defaultDuration().toPixelsRaw(m_zoomRatio);
...
void TemporalIntervalPresenter::createCollapsedSlot(int pos, const Slot& slt)
{
  ...
  if(frontLayer)
  {
    ...
    setHeaderWidth(p, m_model.duration.defaultDuration().toPixelsRaw(m_zoomRatio));
  }
  m_slots.emplace(m_slots.begin() + pos, std::move(p));
  ...
  if(m_model.smallViewVisible())
  {
    ...
    con(proc, &Process::ProcessModel::loopsChanged, this, [this, slot_i](bool b) {
      ...
      if(!(ld.model().flags() & Process::ProcessFlags::HandlesLooping))
      {
        ...
        const auto slot_height = m_model.smallView().at(slot_i).height;
        ld.updateLoops(
            m_context, m_zoomRatio, def_width, def_width, slot_height, this->m_view,
            this);
      }
      else
      {
    ...
    con(proc, &Process::ProcessModel::startOffsetChanged, this, [this, slot_i] {
      ...
      if(slot && !slot->layers.empty())
      {
        ...
        if(!(ld.model().flags() & Process::ProcessFlags::HandlesLooping))
        {
          ld.updateStartOffset(-ld.model().startOffset().toPixelsRaw(m_zoomRatio));
        }
        else
        {
          ld.parentGeometryChanged();
      ...
      for(const LayerData& ld : slt->layers)
      {
        if(ld.model().id() == proc.id())
        {
          if(auto pres = ld.mainPresenter())
          {
            ...
            {
              slt->headerDelegate
                  = factory->makeHeaderDelegate(ld.model(), m_context, slt->header);
            ...
            setHeaderWidth(
                *slt, m_model.duration.defaultDuration().toPixelsRaw(m_zoomRatio));
          }
        }
        else
        {
    ...
    if(auto lay_slt = slot.getLayerSlot())
    {
      ...
      for(LayerData& ld : lay_slt->layers)
      {
        ld.on_zoomRatioChanged(
            m_context, ratio, def_width, def_width, slot_height, m_view, this);
      }
    }
    i++;
  }
