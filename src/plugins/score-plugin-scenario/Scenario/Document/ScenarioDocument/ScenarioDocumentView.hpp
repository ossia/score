...
private:
  ...
  // by-value members that would be `delete` on an address inside this object,
  // which is not a heap allocation -- the same defect the scenes and the base
  // item have to avoid.
  //
  ...
  // As a by-value member that would be `delete` on a non-heap address.
