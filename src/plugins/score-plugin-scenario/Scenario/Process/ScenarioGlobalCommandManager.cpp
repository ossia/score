...
void clearContentFromSelection(
    const Scenario::ScenarioInterface& si, const score::DocumentContext& ctx)
{
  ...
  for(const auto& obj : ctx.selectionStack.currentSelection())
  {
    if(auto proc = dynamic_cast<const Process::ProcessModel*>(obj.data()))
    {
      ...
      if(remove_it)
      {
        ...
        {
          cleaner.submit(
              new Scenario::Command::RemoveProcessFromInterval(*itv, proc->id()));
        }
      }
    }
  }
...
bool EndDateComparator::operator()(
    const IntervalModel* lhs, const IntervalModel* rhs) const
{
