    ...
    const rapidjson::Document& json)
{
  ...
  if (same_doc)
  {
    ...
    if (auto obj = old_p.try_find(ctx))
    {
      ...
      {
        const int slot_index = json.HasMember("SlotIndex") ? json["SlotIndex"].GetInt() : -1;
        if (slot_index == -1)
        {
        ...
        else
        {
          ...
          const bool small_view = json.HasMember("View") ? JsonValue{json["View"]}.toString() == "Small" : true;
  ...
  else
  {
    ...
    if(json.HasMember(score::StringConstant().Process) && json.HasMember(score::StringConstant().uuid))
    {
