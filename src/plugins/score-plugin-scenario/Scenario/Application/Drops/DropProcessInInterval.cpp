...
public:
  DropProcessInIntervalHelper(
      const Scenario::IntervalModel& interval, std::optional<TimeVal> maxdur)
      ...
      , m_itv{interval}
  {
    ...
    {
      if(auto resizer = m_context.app.interfaces<Scenario::IntervalResizerList>().make(
             m_itv, *maxdur))
...
bool DropProcessInInterval::drop(
    const score::DocumentContext& ctx, const IntervalModel& cst, QPointF pos,
