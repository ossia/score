...
//! Given the same document, restore cables that were removed - mostly useful for undo operations.
SCORE_PLUGIN_SCENARIO_EXPORT
void restoreCables(
    const SerializedCables& cbls, const score::DocumentContext& ctx);

//! Restore cables. The objects must exist but it can be in a different document, at a different place in the hierarchy.
SCORE_PLUGIN_SCENARIO_EXPORT
void loadCables(
