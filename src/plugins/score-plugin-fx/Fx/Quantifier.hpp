...
struct Node
{
      ...
      State& self)
  {
    ...
    for (const libremidi::message& in : p1.messages)
    {
      ...
      if (in.get_message_type() == libremidi::message_type::NOTE_ON
          && note.vel != 0)
      {
        ...
        else
        {
          ...
          auto perf_date = int64_t(
              start_q
              * int64_t(1 + tk.date.impl * st.modelToSamples() / start_q));
          int64_t actual_date
              = (1. - precision) * tk.date.impl * st.modelToSamples()
                + precision * perf_date;
    ...
    for (auto it = self.to_start.begin(); it != self.to_start.end();)
    {
      ...
      if (note.date > tk.prev_date && note.date.impl < tk.date.impl)
      {
        ...
        no.timestamp
            = tk.to_physical_time_in_tick(note.date, st.modelToSamples());
        p2.messages.push_back(no);
