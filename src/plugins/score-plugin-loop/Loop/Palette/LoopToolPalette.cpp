...
Scenario::Point DisplayedElementsToolPalette::ScenePointToScenarioPoint(QPointF point)
{
  return {TimeVal::fromPixels(point.x(), m_presenter.zoomRatio()), 0};
}
...
DisplayedElementsToolPalette::DisplayedElementsToolPalette(
    ...
    : GraphicsSceneToolPalette{*view->scene()}
    , m_model{model}
    , m_scenarioModel{*safe_cast<Loop::ProcessModel*>(m_model.interval().parent())}
    , m_presenter{pres}
...
const Scenario::DisplayedElementsPresenter& DisplayedElementsToolPalette::presenter() const
{
...
const Scenario::EditionSettings& DisplayedElementsToolPalette::editionSettings() const
{
  return m_editionSettings;
}
...
std::unique_ptr<GraphicsSceneToolPalette> DisplayedElementsToolPaletteFactory::make(
    Scenario::ScenarioDocumentPresenter& pres,
    const Scenario::IntervalModel& interval,
    QGraphicsItem* parent)
{
  return std::make_unique<DisplayedElementsToolPalette>(pres.displayedElements, pres, parent);
}

bool DisplayedElementsToolPaletteFactory::matches(const Scenario::IntervalModel& interval) const
{
