...
void DeviceExplorerModel::replaceDevice(const QString& name, const Device::Node& node)
{
  auto& root = rootNode();
  auto it = ossia::find_if(root, [&](const Device::Node& n) {
  ...
  const int row = root.indexOfChild(&*it);

  beginRemoveRows(QModelIndex{}, row, row);
  auto next = root.erase(it);
  endRemoveRows();

  beginInsertRows(QModelIndex{}, row, row);
  root.insert(next, node);
  endInsertRows();
}
