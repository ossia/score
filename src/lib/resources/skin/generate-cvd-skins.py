...
def verify_doc(doc, deficiency):
    ...
    for group, roles, _min in contrast.GROUPS:
        ...
        for r in roles:
            c = doc[r][:3]
            a = doc[r][3] if len(doc[r]) > 3 else 255
            if a < 255:
                c = list(contrast.over(tuple(c) + (a,),
                                       tuple(doc["Background1"][:3]) + (255,))[:3])
            sim[r] = simulate(c, deficiency)
        for i in range(len(roles)):
            for j in range(i + 1, len(roles)):
                d = delta_e(sim[roles[i]], sim[roles[j]])
                if d < worst[1]:
                    worst = ((group, roles[i], roles[j]), d)
    return worst
...
if __name__ == "__main__":
    ...
    for name, accents in ACCENTS.items():
        ...
        doc["_comment"] = (
            ...
            f"simulation of {DEFICIENCY_OF[name]}, both as accents (worst "
            f"{pair[0]}/{pair[1]} at dE {worst:.1f}) and as finished roles "
            f"(worst {grp[1]}/{grp[2]} at dE {worst_sim:.1f}). Lifted by "
            f"contrast.py so nothing falls far below DefaultSkin. Generated "
            f"by generate-cvd-skins.py; base hues from the Okabe-Ito palette."
        )
