...
DocumentModel::~DocumentModel()
{
  ...
  // vtable. It survived because the window is one destructor wide and the
  // freed page is usually still mapped and still holds the old pointer -- the
  // gfx soak only reproduced it once the cycles came fast enough to get the
  // allocator to reuse the block.
