...
std::array<float, 3> QGraphicsXYZSpinboxChooser::value() const
{
  return m_value;
}
...
void QGraphicsXYZSpinboxChooser::mousePressEvent(QGraphicsSceneMouseEvent* event)
{
  const auto p = event->pos();
  if(p.x() < 100.)
  {
...
void QGraphicsXYZSpinboxChooser::mouseDoubleClickEvent(QGraphicsSceneMouseEvent* event)
{
  m_value = ossia::vec3f{0., 0., 0.};
  m_grab = true;
  sliderMoved();
...
void QGraphicsXYZSpinboxChooser::mouseMoveEvent(QGraphicsSceneMouseEvent* event)
{
  if(m_grab)
  {
    const auto p = event->pos();
...
void QGraphicsXYZSpinboxChooser::mouseReleaseEvent(QGraphicsSceneMouseEvent* event)
{
  if(m_grab)
  {
    const auto p = event->pos();
