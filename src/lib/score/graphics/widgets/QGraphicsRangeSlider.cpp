...
void QGraphicsRangeSlider::mousePressEvent(QGraphicsSceneMouseEvent *event)
{
    if (event->button() == Qt::LeftButton ){
        ...
        d2c = abs (event->pos().x() - (m_start + (m_end - m_start) / 2 ) * boundingRect().width());
        d2e = abs (event->pos().x() - m_end * boundingRect().width());

        if (d2s < d2c && d2s < d2e) handle = START;
        else if ( d2e < d2c && d2e < d2s) handle = END;
        else if ( d2c < d2s && d2c < d2e) {handle = CENTER; ypos = event->pos().y();}
        else handle = NONE;
...
void QGraphicsRangeSlider::mouseMoveEvent(QGraphicsSceneMouseEvent *event)
{
    switch (handle) {
        ...
        case END:
            ...
            break;
        case CENTER:
            d2c = event->pos().x() - (m_start + (m_end - m_start) / 2) * boundingRect().width();
            ydiff = ypos - event->pos().y();
            ypos = event->pos().y();
            val1 = std::clamp(m_start + d2c / boundingRect().width() - ydiff * y_factor, 0., 1.);
            val2= std::clamp(m_end + d2c / boundingRect().width() + ydiff * y_factor, 0., 1.);
...
void QGraphicsRangeSlider::paint(
    ...
    painter->fillRect(rangeRect, skin.Emphasis2.lighter180.brush); ///TODO: use right brush from skin here
