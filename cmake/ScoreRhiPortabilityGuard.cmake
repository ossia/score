...
#
# CI BUILDS THE TESTS AGAINST Qt 6.4.2. The local reference build is Qt 6.13, so
# a test can compile here and break the Coverage job there. The shape is always
# the same: an API that only exists, or only has that signature, in a Qt newer
# than CI's.
#
...
#      is to use the older name everywhere.
#
...
#
# WHAT THE ALLOW-LIST IS NOT: "if src/ uses it, it is safe". That heuristic lets
# QRhi::statistics() through: RenderList.cpp does call statistics(), but from
# inside `#if QT_VERSION >= QT_VERSION_CHECK(6, 6, 0)`. The oracle is only valid
# for UNGUARDED uses in src/.
#
...
# bites. The files under src/ that do use a concrete QRhi*InitParams:
#   src/lib/score/gfx/Vulkan.cpp                        (guarded, __has_include)
#   src/plugins/score-plugin-gfx/Gfx/Graph/Window.cpp   (guarded, QT_VERSION)
#   src/plugins/score-plugin-gfx/Gfx/Graph/KmsOutputNode.cpp
# -- the last one unguarded, reaching the per-backend private header directly.
...
function(score_check_rhi_portability)
  ...
  foreach(_f IN LISTS _files)
    ...
    # string literals go too, so a QRhi name printed in an INFO message cannot
    # trip a rule either.
