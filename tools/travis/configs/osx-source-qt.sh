...
export CMAKE_COMMON_FLAGS="-DBOOST_ROOT=$BOOST_PATH -DCMAKE_C_COMPILER=/usr/bin/clang -DCMAKE_CXX_COMPILER=/usr/bin/clang++ -DCMAKE_PREFIX_PATH=$CMAKE_PREFIX_PATH -DCMAKE_INSTALL_PREFIX=$(pwd)/bundle -DCMAKE_OSX_DEPLOYMENT_TARGET=10.10 -DOSSIA_DISABLE_COTIRE=1"
