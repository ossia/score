...
#
# THE BASELINE IS DELIBERATELY NOT COMMITTED. verdicts.tsv is one line per
# document keyed by its RELATIVE PATH, and those paths are the titles of the
# user's own working files. The rule for this case is "Never commit the user's
...
# and gate later runs against it. Only the aggregate counts are safe to quote.
set -uo pipefail
