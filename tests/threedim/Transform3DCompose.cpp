// Threedim::Transform3D + TransformHelper (52104d297f, reworked by f92f98ceab).
//
// Both are pure QMatrix4x4 / scene_state logic with no coverage of any kind.
// The two properties that matter downstream are (a) the wrap composes
// translate * rotate * scale in that order and leaves the input tree's
// identity alone, and (b) an unchanged tick does NOT bump the version — a
// spurious bump invalidates every identity-keyed cache in the preprocessor
// every frame (diagnostic 027).
//
          ...
          "[threedim][transform]")
{
  ...
  // data on every TRS pass (diagnostic 026).
