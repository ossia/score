...
//     pinning the spurious-re-dirty defect class (cf. TagAs) as
//     currently-correct.
//
// Out of scope: init()/update()/release() need a live
    ...
    "[threedim][splat][bufferloader]")
{
  ...
  // Spurious-re-dirty pin (the same defect class as in TagAs): ticking
