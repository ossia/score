...
//
// So the assertions are on the descriptor, computed on paper from the
...
// inside the buffer, and the halp::raw_buffer convention is that byte_offset
// addresses into byte_size, so the usable bytes are byte_size - byte_offset.
// Counting byte_size/stride vertices
