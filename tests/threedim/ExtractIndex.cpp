...
// the real definition, so we are exercising shipped engine code.
...
{
  // Cube: 8 vertices, 36 uint32 indices — 144 bytes, not the 8*4 = 32 a
  // vertices-sized computation would give.
...
TEST_CASE(
    ...
    "[threedim][extractbuffer][f7]")
{
  // vertex_count=1000 but index_count=60, uint16 — 120 bytes, not 2000; the
  // over-report would read 1880 bytes past the real index buffer.
