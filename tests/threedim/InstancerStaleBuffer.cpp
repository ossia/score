...
// again. The published instance_transforms must then point at the new buffer
// and the state version must have advanced; still pointing at the stale handle
// with a frozen version is the failure.
    ...
    "[threedim][instancer][f4]")
{
  ...
  // The fingerprint over ALL Points buffers changed, forcing a rebuild that
  // re-routes instance_transforms onto the NEW buffer and bumps the state
  // version.
