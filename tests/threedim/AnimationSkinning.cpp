// A pure state test (no GPU): build a scene with one animated rotation channel
// targeting the node that backs joint 0, tick once, and read the OUTPUT
// scene's skeleton. joints[0].rotation must carry the sampled value, not the
// bind-pose identity quaternion (0,0,0,1). We observe the published
// skeleton_component state, not a rendered image.
    ...
    "[threedim][animation][f9]")
{
  ...
  // The joint's LOCAL rotation TRS is the data SceneGPUState's forward
  // kinematics consumes; it is the sampled 90deg-about-Z quaternion.
