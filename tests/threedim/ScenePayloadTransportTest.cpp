// =============================================================================
// Unrendered payloads are transported, and say so.
//
...
// names invite confusing them; every assertion below spells out the exact
// type.
//
...
// tests/gfx/SceneMergeMemo.cpp (the GPU-less scene-assembly precedent).
//
...
// recompiling its TU.
//
// NOTHING IN THE PRODUCT EVER BUILDS ONE OF THESE:
//
...
//
// Registered as `test_threedim_scene_payload_transport` in
// tests/threedim/CMakeLists.txt, which compiles SceneGraphFilter.cpp (the
// transport visitor under test) and LINKS score_plugin_gfx for flattenScene --
// recompiling an exported TU into a test ODR-violates against the shared
// library.
//
// -----------------------------------------------------------------------------
// NEGATIVE CONTROL (product-side, do not commit).
//
// The FlattenVisitor is not a place to drop them from: it has no branch for
// these four kinds at all (they fall off the end of the else-if chain at
...
//     "payload transport (a): the SceneGraphFilter payload rebuild keeps all four kinds"
//     and in "payload transport (a+b): merge -> filter -> flatten end to end" that reads a
...
TEST_CASE(
    "payload transport (b): a scene of only unrendered payloads flattens to zero of "
    "everything",
    "[scene][flatten][payload][transport]")
{
...
TEST_CASE(
    "payload transport (b): the unrendered payloads add nothing to a scene that does draw",
    "[scene][flatten][payload][transport]")
{
...
TEST_CASE(
    "payload transport (a): ossia::merge_scenes carries all four kinds through",
    "[scene][merge][payload][transport]")
{
  const auto splat = make_splat();
  const auto voxels = make_voxel_field();
  const auto points = make_point_cloud();
...
TEST_CASE(
    "payload transport (a): the SceneGraphFilter payload rebuild keeps all four kinds",
    "[scene][filter][payload][transport]")
{
  const auto splat = make_splat();
  const auto voxels = make_voxel_field();
  const auto points = make_point_cloud();
...
TEST_CASE(
    "payload transport (a+b): merge -> filter -> flatten end to end",
    "[scene][merge][filter][flatten][payload][transport]")
{
  const auto splat = make_splat();
  const auto voxels = make_voxel_field();
  const auto points = make_point_cloud();
...
TEST_CASE(
    "payload transport: SceneGPUState still admits these four kinds are not rendered",
    "[scene][flatten][payload][transport][source]")
{
