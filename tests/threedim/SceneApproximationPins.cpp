// =============================================================================
// Two "this is an approximation, and the code says so" contracts, each taken
// from the engine's own admission in a shipped comment.
//
// ONLY ONE OF THE TWO ADMISSIONS IS STILL TRUE.
//
//   `area lights are point-light approximations` is REAL and is pinned RED
//   with Catch2's `[!shouldfail]`.
//
//   `SceneFilterNode mode 2 is not mode 1` is NOT. That contract -- "mode 2
//   differs from mode 1" -- is ALREADY SATISFIED by the shipped code, and it
//   is the shipped DOC-COMMENT that is wrong. It is therefore written as a
//   PLAIN GREEN test, per the rule that a pin must never be manufactured for a
//   behaviour that is already correct. Full evidence in the scene-filter
//   section below. A separate, clearly-labelled RE-SCOPED pin covers the part
//   of that same admission which IS still open (mode 2 is unconfigurable: the
//   node has no Name port at all).
//
...
// I/O except the one source-text read described below. Null-RHI is for
// *decision-logic* assertions only -- which rung was chosen, which format
// refused, what a gate returned -- and both questions here are
...
// reliably nor more cheaply, and for the filter the answer is a scene tree,
// which has no pixel form at all.
//
// -----------------------------------------------------------------------------
// WHY A NEW FILE AND NOT tests/threedim/KnownDefects.cpp
//
// KnownDefects.cpp is the tree's existing home for expected-red contracts, but
// it is the wrong home for these, on mechanism rather than taste:
//
...
//     PASS on the day it is fixed"). The untagged discrimination controls
//     these cases need would be unobservable inside a target that is red as a
//     whole, and a control whose green cannot be seen is not a control.
...
// pass (catch_totals.cpp:52-62, `testCases.passed` stays 0).
//
// =============================================================================
// `area lights are point-light approximations`. EXPECT RED.
// =============================================================================
//
// THE ADMISSION, quoted verbatim.
//
//   src/plugins/score-plugin-threedim/Threedim/Light.hpp:37-42, the tail of
//   the class doc-comment (:41-42 are its last two lines):
//
...
//   render-thread encoder itself (:141-154 is exactly the function;
//   :137-140 is its comment):
//
...
//       This half of the admission is stale and is recorded here as a plain
...
// A deliberate compromise, and the weakest thing in this file; its exact
// extent is set out below.
//
...
//   fixture ever lands, replace the parse with
...
//     to the shader, it is the ONLY one -- which is why this encoder decides
//     everything, and why a scene-level difference oracle is useless for the
//     pinned half.
//
// =============================================================================
// `SceneFilterNode mode 2 is not mode 1`. **ALREADY CORRECT -> GREEN.**
// =============================================================================
//
// THE ADMISSION, quoted verbatim from
// src/plugins/score-plugin-gfx/Gfx/Graph/SceneFilterNode.hpp:17-22 (:20-22
// are its last three lines):
//
...
// 126-141). So the contract "mode 2 differs from mode 1" holds
...
//
// ORACLE for both scene-filter cases: value-level, GPU-less, on the FILTERED
// SCENE ITSELF -- the real product visitor, never a re-implementation. The
// whole renderer is driven:
...
//
// =============================================================================
// REGISTRATION. ctest target `test_threedim_scene_approximation_pins`, in
// tests/threedim/CMakeLists.txt. It compiles Threedim/Light.cpp in (rebuild()
// is out-of-line) and defines THREEDIM_SRC_DIR for the shipped-source read, the
// ShaderStrings.cpp pattern. SceneFilterNode is SCORE_PLUGIN_GFX_EXPORT, so
// score_plugin_gfx is LINKED, never recompiled. Qt Gui is needed for
// QQuaternion (Light.cpp) and the QMatrix4x4 in TinyObj.hpp.
//
// =============================================================================
// NEGATIVE CONTROLS (product-side, do NOT commit any of them).
//
// For an expected-red pin the useful control is the reverse of the usual one:
// the local product edit that makes the PIN PASS, turning the ctest entry into
// an UNEXPECTED PASS. Both pins get
...
// half a thing.
//
// LIGHT ENCODER, MAKE-THE-PIN-PASS (reverse control).
...
//
//   MUST FLIP: "DEFECT light encoder ..." becomes an unexpected pass (entry red).
//   MUST ALSO REDDEN, expected and correct: the single `CHECK_FALSE(
//   encoder_separates(code, "Rect", "Disk"))` in "light encoder control: the shipped
...
//   This edit is a control, not a fix: RawLightData still has no field for the
//   extent, so area lights would still shade wrong.
//
// LIGHT ENCODER, REDDEN-A-NAMED-SUBSET (forward controls on the untagged half).
//   (i) Light.cpp:147, `case Light::Spot: return 2.f;` -> `return 1.f;`
//       MUST REDDEN: exactly two CHECKs, the Spot ones, in "light encoder control: the
//       shipped encoder parse can say BOTH same and different".
//       MUST STAY GREEN: the producer-level control, all of the scene-filter
//       cases. Pin red.
...
//        in "light encoder (producer, GREEN): ...". The light_type CHECKs in that same
...
//
// SCENE FILTER, MAKE-THE-RE-SCOPED-PIN-PASS (reverse control).
...
//   MUST FLIP: "DEFECT scene filter (re-scoped) ..." becomes an unexpected pass.
//   MUST STAY GREEN: every other case in this file, the light ones included
//   -- nothing else reads input.size().
//
// SCENE FILTER, REDDEN-A-NAMED-SUBSET (forward control on the green half).
...
//   MUST REDDEN: exactly the three CHECKs in "scene filter (GREEN): SceneFilterNode
//   mode 2 already is not mode 1" -- `m2 != m1`, `m2.size() != 1` and the
//   drift-watch `m2 == m0`.
//   MUST STAY GREEN: "scene filter control: the harness discriminates ..." in full
//   (mode 0 vs mode 1 is untouched) and all of the light cases. The re-scoped
//   pin stays RED (still no Name port). This is the control that matters most
//   here:
...
//   MUST REDDEN: exactly the two mode-1 assertions in "scene filter control: the
//   harness discriminates ..." (mode 1 stops dropping the invisible root, so
//   mode0 == mode1). MUST STAY GREEN: all of the light cases.
//
// =============================================================================
...
//! The difference oracle used identically by the light-encoder controls and by
//! the pin: do these two Light modes get DIFFERENT shader-facing type codes?
...
//! The scene both scene-filter behaviour cases run on. TWO ROOTS WITH THE
//! SAME NAME, differing only in `visible`. Rationale in the file header: any
//! name-based predicate is blind to `visible` and must keep both or neither,
//! while mode 1
...
// =============================================================================
// LIGHT ENCODER -- UNTAGGED CONTROLS
// =============================================================================
...
// something.
TEST_CASE(
    "light encoder (producer, GREEN): a rect light already differs from a point light "
    "in the scene_spec",
...
// The discrimination control for the PARSED-ENCODER oracle the pin uses.
...
// entry.
TEST_CASE(
    "light encoder control: the shipped encoder parse can say BOTH same and different",
    "[threedim][light][p2-11][control]")
{
  // The 64-byte budget the file header's fix note rests on: every byte of
  // RawLightData is already spoken for, so a complete fix must widen
  ...
  //
  // This one CHECK is a property of the PARSER, not a contract on the
...
TEST_CASE(
    "DEFECT: the render-thread light encoder collapses area lights onto "
...
// =============================================================================
// SCENE FILTER -- UNTAGGED CONTROL, then the GREEN behaviour case
// =============================================================================

// The discrimination control for the scene-filter oracle, run through the EXACT
...
TEST_CASE(
    "scene filter control: the harness discriminates between two genuinely different "
    "modes",
...
// **NOT A PIN.** A `[!shouldfail]` here would rest on the strength of
// SceneFilterNode.hpp:22 ("behaves like mode 1 until wired"). That clause is
...
TEST_CASE(
    "scene filter (GREEN): SceneFilterNode mode 2 already is not mode 1",
...
// =============================================================================
// SCENE FILTER -- THE RE-SCOPED PIN
// =============================================================================
...
TEST_CASE(
    "DEFECT: SceneFilterNode mode 2 has no Name port, so it "
