...
// animated translation: it must advance monotonically rather than alternate
// back to ~0 every other frame. We assert against the public OUTPUT, not
// m_playback_time, so the test does not depend on the header's internals.
