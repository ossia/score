...
// the stored m_faceSize was clamped to the device maximum. The return value is
// deliberately ignored: the clamp is observable in m_faceSize alone.
//
    ...
    "[threedim][cubemap][f3]")
{
  ...
  score::test::run_in_gui_app([&](const score::GUIApplicationContext&) {
    ...
    // Return value intentionally discarded: see the file header.
  ...
  // An over-limit request must be clamped to something the GPU can actually
  // back. TextureSizeMax alone is not enough: it is a dimension limit, and a
  // mipped RGBA8 cube at 32768 is ~32 GB, which exceeds
  // maxMemoryAllocationSize (VUID-vkAllocateMemory-pAllocateInfo-01713 on
  // NVIDIA, UNASSIGNED-vkAllocateMemory-maxMemoryAllocationSize on lavapipe).
  CHECK(out.faceSize <= out.maxSz);
  CHECK(out.faceSize <= 8192);
  CHECK(out.faceSize > 0);
}
