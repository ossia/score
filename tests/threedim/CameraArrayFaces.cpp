// Threedim::CameraArray — the producer side of issue #163.
//
// CameraArray emits the six GL-ordered cubemap faces at 90° FoV that a
// MULTIVIEW=6 shader indexes through gl_ViewIndex. Pinned here: the face
// count, the distinct ids merge_scenes needs, and the orientations — the one
// thing that is easy to get backwards, because QQuaternion::fromDirection maps
// local +Z to its argument while a GL camera looks along local -Z, so the code
// passes the NEGATED forward vector.
//
