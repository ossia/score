// The full-rebuild latch must not outlive its frame.
//
...
// must be cleared right after run_commands. If it latched, the next edge
// publication -- however small -- would be downgraded into another full
// rebuild, silently turning every later incremental diff into a from-scratch
// reconstruction.
//
...
//            incremental path; a latched flag says full=1.
    ...
    "[gfx][l3][incremental][latch][gui]")
{
  ...
  score::test::run_in_gui_app([&](const score::GUIApplicationContext& ctx) {
    ...
    // the exact shape that can latch the flag.
