// Its own single-test executable because the defect is a SIGABRT and
// the child-fork isolation has to stay to catch a regression.
//
...
//
// There are TWO levels of it. An EMPTY payload is not a JSON array and is
// caught by IsArray(). The array's CONTENTS are the same hazard and that
// guard does not reach them: rapidjson's operator[]
...
//
// GfxProcessLibrary.cpp cannot encode this itself, because the abort kills
// the binary. Two things fix that:
//
...
//      and reports zero drops. The deserializer refuses a non-array payload
//      (returning an empty list) rather than tripping rapidjson's IsArray()
//      assertion, so the child exits 0. The child-process isolation is kept
//      so a regression re-abort is caught as a wait-status, not a killed
//      test binary.
//
    ...
    "[gfx][library][gui]")
{
  ...
  // a drag source can plausibly produce. The empty payload fails
  // json.IsArray(). The rest are the SAME defect one level down, past that
  // guard: the array is admitted and rapidjson then asserts on its CONTENTS
  // -- operator[] asserts IsObject() on the element and asserts again when
  ...
  score::test::run_in_gui_app([&, payload = payload](
    ...
    if(pid == 0)
    {
      // Child: the drop under test. _exit codes: 0 = handled cleanly with
    ...
    // CORRECT behaviour: the child survives and reports no drops. The
    // deserializer refuses an unusable payload -- and an unreadable entry
    // inside a readable array -- with a qWarning rather than an assertion.
    INFO(
