...
// and supports an array suffix in the TYPE string (isf.cpp splits the type on
...
//
// CameraUBOData (Gfx/Graph/CameraMath.hpp, sizeof pinned to 240 by the
// static_assert there and re-pinned by tests/threedim/KnownDefects.cpp):
...
//
// The declared block is 6 * 240 = 1440 bytes, exactly the extent
// ScenePreprocessorNode advertises for the "camera" auxiliary: both
// publication sites pass cameraAuxByteSize(camera count) from CameraMath.hpp,
// and the consumer takes the aux's byte_size over the wrapped buffer's own
// size (RenderedRawRasterPipelineNode).
//
