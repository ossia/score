/*{
  "DESCRIPTION": "Reads the upstream `items` auxiliary and owns a `copy` auxiliary sized $COUNT_items. Writes red = items.length() / 255 and green = copy.length() / 255 into a 4x4 image, so both sizes can be read back after the upstream count changes. Used by GfxAuxSizeA5.",
  "CREDIT": "test",
  "ISFVSN": "2.0",
  "MODE": "COMPUTE_SHADER",
  "CATEGORIES": ["TEST"],
