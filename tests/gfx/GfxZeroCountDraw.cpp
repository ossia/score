...
// an "_indirect_draw_count" auxiliary (see CustomMesh.cpp). A producer that
...
//    only aborting on macOS. A counter rather than a warn-once log line, which
//    would not survive a Catch2 process running several backends in sequence:
//    the first session consumes the only line and every later one reads false.
