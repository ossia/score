...
// match what was saved. Every one of these models rebuilds its ports from the
// parsed shader, and three of them leave a different number of ports behind
// when the shader fails to parse, so a shader edit through a bad intermediate
// state is the fragile path.
//
