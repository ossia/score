...
// Vulkan. "OpenGL only" is the tempting reading -- Direct3D and Metal put the
// framebuffer origin at the top like Vulkan and so should need nothing -- but
// it is false: a compute storage image reaches the delivered picture mirrored
// on D3D11, D3D12 and Metal exactly as it does on OpenGL.
//
// gfx_csf_orient_macros asserts the resulting picture, but only where the
// backend runs, and the two backends this project's CI can run, OpenGL and
// Vulkan, are precisely the two an "OpenGL only" gate gets right. Nothing on a
// Linux box can tell the two gates apart that way.
//
...
{
  // Not platform_backends(): a miswritten gate is invisible on the two backends
