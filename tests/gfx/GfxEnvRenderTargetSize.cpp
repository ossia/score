// =============================================================================
// ENVIRONMENTLOADER'S render_target_size OVERRIDE IS HONOURED.
//
// What EnvironmentLoader actually is: Threedim/EnvironmentLoader.hpp
...
//
//   producer half   EnvironmentLoader.cpp:56-61 -- when both spinbox values are
...
//                   NodeRenderer.cpp:495-541).
//   consumer half   ScenePreprocessorNode.cpp:3705, packAndUploadCameras:
//                   `QSize rsize = renderer.state.renderSize;` (:3721) is
//                   REPLACED by the environment's render_target_size when the
//                   bit is set and both dimensions are > 0 (:3724-3732). rsize
//                   then feeds BOTH packCameraUBO call sites -- the
//                   default-camera branch (:3745) and the flattened-scene
//                   camera branch (:3756) -- which derive the projection
//                   aspect (CameraMath.cpp: aspect = w/h when h > 0) and the
//                   camera UBO's renderSize.xy from it.
//
// SCOPE (verified by grep across src/): packAndUploadCameras
...
// this file pins that as the current contract (the "extent" here is the UBO
// renderSize the shaders see, not the texture allocation).
//
...
//     (GpuUtils.hpp:285), fires the port's update() -> rebuild()
//     (GpuUtils.hpp:180, `if_possible(t.update(state))`), and the node's
//     operator()() republishes its scene each frame
//     (CpuFilterNode.hpp:286, per-frame processControlIn + update).
...
//     ossia::merge_scenes (NodeRenderer.cpp:539).
...
//     Geometry Out (ScenePreprocessorNode.cpp:2829), and the raw-raster
//     consumer name-matches its uniform INPUT "camera" against the geometry's
//     auxiliary_buffers (RenderedRawRasterPipelineNode.cpp:1678-1680 -- the
...
// Validation is readback dimensions/bytes.
//
// CPU-ONLY CASES (run on GPU-less boxes where every backend SKIPs): the
// producer stamp/clear contract of EnvironmentLoader.cpp:56-61, the
...
constexpr int kOverrideW = 96; // the override under test
...
constexpr const char* kProbeFS = R"__(/*{
  "DESCRIPTION": "probe: draws scene geometry through camera.viewProjection and encodes the camera UBO into flat colour: R = renderSize.x/255, G = renderSize.y/255, B = projection[0][0]/4. The ScenePreprocessor's render_target_size override must be visible in all three channels; the drawn footprint's aspect follows projection[0][0].",
  "CREDIT": "test",
  "ISFVSN": "2.0",
...
Outcome run_env_rts(score::gfx::GraphicsApi api)
{
  ...
  score::test::run_in_gui_app([&](const score::GUIApplicationContext& app) {
    ...
    // scene input (per-(port, source) keying, NodeRenderer.cpp:495-541) --
    // the exact shape of the 15 motivating environment_loader scores. The
    // Camera makes the flattened-camera branch (:3756) the one under test,
    // not just the no-camera default (:3745); both consume the same rsize.
    ...
    "[gfx][threedim][environment][p1-19]")
{
  ...
  // Default 0,0: no stamp (EnvironmentLoader.cpp:56-57 guard).
  env.rebuild();
  env();
  REQUIRE(env.m_state);
  CHECK(
  ...
  // {96, 48}: stamped (EnvironmentLoader.cpp:59-61).
  ...
  // sizes must still win the merge (EnvironmentLoader.cpp:51-54 comment).
    ...
    "[gfx][threedim][environment][p1-19]")
{
  ...
  // (:3745, :3756): no aspectOverride, so aspect = w/h (CameraMath.cpp).
    ...
    "[gfx][threedim][environment][scene][p1-19]")
{
  ...
  // (grep across src/: ScenePreprocessorNode.cpp:3724-3732 alone) -- so the
