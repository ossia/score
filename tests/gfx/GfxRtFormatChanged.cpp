...
// must rebuild the input render target + passes.
//
...
// NO GOLDEN, deliberately: an rgba16f golden can bake in the very defect its
// shader is there to detect, so the oracle here is analytic.
//
...
// setRenderTargetSpec (tests/fixtures/score_test/Gfx.hpp) calls
// Node::process(int32_t, render_target_spec), which stores the spec in
// Node::renderTargetSpecs and bumps renderTargetChange() (Node.hpp).
// On the next RenderList::render(), checkForChanges() raises
// renderer->renderTargetSpecsChanged (NodeRenderer.hpp), rt_changed is
// accumulated, and -- because no full rebuild is pending -- the SURGICAL branch
// runs (RenderList.cpp, `if(rt_changed && !rebuilt)`):
...
//     then releases the old input RT and recreates it at the new format
//     (`createRenderTarget(state, newSpec.format, newSpec.size, ...)`);
//   * phase B re-inits B's renderer (releaseState/initState) and rebuilds B's
//     OWN output passes so the downstream B->sink pass survives;
//   * phase C re-adds A's upstream pass into the recreated RT.
// We deliberately send a FORMAT-ONLY spec (size left unset):
// Node::resolveRenderTargetSpecs (Node.cpp) then falls back to
// renderer.state.renderSize = 64x64 = the old size, so the ONLY thing that
// changes is the format -- this test cannot pass by riding the size-change path
// GfxRtChanged.cpp already covers.
//
...
//
// NULL FALLBACK: with SCORE_TEST_API=null there are no
...
// QRhiTexture::RGBA8 before / QRhiTexture::RGBA16F after (Node.cpp,
...
// in src/plugins/score-plugin-gfx/Gfx/Graph/RenderList.cpp
// (`rt_changed |= renderer->renderTargetSpecsChanged;` -> `rt_changed |= false;`).
// The surgical branch then never runs, the intermediate RT stays RGBA8, the
// >1.0 value clips in both phases, and the post-change
// `solid(out.b, {255, 64, 0, 255})` check goes red (red stays ~128).
// Equivalently, dropping the format term from the specChanged comparison fails
// the same check.
//
    ...
    "[gfx][l3][incremental][rtchanged][rtformat]")
{
  ...
  score::test::run_in_gui_app([&](const score::GUIApplicationContext&) {
    ...
    // One sink => exactly one RenderList (Graph::renderLists()).
    ...
    // Decision before: no spec stored => default RGBA8
    // (Node::resolveRenderTargetSpecs, and the RenderTargetSpecs default in
    // Node.hpp).
    ...
    // resolves back to the same 64x64 (Node::resolveRenderTargetSpecs) -- no
    // size change.
    ossia::render_target_spec spec;
    spec.format = ossia::texture_format::RGBA16F;
    setRenderTargetSpec(*p.isf(b), specPort, spec);

    // Decision after: the spec maps through ossia_format_to_rhi (Node.cpp).
  ...
  // Format-selection decision (Node::resolveRenderTargetSpecs).
  ...
  // Before the change (intermediate RGBA8): the produced 2.0 clips to 1.0 and
  // halves to 0.5 -> red ~128; green 0.5 -> 0.25 -> ~64. An empty readback on
  // a supported backend is a FAILURE, not a skip.
  REQUIRE(out.a.valid());
  CHECK(solid(out.a, {128, 64, 0, 255}, 3));

  // After the runtime FORMAT change (intermediate RGBA16F): the 2.0 survives
  // the float round trip and halves to exactly 1.0 -> red 255. Green stays ~64:
