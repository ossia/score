...
// look healthy there. This one drives compute shaders and point-cloud geometry,
// the nodes that carry meshes, storage images and preprocessor buffers across a
// rebuild, and it resizes as well as entering and leaving full screen.
//
...
/// A compute pass and a geometry consumer alongside the plain fragment case:
/// they own the resources a resize has to carry or rebuild.
std::vector<QString> shader_set()
{
  // make_isf_node dispatches on the extension, so a .cs here is a compute node
  // with its own storage images -- the family whose resources a resize has to
  // carry or rebuild.
    ...
    "[gfx][window][screen][resize][torture]")
{
  ...
  run_in_gui_app([&](const score::GUIApplicationContext&) {
    ...
    // sample inside the animation and count zero presents, racing the window
    // server rather than catching a defect.
    //
