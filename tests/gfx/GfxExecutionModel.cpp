...
//
// Two levels of coverage, because the corpus execution-model shaders
// (shadow_cascades PER_LAYER, prefilter_ggx PER_MIP) need full scene
// infrastructure -- indexed MDI geometry, per-draw and indirect SSBOs, a source
// cubemap -- and emit depth, cubemap and array targets that BackgroundNode, with
// its 2D sampler and single-layer readback, cannot read back per target:
//
//   1. parse and descriptor coverage of the corpus shaders: the EXECUTION_MODEL
//      block and the multi-layer / cubemap OUTPUT surface must parse. No GPU
//      bake.
//
//   2. a self-authored procedural PER_LAYER render (rr-perlayer.{vs,fs}, no
//      geometry input, 4-layer colour TextureArray output) that drives the
//      runtime: all four layer invocations must run without a use-after-free or
//      validation error on every backend, which is where a per-invocation SRB
//      regression would crash or diverge past layer 0.
//
// BackgroundNode reads back a single 2D view, so per-layer, per-mip and per-face
// individual-target verification is not expressible here. The procedural render
// asserts "completes, reads back a valid non-empty frame on every backend, and
// backends agree", the strongest statement the fixture can make.
// =============================================================================
...
TEST_CASE("procedural PER_LAYER raw-raster renders all layers", "[gfx][l3][execmodel]")
{
  ...
  // error and produce a valid readback. Per-layer content verification is not
  // expressible via BackgroundNode.
