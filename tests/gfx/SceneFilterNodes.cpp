// =============================================================================
// L3 — the three scene-filter nodes added by 45d1202327:
//   FlattenedSceneFilterNode, MergeGeometriesNode, SceneFilterNode.
//
// `grep -rl 'SceneFilterNode\|FlattenedSceneFilterNode\|MergeGeometriesNode'
// tests/` was empty before this file, while all three are user-facing processes
// at the top of the stack (Threedim/{SceneFilter,FlattenedSceneFilter,
// MergeGeometries}/Executor.cpp). tests/nodes/Processes.cpp round-trips their
// MODELS; nothing exercised the filtering or the merging.
//
// The chain here is a CSF geometry producer, not a Threedim cube through
// ScenePreprocessorNode: the predicate reads geometry_spec metadata, which any
// geometry producer supplies, so the test needs neither score_plugin_threedim
// nor a document. A CSF producer emits geometry with filter_tag == 0 and
// filter_material_index == 0, the same untagged values the preprocessor stamps
// (ScenePreprocessorNode.cpp), which makes the predicate decisive: with the
// "equals 0" mode the points draw, with "differs from 0" nothing survives.
//
// SCOPE. SceneFilterNode takes a scene_spec, and the only scene_spec sources in
// the tree are halp producers in score_plugin_threedim reached through the
// Crousti wrapper; its tree-rewriting visitor also lives in an anonymous
// namespace, so it is unreachable from another translation unit. What is
// asserted for it here is its port surface and its control routing; the
// share-copy identity contract stays uncovered and is called out below.
// =============================================================================
