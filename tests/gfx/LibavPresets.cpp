  ...
  run_in_app([&](const score::GUIApplicationContext& appctx) {
    ...
    if(isInput)
    {
      ...
      if(lavfi)
      {
        ...
        // filter output across major versions -- which is why the source here
        // is `yuvtestsrc` rather than `mandelbrot` -- so on a machine whose
        ...
        // catches anything -- score checking score.
        //
