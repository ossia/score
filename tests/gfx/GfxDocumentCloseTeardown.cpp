// =============================================================================
// Closing a document mid-render tears
// down cleanly, and GfxContext::destroyOutput unregisters the output from the
// render clocks BEFORE it destroys it.
//
// ctest name `test_gfx_document_close_teardown`. The directory is tests/gfx/:
// this drives the real plug-in's GfxContext through Gfx::DocumentPlugin, which
// is exactly what tests/gfx/GfxEdgeConsumeLatch.cpp does, and it needs
// GFX_TEST_CORPUS_DIR for its ISF producer -- both of which score_add_gfx_test
// supplies.
//
...
// Gfx::DocumentPlugin (installed on every created document by
// ApplicationPlugin::on_createdDocument), its GfxContext, and the real producer
// channel GfxExecutionAction::setEdge/endTick -- the same rig
// GfxEdgeConsumeLatch.cpp uses. One ISF producer (corpus/isf-solid-color.fs)
// feeds one offscreen BackgroundNode sink, which is what the app's headless
// window device owns.
//
...
// whose tick closure is, verbatim:
//
//     clock->start([clock] {
...
// Counting them is a direct runtime observation of clock membership -- not a
...
//     use-after-free destroyOutput's own comment describes, because in
//     production the node is freed moments later (~offscreen_device, or
//     ~GfxContext's `nodes` map).
//
...
// become empty, so a lone output leaves no clock at all and the silence would
// prove nothing. The destroyOutput case therefore registers TWO sinks at the
// same rate -- which recomputeTimers coalesces onto ONE shared TimerClock --
// and tears only the first one down. The second, the witness, must go on being
// ticked throughout the post-teardown pump; if it does not, the child reports
// CHILD_CLOCK_STOPPED and the case fails as inconclusive rather than passing on
// a dead loop. With the negative control below applied, the victim stays in
// that same, demonstrably-live clock, so its counters cannot stay at zero.
//
...
// began. It does NOT distinguish a pure statement swap -- moving the
// render-clock unregister loop to just after destroyOutputRenderList with
// nothing in between -- because nothing pumps the event loop between those two
// statements, so no tick can be delivered in the gap and there is nothing to
// see. Making the probe's destroyOutput() pump the loop itself does not work
// either: at that moment Graph::releaseOutputRenderList has already done
// `output.setRenderer({})` while renderState() is still live, so
// GfxContext::updateGraph -> Graph::createMissingRenderLists would REBUILD the
// render list in the middle of the teardown -- a self-inflicted crash on a
// correct build, i.e. a fake red. What is asserted instead is the strongest
// thing observable from outside the plug-in: the unregister happened, before
// control ever leaves destroyOutput.
//
// -----------------------------------------------------------------------------
// NEGATIVE CONTROL ("destroy before unregistering from the render clock
// -> UAF"). The hook is exact: in
// src/plugins/score-plugin-gfx/Gfx/GfxContext.cpp, delete the whole
// render-clock unregister loop in GfxContext::destroyOutput
//
...
//           }
//
// so that destroyOutput goes straight to m_graph->destroyOutputRenderList().
//
...
//   the clocks by a different route (m_renderClocks.clear() before
//   `delete m_graph`).
//
//   The ~GfxContext half has its own control: move that
//   `m_renderClocks.clear();` to after `delete m_graph;` -- ~Graph then calls
//   out->destroyOutput() on nodes the clocks still point at, and the timer pool
//   is still alive because std::destroy_at(&m_timers) has not run either. That
...
// verbatim, as tests/gfx/GfxIncrementalResizeFork.cpp.
//
// PARENT STAYS GRAPHICS-FREE BEFORE FORKING, for the same reason that file
// gives: forking a process that already owns a GL/Vulkan context or a booted
// QApplication is undefined behaviour -- driver threads, fds and locks do not
// survive fork(). The parent here only calls platform_backends() (a qgetenv),
...
// score_test/ForkProbe.hpp); no usable RHI backend for the generated api
// (probe_api, score_test/Gfx.hpp); no document delegate. A backend that cannot
// come up is CHILD_SKIP, never a silent Null-backend pass -- the house rule
// for hardware.
// =============================================================================
...
// ours -- same convention as GfxIncrementalResizeFork.cpp.
// ---------------------------------------------------------------------------
...
// (the TimerClock marshals its tick onto its owner's thread), and each forked
// child runs exactly one scenario.
// ---------------------------------------------------------------------------
long long g_seq = 0;
...
/// The offscreen sink the app's headless window device owns, instrumented at
/// the two points the render clock's tick closure touches and at the teardown
/// point Graph::destroyOutputRenderList calls.
///
...
/// platform, so on a display-less box the child would
...
enum class Teardown
{
  ...
  //! ~Gfx::DocumentPlugin -> ~GfxContext.
  CloseDocument,
  //! GfxContext::destroyOutput(), the synchronous path a device-owned output
  //! takes.
  DestroyOutput
};
...
/// the status pipe, as GfxIncrementalResizeFork.cpp does.
ChildRun run_child_scenario(score::gfx::GraphicsApi backend, Teardown how)
{
  ...
  score::test::run_in_gui_app([&](const score::GUIApplicationContext& ctx) {
    ...
    // takes the TimerClock branch) and 60 Hz, which is also what
    // BackgroundNode's ctor turns into its manualRenderingRate.
    ...
    // live render clock: SKIP instead.
    ...
    // A real producer: without one, BackgroundNode::render() short-circuits on
    // its renderers.size() > 1 gate and nothing is ever recorded on the GPU --
    // "mid-render" would be a lie.
    ...
    // the same rate, so the two coalesce onto one shared TimerClock and the
    // clock is NOT erased when the victim leaves it. Without it, "no callbacks
    // after the teardown" would be satisfied by a clock that no longer exists.
    ...
    // The real producer channel, exactly as GfxEdgeConsumeLatch.cpp drives it:
    // every tick re-sends the complete edge set; endTick publishes
    ...
    // which is what creates the TimerClock.
    g.updateGraph();
    ...
    switch(how)
    {
      ...
      case Teardown::DestroyOutput:
        // The synchronous device-owned-output path. It RELEASES the
        // GfxContext's ownership of the node, so the sink is ours to free
    ...
    if(victim)
    {
      // GfxContext::destroyOutput released its unique_ptr; the caller is the
      // owner now, exactly as ~offscreen_device's m_node is.
      delete victim;
      victim = nullptr;
    }
  });
...
ForkedRun fork_and_run(score::gfx::GraphicsApi backend, Teardown how)
{
  ...
  if(pid == 0)
  {
    ...
    // off waitpid() -- the same convention ForkProbe.hpp documents.
    ...
    "[gfx][l3][document][lifecycle][fork]")
{
  ...
  // no matter how early it happens. A signal-11 verdict from this rig on macOS
  // is the harness's own label for the child slot, not a diagnosis of where it
  // died.
  //
  // The fix is for the child to exec() before touching Qt -- re-exec this
