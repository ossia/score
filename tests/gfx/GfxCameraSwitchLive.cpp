// =============================================================================
// SWITCHING THE ACTIVE CAMERA MID-RENDER MOVES THE IMAGE, AND
// SWITCHING BACK REPRODUCES THE FIRST FRAME BYTE-FOR-BYTE.
//
// HOW THE ACTIVE CAMERA IS SELECTED:
//
//   field       scene_state::active_camera_id, a scene_node_id
//               (3rdparty/libossia/src/ossia/dataflow/geometry_port.hpp).
//   producer    Threedim::Camera mints a stable non-zero node id on first
//               rebuild (Camera.hpp), stamps it on its emitted scene_node
//               (Camera.hpp) and sets active_camera_id = that id on every
//               rebuild (Camera.hpp) -- each Camera nominates ITSELF.
//   merge       ossia::merge_scenes keeps the FIRST non-zero active_camera_id
//               in contributor order (geometry_port.cpp) and stamps it
//               on the merged state (geometry_port.cpp).
//   flatten     flattenScene collects tree-payload cameras with the enclosing
//               node's id (SceneGPUState.cpp; currentNodeId set in
//               visitNode, SceneGPUState.cpp) and resolves
...
//               fs.activeCameraIndex);` in the pack loop,
//               ScenePreprocessorNode.cpp); the flatten+pack runs
//               unconditionally every frame and the buffer is diff-uploaded.
...
//               (ScenePreprocessorNode.cpp); raw-raster shaders get
//               it as the "camera" auxiliary buffer by name
//               (ScenePreprocessorNode.cpp, mechanism proven by
...
//   * A CameraSwitch process (Threedim/CameraSwitch.hpp) whose Select mode
//     forwards the picked upstream camera scene verbatim (CameraSwitch.hpp,
//     `index` control). TRANSPORT CAVEAT: at
...
//     (Crousti/GpuUtils.hpp), and that merged scene is built by
//     NodeRenderer::rebuildMergedScene across ALL (port, source) entries
//     regardless of which port they arrived on (NodeRenderer.cpp, including
//     its per-port store). So wiring two Camera GfxNodes into a
...
// (ScenePreprocessorNode.cpp), never through a texture-channel
// reallocation. Scope: this pins the no-texture-realloc half, and the
// byte-exact phase-C return is what pins that nothing downstream
...
// (Camera.hpp), merge-order first-non-zero-wins
// (geometry_port.cpp), CameraSwitch Select forwarding with exact
// pointer return (CameraSwitch.hpp), and flattenScene's id-to-index
// resolution incl. the fallback (SceneGPUState.cpp; flattenScene is
// SCORE_PLUGIN_GFX_EXPORT, SceneGPUState.hpp).
//
...
//   `const int active = 0;` (hard-code camera index 0 at the selection
//   site). The roots order is FIXED [camA, camB], so slot 0 is
...
//   selection lives at SceneGPUState.cpp (the id-match loop);
//   deleting that loop instead turns the flattenScene CPU case red.
//
// Registered in tests/gfx/CMakeLists.txt beside the
...
// the test target (CameraSwitch is header-only and needs no extra TU), and
// OffsetAllocator is linked in because this TU reads GpuResourceRegistry
// state through RenderList::registry().
//
...
namespace
{
constexpr int kSize = 128; // offscreen sink: 128 x 128 (proven framing)
...
// byte-match CameraUBOData, Gfx/Graph/CameraMath.hpp; omitting
...
constexpr const char* kProbeFS = R"__(/*{
  "DESCRIPTION": "probe: draws scene geometry through camera.viewProjection and encodes the ACTIVE camera's world position into flat colour: R=(x+4)/8, G=(y+4)/8, B=(z+4)/8. Slot 0 of the packed camera UBO is the active camera, so the drawn colour identifies WHICH camera the ScenePreprocessor selected.",
  "CREDIT": "test",
  "ISFVSN": "2.0",
...
Outcome run_camera_switch(score::gfx::GraphicsApi api)
{
  ...
  score::test::run_in_gui_app([&](const score::GUIApplicationContext& app) {
    ...
    // iteration order: first NON-ZERO wins (geometry_port.cpp).
    p.render(6);
    ...
    // (SceneGPUState.cpp), packAndUploadCameras packs camB at slot 0
    // (ScenePreprocessorNode.cpp), and the probe turns camB-coloured.
    ...
    "[gfx][threedim][camera][p1-3]")
{
  ...
  // Camera.hpp: a non-zero id is minted on first rebuild, and each camera
  // nominates itself on every rebuild.
  ...
  // Camera.hpp -- the emitted scene_node carries the id, which is what
    ...
    "[gfx][threedim][camera][p1-3]")
{
  ...
  // geometry_port.cpp: first non-zero wins, and it is stamped on the merged
  // state. Both cameras stay present either way.
  {
...
TEST_CASE(
    ...
    "[gfx][threedim][camera][p1-3]")
{
  // The halp-level contract: Select forwards the picked upstream scene_spec
  // verbatim, per-tick, driven by the `index` control (CameraSwitch.hpp).
  ...
  // (Crousti/GpuUtils.hpp; NodeRenderer.cpp merges across
...
TEST_CASE(
    ...
    "[gfx][threedim][camera][p1-3]")
{
  // SceneGPUState.cpp: index defaults to 0, then an id-match loop resolves
  // it. Cameras are collected with the enclosing node's id. flattenScene is
  // SCORE_PLUGIN_GFX_EXPORT (SceneGPUState.hpp).
  Threedim::Camera a, b;
  const auto sa = tickCamera(a, kEyeA);
  const auto sb = tickCamera(b, kEyeB);
    ...
    "[gfx][threedim][camera][scene][p1-3]")
{
  ...
  // flatten + diff-upload (ScenePreprocessorNode.cpp), not through
