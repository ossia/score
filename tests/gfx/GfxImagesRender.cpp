...
//
// GfxImagesProcess.cpp covers the model, not the pixels, and the failure this
// process is prone to is a pixel-level one: hit play and the image does not
// come up, then clicking a preview viewport -- which rebuilds the render lists
// -- makes it appear on the main output. That is the incremental path: the
// renderer is initialized before the exec thread has delivered the image list,
// so texture creation, upload and SRB rewrite all happen through
// update()/Message processing on a live renderer, and a full rebuild hides any
// mistake there because initState() then runs with the images already present.
//
// So every scenario here permutes when the image list reaches the node relative
// to renderer creation and rendering, and asserts the readback on every
// available RHI backend.
