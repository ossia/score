...
{
  // CpuAnalysisNode.hpp had 0.0% coverage -- nothing in the tree instantiated
  // the shape that selects it. The oracle is the PIXEL the node saw: the ISF
  // producer paints a known flat colour, so "it ran" and "it received the right
  // image" are separable, which a call-counter alone would not give.
  const auto api = GENERATE(from_range(platform_backends()));
