...
//
// So an ISF/CSF shader declaring a cube storage image (RESOURCES entry with
// TYPE "image_cube" -> `imageCube` in the generated GLSL) cannot bake for
// D3D11 or D3D12, at SM 5.0 or at SM 6.1, while OpenGL, Vulkan and Metal
// translate it fine. The message comes out of SPIRV-Cross's
// image_type_hlsl, not out of fxc/dxc, so this is a code-generation failure and
// not a driver one -- it happens on any host, with or without a D3D device.
//
// THE LOWERING, AND WHY IT COSTS THE RUNTIME NOTHING
//
...
//
// Qt's D3D backends bind a 2D-array view regardless: a QRhiTexture with the
// CubeMap flag gets its UAV created as D3D11_UAV_DIMENSION_TEXTURE2DARRAY
...
// Only the shader's spelling of that view is untranslatable. Nothing about the
...
//     through IMG_STORE_CUBE -- asserted against the bytes on disk, so a
//     change that "works" by rewriting the corpus cannot pass;
...
// (gfx_csf_cube_array), which needs a real backend -- and a host without a
// Direct3D device runs only OpenGL and Vulkan, i.e. the two backends the
// constraint does not touch. The Direct3D render leg is left to a Windows sweep.
//
...
{
  // Not platform_backends(): the defect is invisible on the two backends this
    ...
    "[gfx][l1][csf][cube][hlsl]")
{
  ...
  // The un-lowered imageCube fails at BOTH, so neither may be left behind.
    ...
    "[gfx][l1][csf][cube][hlsl][api]")
{
  ...
  // (a) On disk. Making D3D pass by rewriting the shader author's source into
  // an image_2d_array with hand-rolled face maths is the change this forbids.
    ...
    "[gfx][l1][csf][cube][hlsl]")
{
  ...
  INFO("emitted HLSL:\n" << b.code.toStdString());
  // SPIRV-Cross emits the author's own (xy, face) as the array subscript,
  // whole, with no remap, split or reorder around it.
