...
// is correct and distinct, on every RHI backend. mrt-single-color and
// isf-mrt-four-outputs are asserted in Gfx.cpp; the color+depth and G-buffer
// variants are here.
//
// Corpus covered here:
...
//                              one image input.
//
// The MRT + persistent-SSBO shader (isf-mrt-persistent-ssbo.fs) lives in its
// own target, test_gfx_isf_mrt_persistent, so that a crash there cannot take
// this file down -- see IsfMrtPersistent.cpp.
//
...
//   on both backends, so only the presence of the port is asserted -- depth
//   content would need an R32F / depth readback.
// -----------------------------------------------------------------------------
TEST_CASE(
    "isf-mrt-depth-color exposes color+depth ports", "[gfx][l3][isf][mrt]")
{
  ...
  // Attachment 1 = depth: reads back as black in RGBA8 on both backends, so
  // only the presence of the port is asserted.
...
//
// The passthrough's green channel (uv.y) shows a vertical-orientation
// difference between OpenGL and Vulkan through the linear-chain path, a
// cross-node intermediate-texture Y convention interaction, so only the
// Y-flip-invariant red (X) gradient is asserted here.
// -----------------------------------------------------------------------------
TEST_CASE("mrt-gbuffer writes color/normals/edges ports", "[gfx][l3][isf][mrt]")
{
  ...
  // colorOut: passthrough of uv gradient -> red rises left->right; the X axis
  // is consistent across backends, unlike green/Y (see the file header).
