...
//   directly: GfxPipeline::removeNodeIncremental == Graph::removeNodeAndEdges +
//   Graph::removeNode. removeNodeAndEdges is the engine path that
...
//   releases the removed node's own renderers (removeNodeFromRenderLists) and
//   then reconcileAllRenderLists releases every renderer of a node the removal
//   made transitively unreachable and erases its node->renderedNodes entry
...
//   removed Node object in a nursery which run_commands flushes via
//   QTimer::singleShot(100, ...), so the Node OBJECT outlives its graph
//   membership by ~100 ms. The fixture reproduces that window
//   DETERMINISTICALLY instead: the removed node object stays alive for the rest
//   of the run (owned by GfxPipeline, still receiving per-frame Messages from
//   render()) while it is out of the graph -- exactly the state the nursery
//   keeps a node in until the timer fires. We still pump 12 frames
//   (>= 10) after the removal, so any deferred or queued destruction that
//   renders interleave with has fired before the post-removal frames are
//   asserted.
//
...
//       (cb.beginPass(rtForPass, Qt::transparent, ...) in
//       RenderedRawRasterPipelineNode.cpp), and a mesh whose geometry went away
//       drops the pass until geometry comes back -- a dropped pass leaves the
//       last valid frame in the target, a running pass with no draw leaves the
//       clear. Both are legitimate; both are uniform and stable, so the
//       post-removal frames must also be identical to each other. A draw from a
//       freed geometry buffer is ASan's to catch; a garbage frame (non-uniform
//       or off-colour) is this test's to catch.
...
//       node the removal made transitively unreachable, while the
//       still-reachable consumers keep their renderers. Removing the TOP
//       producer A orphans exactly {A} (F stays reachable through its F->R
//       edge); the follow-up removal of F then orphans {F}, mirroring
//       GfxNodeRemoval.cpp's staged removals.
//
// NEGATIVE CONTROL (product-side):
//   1. Neuter the unreachable-renderer release in
//      Graph::reconcileAllRenderLists -- comment out its
//      renderer->releaseState(*rl) / delete renderer /
//      node->renderedNodes.erase(rn_it) block. A's renderedNodes stays at 1
//      after the removal -> the CHECK(aRenderersAfterRemoveA == 0) below goes
//      red, and the leaked renderer's GPU buffers dangle for ASan.
//   2. Destroy the node immediately instead of via
//      the deferred path: in GfxContext::remove_node replace
//      nursery.push_back(std::move(node_it->second)) with
//      node_it->second.reset(), or drop the QTimer::singleShot and clear the
//      nursery synchronously inside run_commands -> use-after-free on the
//      app-level removal path.
// =============================================================================
...
// syn-geo-producer.cs -> green. Bounds match SyntheticFeatures.cpp.
...
// (RenderedRawRasterPipelineNode.cpp), i.e. near-black RGB. Alpha is
    ...
    "[gfx][l3][incremental][removal][raster]")
{
  ...
  score::test::run_in_gui_app([&](const score::GUIApplicationContext&) {
    ...
    // must release A's renderer and erase its renderedNodes entry. F stays
    // reachable (F->R edge intact) and must keep its renderer; so must R. The
    // A node OBJECT stays alive, owned by the fixture -- the deterministic
    // equivalent of GfxContext's nursery window.
    ...
    // >= 10 frames after the removal, split so two independent
  ...
  // be released and its renderedNodes entry erased. The still-reachable
