...
{
  // The TEMPORAL TYPE:"audio" family is covered elsewhere: setAudio() /
  // render_isf_audio() (Gfx.hpp) feed a known ossia::audio_vector into the
  // node's audio inlet through the same ProcessNode::process(port,
  // audio_vector) path the exec engine uses, and a constant buffer is analytic
...
{
  // Control-value injection exists (setControl / render_isf_controls in Gfx.hpp;
  // see test_gfx_control). What stays undrivable in isf-all-input-types.fs is
  // the non-2D texture inputs (cubemap / 3D) and audioFFT / audioHist.
...
TEST_CASE(
    ...
    "[gfx][l3][isf][inputs]")
{
  // The fixture injects control values through the same material-UBO path the
  // exec engine drives (setControl -> ProcessNode::process(port, ossia::value)
  // -> material slot + materialChange). float / color / point2D / point3D /
  // bool / long are covered analytically per backend in test_gfx_control
  // (GfxControl.cpp); this anchor only records that.
  //
