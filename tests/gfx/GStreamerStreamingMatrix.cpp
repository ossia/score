...
TEST_CASE("every mapped GStreamer format carries the picture at an even size",
          "[gfx][gstreamer][matrix][media]")
{
  // FINDINGS 1-3 of VideoFrameLayoutTest, end to end: a chroma plane pointed
  // nowhere (NV16, P010) or a stride of 160 bytes for a 256-byte row (Y210).
  // Nothing about the geometry is odd here -- those three are mis-described
  // outright when they are wrong.
  const auto& sweep = evenSweep();
  if(!sweep.ran)
    SKIP(sweep.skipReason);
          ...
          "[gfx][gstreamer][matrix][media]")
{
  ...
  // rounding those down to 32 wide, and starting plane 2 at height/2 = 19 rows
  // in, puts plane 2 inside plane 1.
  const auto& sweep = oddSweep();
  if(!sweep.ran)
    SKIP(sweep.skipReason);
