    ...
    "[gfx][l3][incremental][removal]")
{
  ...
  score::test::run_in_gui_app([&](const score::GUIApplicationContext&) {
    ...
    // and remove the now-orphaned M too: a stale renderedNodes[rl] entry is
    // what a later releaseState-on-freed-list UAF trips over.
    p.render(3);
    out.b = p.readback(s0);
  ...
  // their renderer released and their renderedNodes entry erased; left
  // non-empty they are leaked and dangling.
