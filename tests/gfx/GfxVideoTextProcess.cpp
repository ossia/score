...
//
// Both are "leaf" processes — no shader to edit — so what matters is the
// property commands a live edit drives (the clip, the
...
QString test_clip()
{
  ...
  // ffmpeg died -- a full disk, say -- leaves a zero-byte clip.mp4 behind. That
  // run returns {} and SKIPs correctly, but every later run in the same scratch
  // dir passes the existence check below and hands the stub to the decoder, on
  // every backend: ffprobe says "moov atom not found", makeDecoder() returns
  // null, and the case fails.
  //
