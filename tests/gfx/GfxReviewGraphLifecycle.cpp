    ...
    "[GraphLifecycle][registry]")
{
  ...
  score::test::run_in_gui_app([&](const score::GUIApplicationContext&) {
    ...
    {
      // Buffer readbacks take QRhiBufferReadbackResult* on Qt 6.4 and fold
      // into QRhiReadbackResult later; RenderState.hpp aliases the older name
      // for newer Qt, so this one spelling compiles on both.
      QRhiBufferReadbackResult pr;
