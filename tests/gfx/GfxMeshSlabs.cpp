...
//
// GfxDynamicSlot.cpp calls sweepMeshSlabs() for its dynamic-texture side effect
// but never acquireMeshSlab(), so the whole slab path was inert during the one
// test that looked related, and the scalar-arena allocate/free/isLive surface
// had no test at all.
//
...
//    its component; the preprocessor validates it with isLive() before reading
//    the arena. Without the generation bump, a ref to a freed-and-reallocated
//    slot still validates and the consumer reads another producer's bytes.
//
//  * the mesh-slab grace queue. A slab re-acquired with different counts is
//    enqueued rather than freed, because an indirect_draw_cmds entry issued
//    last frame may still reference its byte offset through an in-flight draw.
//    The enqueue stamps released_frame = current_frame; stamping 0 instead
//    collapses the guard to "wait `grace` frames after boot", after which every
//    release is freed on the very next sweep — a same-frame use-after-free.
//    usedBytes (decremented eagerly at logical release) and freeBytes (read off
//    the OffsetAllocator) diverge exactly while a release sits in the queue,
//    which is what makes the guard observable from outside.
//
// init() needs a real QRhi for the arena and stream buffers, so this runs on
// every available backend like its neighbours and SKIPs where none comes up.
// =============================================================================
