...
struct Arenas
{
  ...
  // Cross-arena probe. foreignProbeRan is the positive control:
...
bool withRegistry(score::gfx::GraphicsApi backend, F&& f)
{
  ...
  score::test::run_in_gui_app([&](const score::GUIApplicationContext&) {
    ...
    // whole device -- on D3D12 that shows up as "Live Object" lines at exit.
    //
    ...
    // know whether the QRhi is still alive. Relying on the destructor leaves
    // four arena buffers allocated (env/0, env/1, raw_camera/0, raw_camera/1)
    // and Vulkan aborts the process on st->destroy():
...
TEST_CASE("GpuResourceRegistry: the scalar arenas and their ABA guard", "[gfx][l3][registry][arena]")
{
  ...
  out.skipped = !withRegistry(
      backend, [&](Reg& reg, QRhi&, QRhiResourceUpdateBatch& batch) {
        ...
        // A ref from ANOTHER arena passes isLive(), because isLive
  ...
  // The positive control comes first: if the probe never ran, the
