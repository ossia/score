// =============================================================================
// UNIT -- GPUBufferScatter dispatch-dims clamp and shader index linearization.
//
// GPUBufferScatter::dispatch() must not put the entire workgroup count on the X
// axis: element_count is the per-attribute vertex or instance count of geometry
// being format-converted, so it is content-controlled and can exceed
// 65535 * 256 = 16,776,960, and an X dispatch above 65535 groups is invalid
// (GL_INVALID_VALUE, or VUID-vkCmdDispatch-groupCountX).
//
// computeDispatchDims() spreads the count across Y and Z so no axis exceeds the
// backend max, and the scatter shader reconstructs the linear element index from
// all three gl_GlobalInvocationID components through the params UBO:
//     width_x = num_workgroups_x * localX          (localX = 256)
//     i = (gid.z * num_workgroups_y + gid.y) * width_x + gid.x
//
// Scope: computeDispatchDims() is a private const member and the linearization
// lives in a GLSL string, so neither is externally callable. This is a unit test
// of the ALGORITHM: it replicates both formulas verbatim, with the header's
// default backend max of 65535, and asserts three properties across element
// counts spanning the 65535 boundary --
//     coverage    the grid dispatches at least element_count threads;
//     axis bounds every axis is <= the backend max;
//     bijection   the linearization maps the dispatched thread box one-to-one
//                 onto [0, total_threads), so every element index is written by
//                 exactly one thread and over-dispatched threads are the only
//                 slack.
// Replicating rather than linking means this cannot flip RED against a changed
// engine: if the production formula changes, this reference must be updated in
// lockstep.
// =============================================================================
