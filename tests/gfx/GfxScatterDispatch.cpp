...
void checkDims(uint32_t element_count, bool sweep)
{
  ...
  // (b) AXIS BOUNDS — no axis may exceed the backend max.
    ...
    "[gfx][unit][scatter][dispatch]")
{
  ...
  // One element past that: a single-axis dispatch would need 65536 workgroups
  // and overflow X. x clamped to <= 65535, y >= 2.
  {
  ...
  // 65536*256 = 16,777,216 elements -> 65536 workgroups. A naive single-axis
  // dispatch is 65536 > 65535 (INVALID); computeDispatchDims keeps x <= 65535
  // and spreads the remainder onto Y.
  {
    ...
    REQUIRE(naiveWorkgroups > 65535);       // ...which no backend accepts
    const DispatchDims d = computeDispatchDims(n);
    CHECK(int64_t(d.x) <= 65535);
    CHECK(d.y >= 2);
    checkDims(n, /*sweep*/ true);
  ...
  // Large content-controlled count (20,000,000 vertices).
