// =============================================================================
// L3 GPU render + readback tests: the offscreen ISF pipeline loop across every
// RHI backend available on this machine.
//
// Each test builds a minimal pipeline with score::test::gfx::render_isf_chain,
// renders it offscreen for a few frames, reads back the RGBA8 output textures
// and asserts on pixel values, iterating platform_backends() with GENERATE so
// the identical readback is validated on every backend.
//
// A backend that cannot initialize here -- no driver or ICD, wrong platform, a
// legacy GL context -- is SKIPped for that backend only, with its name in the
// reason. An available backend must render and match.
//
// All GPU work runs on the main thread inside run_in_gui_app; pixels are
// harvested there and the assertions run afterwards.
//
...
// -----------------------------------------------------------------------------
// TODO: CSF compute readback. csf-image-r32f.cs writes a radial gradient into a
// 256x256 R32F image through a COMPUTE_SHADER pass that a downstream ISF shader
// samples. Driving it needs the fixture to build a CSF ISFNode through the
// compute constructor, run the dispatch, and read back an R32F rather than an
// RGBA8 texture.
// -----------------------------------------------------------------------------
