// =============================================================================
// The academy shader-include path is registered, or its absence is
// diagnosable.
//
...
//
//   * shaderIncludePaths() builds the search list fresh on every call: the
//     Library packages directory
//     (Library::Settings::Model::getPackagesPath() == RootPath + "/packages")
//     plus every first-level subdirectory of it, so a shader library shipping
//     as a package (openpbr/, lygia/, ...) is includable by bare header name.
//     There is NO separate runtime registration API: shaderIncludePaths()
//     itself states "no static registration mechanism lives here anymore".
//     "Registering" the
...
//     resolveIncludes() sets
//       Shader include not found: "<header>" (searched: <path list>)
//     and aborts the expansion. ProgramCache::get() forwards it prefixed
//     with the stage ("Fragment: ", "Vertex: ") and returns std::nullopt --
//     the caller gets a diagnostic, not a frame.
//
//   * Included files are framed with `#line 1 "<canonical path>"` markers so
//     a glslang error inside the header names the header, not the including
//     shader.
//
//   * Failures are NOT cached: ProgramCache::get() inserts into `programs`
//     only on success, so the same source recompiles cleanly once the package
//     appears.
//
...
// constructs score::GLCapabilities{}, which wants a GL context; the Vulkan
// answer is the constant QShaderVersion(100) and QShaderBaker always emits
...
// (src/addons/CMakeLists.txt); there is no runtime plugin-disable switch.
// So the positive half detects availability at runtime --
...
//
// Registered in tests/gfx/CMakeLists.txt in APP mode: the resolver reads
// Library::Settings::Model out of score::AppContext(), so a booted application
// is required, exactly like test_unit_packagemanager_firstrun.
//
...
// search path is the packages dir plus its first-level subdirectories, re-read
// on every resolution. Register a temp library, resolve; deregister, and the
// failure names the header and where it looked.
//
// Negative control: make the quoted-include miss non-fatal -- i.e. replace
// resolveIncludes()'s `ctx.error = ...; return {};` with the bracketed
// branch's verbatim fallthrough -- and the "deregistered" half of this case
// goes red: preprocessShaderIncludes
    ...
    "[gfx][shader][include]")
{
  score::test::run_in_app([&](const score::GUIApplicationContext& ctx) {
    ...
    // A package subdirectory's header resolves by bare name...
    {
      ...
      // ...framed with a #line marker naming the header, so downstream
      // glslang errors are attributable to the included file.
...
// =============================================================================
// Negative then positive, through the pipeline the lgm scores actually use:
// ProgramCache::instance().get().
//
...
// was not cached (ProgramCache stores only successes).
//
// Negative control: cache the failure too -- hoist `programs[cacheKey] = ...`
// above the include resolution, or store the nullopt result on the fragment
// error path -- and the second half of this case goes red: the
    ...
    "[gfx][shader][include][openpbr]")
{
  score::test::run_in_app([&](const score::GUIApplicationContext& ctx) {
    ...
    // constant QShaderVersion(100), whereas the OpenGL branch instantiates
    // GLCapabilities, which wants a GL context this offscreen test does not
    // have.
    ...
    // --- Without the include path: DIAGNOSABLE failure, not a black frame.
    {
      ...
      REQUIRE(!error.isEmpty());
      // Stage prefix from ProgramCache::get....
    ...
    // scan) and recompile the SAME source.
...
// =============================================================================
// The positive half against the REAL openpbr.h, on machines that have it. This checkout does not build score-addon-academy (src/addons/
// carries no such directory; addons build when present there and absent from
// the configure-time SCORE_DISABLED_PLUGINS list, src/addons/CMakeLists.txt),
// so availability is detected at runtime: either the default library that the
// booted app resolved provides the header, or
    ...
    "[gfx][shader][include][openpbr][academy]")
{
  score::test::run_in_app([&](const score::GUIApplicationContext& ctx) {
    ...
    // attributable (framed by the #line marker naming the real file) and
    // non-trivial.
    CHECK(expanded.contains("#line 1 \""));
