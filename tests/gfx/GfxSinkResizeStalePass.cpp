...
// The full application recycles it reliably, but in a small fixture the
// replacement render-pass descriptor usually lands on the very address the
// freed one occupied, so the dangling read gets the right data — and the next
// render frame's deferred rebuild repairs the pipelines anyway, so the PIXELS
// come out right either way. Neither the crash nor the pointer value is
// therefore a dependable pin.
//
    ...
    "[gfx][l3][incremental][resize]")
{
  ...
  // THE PIN. A pass built here would run against the QRhiRenderPassDescriptor
