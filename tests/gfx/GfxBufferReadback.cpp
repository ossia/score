...
//
// Covers the gap documented by csf_geometry_readback_skip_reason() in
// Gfx.hpp: a compute node whose only outputs are Types::Geometry /
// Types::Buffer is reachable from an OutputNode sink
...
//
// Coverage:
//   * a CSF geometry producer's position/color SSBOs read back with the exact
//     float values the shader wrote (closed form below).
//   * the SSBO-counter half: an `instanceHits` storage buffer written by a
//     PER_INSTANCE pass reads back exactly one hit per instance, proving the
//     dispatch was sized from INSTANCE_COUNT.
//
...
// A geometry-ONLY CSF -- no raster consumer, no image output -- must
...
// SSBO-counter half: the instanceHits storage buffer written by
...
// both and that they agree, so that MULTIPLE outputs are read.
TEST_CASE(
    ...
    "[gfx][l3][csf][instancing][readback]")
{
  ...
  // The core assertion: dispatch sized from INSTANCE_COUNT (4), not the
