...
// Gfx::ImageCache::acquire() inserts a new entry at refcount 1 and release()
// does `--count; if(count <= 0) erase`. Insert at 0 instead and the FIRST
// release always evicts, no matter how many holders there are: two Images
// processes listing the same file, one of them removed, and the entry is gone
// while the other still uses it. Nothing crashes — score::gfx::Image copies its
// QImage frames — but the cache stops being a cache exactly in the case it
// exists for, and the next acquire re-decodes from disk.
TEST_CASE(
