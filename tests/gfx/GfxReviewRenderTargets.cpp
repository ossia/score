// Render-target cases. Each one is stated as a contract the engine either
// meets or does not.
//
...
// -----------------------------------------------------------------------------
// 01. The rectangular PER_MIP tail. Qt allocates the chain from max(w,h), so a
// 128x8 texture has 8 levels; a pass count taken from min(w,h) would only cover
// four of them. Read level (count-1) back DIRECTLY -- no sampler in the path,
// which would confound the result.
//
...
// -----------------------------------------------------------------------------
// 02. Same shader as 01 minus the depth attachment, i.e. ONE colour output.
...
// -----------------------------------------------------------------------------
// 03. A LAYERS:2 output rendered through the simple (non-MRT) renderer.
// -----------------------------------------------------------------------------
TEST_CASE(
...
// -----------------------------------------------------------------------------
// 07. A depth-only source feeding two inputs of one consumer, on the same
