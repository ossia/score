...
//
// With no refusal in the fixture, `SCORE_TEST_API=null ctest -R gfx` runs every
// pixel case all the way through create() and render(), and then dies on
...
// zero pixel goes GREEN against a buffer nothing ever drew into.
//
// The contract this file pins, and which score_test/Gfx.hpp implements:
//
...
// readback: Null does not hand back an empty buffer no assertion could be
// fooled by. QRhi's Null backend services readBackTexture with a
// correctly-sized, correctly-strided RGBA8 image: valid() is TRUE, at() works,
// and every one of the 4096 pixels carries ONE value -- whatever Qt happens to
// fill the buffer with, not even the black an unrendered target would suggest.
// Nothing anywhere in the result distinguishes it from a real frame.
//
...
//
// NEGATIVE CONTROL: delete the `api == score::gfx::Null`
    ...
    "[gfx][null][decision]")
{
  ...
  //
  // Worse than "empty": the readback is a perfectly well-formed
