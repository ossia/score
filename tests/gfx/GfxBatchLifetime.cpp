...
// OpenGL/D3D11/Vulkan crash seen in RenderList::render -> resourceUpdate /
// beginPass.
//
// Regression guard. Both cases build a rendered A -> sink graph, then, with no
// render in between: add node B + edge B -> sink incrementally (leaving B's
// material-UBO upload pending in the initial batch), tear B down through the
// same incremental path the app uses, and render again. The pending batch has
// to be submitted before the teardown and the sink must still show A; without
// that the next frame is a heap-use-after-free inside QRhi's
// enqueueResourceUpdates, a hard crash under ASan. Green on OpenGL and Vulkan.
//
    ...
    "[gfx][l3][incremental][batchlife]")
{
  ...
  // The post-mutation frame rendered -- a use-after-free would crash before
  // reaching here -- and still shows A.
  REQUIRE(out.b.valid());
