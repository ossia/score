...
// WHICH one fires per change kind is the whole point:
//
...
// for "a rebuild happened" is the QRhiTextureArray reallocation
// (pointer + layer-count change in the registry), and the assertable
...
// deleteLater()'d old array -- i.e. never setting mustRecreatePasses makes
// the array-realloc case render the old texture.
//
// A STRUCTURAL CAVEAT on the plain-swap FRAME-change leg (case "the frame
// follows a live handle swap"), which bounds what that case proves:
...
// The frame still follows the swap on OpenGL and Vulkan: the
// per-frame dynamic-slot refresh (:3093-3095) carries it. The registry-level
// "which path" case is green either way (the slot IS rerouted and the arrays
// ARE untouched); only the on-screen propagation depends on that refresh. If
// the frame leg ever fails, the fix is product-side: give meshesUnchanged
// (:4497) a dynamic-slot fingerprint term so a slot-map change republishes the
// meshes vector.
//
...
constexpr const char* kFsDyn = R"__(/*{
  "DESCRIPTION": "dynamic-slot probe: quarter i of the screen samples baseColorDyn[i], so WHERE a colour appears identifies WHICH dynamic slot the registry routed the live texture to, and the colour identifies WHICH QRhiTexture is bound there.",
  "CREDIT": "test",
  "ISFVSN": "2.0",
...
constexpr const char* kFsArr = R"__(/*{
  "DESCRIPTION": "bucket-array probe: samples layer 0 of the BaseColor channel's bucket-0 QRhiTextureArray. If the array is reallocated (layer growth) and the engine propagates it, the on-screen colour follows layer 0 of the NEW array; a consumer left bound to the destroyed old array is the negative-control failure.",
  "CREDIT": "test",
  "ISFVSN": "2.0",
  "MODE": "RAW_RASTER_PIPELINE",
  "CATEGORIES": ["TEST-SYNTHETIC", "TEST-SCENE"],
...
TEST_CASE(
    "a live material texture handle swap reroutes the dynamic slot "
...
// contract: the frame changes when the handle changes.
// Static analysis (see the file header) suggests the engine might never
// propagate a dynamic-slot-only change to the drawn frame (meshesUnchanged has
// no dynamic-slot term; the MaterialGPU upload is fingerprint-gated). It
// passes deterministically on OpenGL and Vulkan -- the per-frame dynamic
// refresh (ScenePreprocessorNode.cpp:3093-3095) is sufficient in this scenario.
// The case therefore GATES rather than tolerating a failure: a tolerated
// failure that passes would hide a future regression of exactly this contract.
TEST_CASE(
    "the frame follows a live handle swap without a rebuild",
    "[gfx][scene][material][dynamic-slot][p1-20]")
{
  const auto api = GENERATE(from_range(platform_backends()));
  CAPTURE(backend_name(api));
...
TEST_CASE(
    "a material texture change that grows the channel array "
