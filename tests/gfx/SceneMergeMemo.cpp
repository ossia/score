// =============================================================================
// N producers into one Scene In merge once and memoize.
//
...
// (NodeRenderer.hpp). NodeRenderer::process(port, scene_spec, key)
// stores each delivery in m_portScenes and calls rebuildMergedScene(), which:
//
...
//     (MergeCacheKey in NodeRenderer.hpp). On a signature match it
//     re-publishes the previous merged scene_spec verbatim and returns
//     without merging;
//   * short-circuits a single contributor to a plain pointer copy;
//   * otherwise calls ossia::merge_scenes.
//
...
// a brand-new scene_state and stamps it version = max(inputs)+1
// (geometry_port.cpp). The memo-hit branch instead re-assigns the cached
// scene_spec, i.e. the very same shared_ptr. The test
...
//
// The "producer bumps" mirror what real halp producers do, per the memo-cache
// comment in NodeRenderer.hpp: they keep one stable scene_state shared_ptr and
...
// class-level SCORE_PLUGIN_GFX_EXPORT (NodeRenderer.hpp, Node.hpp), and
...
//   * Delete the memo-hit early-return at NodeRenderer.cpp
...
//     test goes red.
//   * Drop the version half of the memo key at NodeRenderer.cpp --
...
//     "bumping one producer re-merges exactly once" goes red. The idle-frame
//     test stays green under this mutation; only the bump-detection test
//     catches it.
// =============================================================================
...
// the pattern the memo key documents at NodeRenderer.hpp. `tag`
...
// is true, geometry_port.hpp) but environment fields are set. The
// rebuild loop must keep it (NodeRenderer.cpp), and merge_scenes
// overlays its params_ambient group (geometry_port.cpp).
    ...
    "three producers into one Scene In port merge additively", "[gfx][scene]")
{
  ...
  // (NodeRenderer.cpp -- one m_portScenes slot per (port, source)).
  p1.deliver(r);
  ...
  // (geometry_port.cpp), not any producer's own state.
  CHECK(r.scene.state != p1.state);
  CHECK(r.scene.state != p2.state);
  CHECK(r.scene.state != p3.state);

  // merge_scenes stamps version = max(inputs) + 1 (geometry_port.cpp).
  CHECK(r.scene.state->version == 4);

  // The memo signature tracked all three contributors (NodeRenderer.hpp).
  CHECK(r.m_mergeCacheInputs.size() == 3);

  // forEachSceneOnPort enumerates all three slots (NodeRenderer.hpp).
  int seen = 0;
...
TEST_CASE("an idle frame re-runs merge_scenes zero times", "[gfx][scene]")
{
  ...
  // NodeRenderer.cpp). Same {pointer, version} signature => the
  // memo-hit branch at NodeRenderer.cpp must re-publish the SAME
  // scene_state shared_ptr; a re-merge would produce a new allocation
  // (geometry_port.cpp) and this pointer would change.
  ...
  // The cached output is that same state (NodeRenderer.cpp).
...
TEST_CASE("bumping one producer re-merges exactly once", "[gfx][scene]")
{
  ...
  // scene_state::version (geometry_port.hpp) -- the exact producer
  // behavior the {pointer, version} memo key exists for
  // (NodeRenderer.hpp). The signature entry at NodeRenderer.cpp
  ...
  CHECK(contains_root(r.scene, p2.root.get()));
  CHECK(contains_root(r.scene, p3.root.get()));
  // with the recomputed version max(1, 7, 3) + 1 (geometry_port.cpp).
    ...
    "a single contributor short-circuits to a pointer copy", "[gfx][scene]")
{
  ...
  // (NodeRenderer.cpp): the published scene_state is the producer's
  ...
  // produced version max+1 = 6, a fresh state, geometry_port.cpp).
  REQUIRE(r.scene.state);
    ...
    "an env-only producer with empty roots is not dropped", "[gfx][scene]")
{
  ...
  REQUIRE(env.state->empty()); // the property under test, geometry_port.hpp
  ...
  // (NodeRenderer.cpp). Had the env-only producer been dropped,
  // valid.size() would be 1 and the short-circuit at NodeRenderer.cpp
  ...
  // merged scene (params_ambient group, geometry_port.cpp).
