...
// A hardcoded 256 bytes -- enough for the camera UBO (240 B) and most other
// small UBOs -- gives any larger block a buffer that stops short of the members
// the shader reads. Sizing it from `e.size` /
...
// of a 256-byte placeholder. The placeholder is zero-filled, so a correctly
// sized one makes `big.tail.x + big.tail.w` a deterministic 0 and the shader
// emits (0, 255, 64, 255).
//
// With the placeholder forced back to 256 B, OpenGL's out-of-range read does
// not yield the zeros the placeholder promises, so the whole colour flips;
// Vulkan is unaffected, because the out-of-range read yields zeros there.
