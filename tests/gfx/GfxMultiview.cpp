    ...
    "[gfx][l3][multiview][binding]")
{
  ...
  // black (gl_ViewIndex / layered-RT path unsupported headless), so the
  // per-view PIXEL guard is not expressible on GL; only the crash-free-build
  // guard above is. The strong pixel+crash guard runs on Vulkan (and D3D12 on
  // Windows), where an SRB collision is a hard failure and a correct engine
  // renders layer 0 (VIEW_INDEX 0) reddish.
