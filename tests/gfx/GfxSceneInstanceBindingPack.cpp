// =============================================================================
// THE UNIFIED-MDI GEOMETRY MUST FIT QT'S D3D11 VERTEX-BINDING CAP.
//
// QD3D11CommandBuffer::MAX_VERTEX_BUFFER_BINDING_COUNT is 8 and
// QRhiD3D11::setVertexInput warns then CLAMPS to it: a ninth vertex buffer is
// not recorded, the D3D11 input slot stays unbound, and every attribute on it
// reads zeroes. Nothing throws and nothing is skipped -- the draw is wrong.
//
// ScenePreprocessorNode's unified-MDI geometry has six per-vertex streams
// (position, normal, uv0, tangent, color0, uv1) plus the per-instance
// translation, instance_color0 and instance_draw_id. remapPipelineVertexInputs
// (Utils.cpp) compacts the layout down to the bindings an attribute actually
// lands on, which is why most shaders stay well under the cap -- but a shader
// reading every stream lands on all of them and the compaction has nothing
// left to drop. That is the case the helmets document hits, and the case this
// file pins.
//
// The producer packs: translation and instance_color0 are both per-instance
// vec4s stepping at rate 1, so they share ONE 32-byte binding (translation at
// byte 0, color at byte 16) instead of one binding each. Eight bindings, and a
// shader reading everything fits.
//
...
// per-instance bindings for any scene with at least one slot, so the cap is
// reachable from every scene document, not only the instanced ones.
// =============================================================================
    ...
    "[gfx][scene][instancing][d3d11][a24]")
{
  ...
  // is 8; a ninth binding is dropped by QRhiD3D11::setVertexInput and its
  // attributes read zeroes. Without the translation / instance_color0
  // interleave this geometry publishes nine.
