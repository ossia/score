...
// tear the previous one down first. On Vulkan that means a vkCreateDevice plus
// a vkDestroyDevice per click, all on the GUI thread.
//
// SharedVulkanDeviceCache avoids that by keeping one imported VkDevice for the
...
//
// A COUNT ALONE CANNOT PIN THAT. `createdDeviceCount() <= 1` is also what one
// gets when the cache is not used at all: `SCORE_GFX_NO_VKDEVICE_CACHE=1` sends
// every selection down createSharedVulkanDevice() and leaves the counter at
// ZERO, so a count-only assertion passes vacuously while each selection still
// pays for the whole device pair. The regression this file guards is latency,
// not a count.
//
...
// destroyed per selection, which is what every non-preview caller of
// createRenderState relies on) and once Cached.
...
TEST_CASE("Cached previews create one VkDevice for the whole session", "[gfx]")
{
  ...
  //
  // The gate is the OWNED run, not `devicesCreated`. Gating on
  // `devicesCreated == 0` is vacuous for the same reason a count-only assertion
  // is: with SCORE_GFX_NO_VKDEVICE_CACHE=1 the counter stays at zero, so the
  // bypass switches the assertion off instead of tripping it. What actually
  // says "the expensive path is in play on this box" is the owned run's own
  // cost: a machine with no video-decode queue takes the plain QRhi::create
  // fallback and is fast in BOTH halves, so there is no stutter to assert
  // about. Above the standard, the device pair is being paid and the cached
  // half has to avoid it.
  //
  // The comparison itself is a RATIO against a baseline taken moments
  // earlier on the same box, so it holds on a runner of any speed; 0.5 is a
  // wide margin around the ratio a cache that skips the device pair gives,
  // which is an order of magnitude. The 50 ms in the gate is the project's
  ...
  // LIMITATION, not a threshold to tune. Where the device pair is cheap the
  // comparison is noise-dominated: repeated samples of the SAME measurement
  // (the test always drives the Vulkan path, whatever SCORE_TEST_API says)
  // straddle the ratio in both directions, some saying the cache helps and
  // some saying it hurts, while devicesCreated stays at 1 throughout -- so the
  // cache itself is working and there is simply no saving to assert there.
  //
  // Relaxing the constant to fit such a box is not the answer: a value loose
  // enough to let the noise-dominated backends through fits a number to one
  // machine's noise rather than testing score. The exact claim
  ...
  // that means something where the pair dominates.
