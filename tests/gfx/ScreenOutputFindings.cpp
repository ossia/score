...
//     QRhi resources (the ScaledRenderer's pipeline, SRB, samplers, UBOs).
//     Graph::destroyOutputRenderList() fixes the order: release and deregister
//     the RenderList first, destroy the output second. The setters take that
//     order through OutputConfiguration::onReleaseRenderList, the callback the
//     Graph hands every output it initialises.
//
    ...
    "[gfx][window][screen][regression]")
{
  ...
  run_in_gui_app([&](const score::GUIApplicationContext&) {
    ...
    // which must not reach a RenderList whose QRhi is already deleted.
  });
