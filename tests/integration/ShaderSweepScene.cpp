...
//
// That gap closes without an execution engine. oscr::GfxNode<T>, the Crousti
// wrapper turning a halp process into a score::gfx node, needs only a
// ProcessModel -- constructible standalone from duration, id, ctx, parent --
// and a DocumentContext, which score::test::new_document supplies. No interval,
// no executor, no transport. So this file instantiates the real producers and
// wires them into the real score::gfx::ScenePreprocessorNode, which is what
// "SceneFlattener" is; all it lacked was an ossia::scene_spec source.
//
...
void runScene(const score::GUIApplicationContext& ctx)
{
  ...
  // ShaderSweepWired.cpp substitutes csf-storage-rw.cs for the builder. Wire the
  // real one and measure: Threedim::Camera's only outlet is scene_out, an
  // ossia::scene_spec stamped Types::Geometry, while a fullscreen ISF's
  // uniform_input is a Types::Buffer port, and bindUpstreamBuffers only borrows
  // from a Buffer source. If the edge delivers nothing, the frame is unchanged by
  // its presence and the substitution is the only wiring that reaches the binding.
  {
