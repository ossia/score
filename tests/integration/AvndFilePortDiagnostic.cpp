// =============================================================================
// Pointing an avnd raw-file port at a path that does not exist must say so.
//
// `oscr::loadRawfile` (Crousti/File.hpp) returns a null handle for a file it
// cannot open, and all three of its callers are written
//
//     if(auto hdl = loadRawfile(inlet->value(), ctx, has_text, has_mmap)) { ... }
//
// (ExecutorPortSetup.hpp at initialize and on a live control change, and
// GpuUtils.hpp on the GPU path). A missing file therefore produces nothing
...
// scores use the Asset Loader alone. The diagnostic sits at this layer so it
// covers every avnd raw-file port rather than one node.
//
