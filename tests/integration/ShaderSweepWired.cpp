...
// cannot honour that: ShaderSweepRaster wires ONE generic producer
// (csf-vertex-count-expr.cs) into every rasterizer and ShaderSweepISF wires
// nothing at all into every ISF, so a tester that needs a camera UBO, a layered
// texture or a second stage renders its zero-input fallback and the sweep scores
// the silence. That blankness measures the harness, not the renderer.
//
// This file closes the gap for the subset whose prescribed graph is buildable
// out of score::gfx nodes. The upstream types the clauses name — SceneFlattener,
// CameraUBOBuilder, CameraArrayBuilder, FbxLoader, ObjLoader, DirectionalLight,
// Primitive cube, BufferLoader — are all real score processes, but they are
// avnd/halp processes in score_plugin_threedim reached through the Crousti
// GfxNode wrapper and the document/execution layer. The gfx fixture builds
// score::gfx nodes directly (ISFNode + BackgroundNode), so none of the eight is
// constructible here; see the per-group notes below for what each group
// substitutes and what that costs. score::gfx::ScenePreprocessorNode — the
// engine node behind SceneFlattener — IS exported and default-constructible, but
// it consumes an ossia::scene_spec that only a threedim producer can mint, so
// the 14 scene testers stay out of reach.
//
// Two of the assertions here are not "is it blank": a fullscreen tester whose
// output is a single colour by construction (a counter ramp, a camera-derived
// tint) is uniform on every frame no matter what, so uniformity says nothing.
// Those cases assert the frame CHANGES — across a short and a long run for the
...
//!
//! This is not a shortcut around the real builder — ShaderSweepScene.cpp wires
//! an actual oscr::GfxNode<Threedim::Camera> into this very shader's `camera`
//! port and measures the frame as byte-identical with and without it, on both
//! backends. Threedim::Camera's only outlet is an ossia::scene_spec, and
//! bindUpstreamBuffers borrows solely from a Types::Buffer source
//! (IsfBindingsBuilder.cpp:724); the engine's one camera-UBO delivery path is
//! the flattener's name-matched `camera` auxiliary, which only
//! RenderedRawRasterPipelineNode consults. So for a FULLSCREEN ISF there is no
//! wiring that carries camera matrices, and a Buffer producer is the only thing
//! that reaches the binding at all. What these cases therefore validate is the
//! binding path the testers describe ("a UBO sourced from an upstream Buffer
//! port, bound via QRhiShaderResourceBinding::uniformBuffer"), not the matrix
//! contents — which no reachable graph can supply here.
