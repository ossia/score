...
#
# Exit 77 (ctest SKIP) when the gallery binary or a display is unavailable;
# exit 1 when any object crashed on either backend.
set -u
...
# "download the user library?" modal, which is armed by a 1 s single-shot timer
# and so only reaches objects whose render outlives that. Without it the
# resulting QDialog::exec() reads as CRASH(124) against whichever object
# happened to be slow.
#
