...
// the geometry testers declare no OUTPUTS block: their only RESOURCE is
// `"TYPE": "geometry"`, so they write vertex buffers and never a texture.
// textureForOutput() returns null for them and the node warns "No output
// texture available for graphics pass", so that sweep cannot score them.
//
...
namespace
{
//! A frame with no pixels at all is not uniform -- it is a failure, not a pass.
...
//! The .cs corpus splits in two by what a shader writes: some declare a
//! `"TYPE": "geometry"` RESOURCE and drive a rasterizer, the rest declare only
//! images and are read back directly. Wiring an image producer to a raster node
//! yields "geometry producer ... has no Geometry output port", a routing
//! mistake on our side rather than a defect in the shader -- so each sweep takes
//! the half it can answer for.
