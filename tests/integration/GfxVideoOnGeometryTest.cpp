...
//
// The lossless clip JsGraphE2ETest.cpp generates (MovingPattern.hpp, exact-
// equality oracle) is decoded once and fed to TWO sinks in the same document,
// in the same process, on the same render step:
//
...
// is the graph JsGraphE2ETest.cpp already pins byte-exact. Its frame index is
...
// ---------------------------------------------------------------- the shape
//
// Geometry: Threedim::Plane, uuid 1e923d52-3494-49e8-8698-b001405000da. Its
// mesh is vcg::tri::Grid(m, hdivs, vdivs, 1., 1.), whose vertices are
// (j*wl/(w-1), i*hl/(h-1), 0) -- i.e. x,y in [0,1] at z = 0, 16x16 divisions by
// default = 450 triangles. It is a real mesh travelling the real geometry port,
// not a full-screen quad shortcut.
//
// Material: Gfx::RenderPipeline::Model, uuid
// dbfc2101-40d7-4807-8804-571e88992e7e, the "Render Pipeline material"
// branch. Its ports are, in order, Geometry In then whatever the
// shader's INPUTS declare -- so inlet 0 is geometry and inlet 1 is the shader's
// image input, exactly as ThreedimRenderTest.cpp wires it.
//
...
// no second sampler), which tests/gfx/SyntheticFeatures.cpp already exercises
// on every platform backend. Only the two-line VERTEX body is written here, and
// only because the corpus one
...
// mesh's own texcoord attribute: for a Z-facing face Primitive.cpp writes
// uv = (p.x, p.y) verbatim. Declaring `position` as vec4 while the mesh
...
// takes binding, format and offset from the geometry and only the location from
// the shader, resolving the name through findGeometryAttribute ->
// ossia::name_to_semantic.
//
// -------------------------------------------------- why byte-exact is honest
//
// "Byte-exact" here means max per-channel difference 0 over the inner half of
// every one of the 16 pattern blocks, which is the region MovingPattern was
// built to make exactly-representable. The chain, step by step:
//
//  1. NO COLOUR CONVERSION. The clip is rawvideo RGBA muxed into NUT
//     (writeClip below, the same recipe as JsGraphE2ETest.cpp and for the same
//     reason: every other muxer converts and the test would then be asserting
//     ffmpeg's conversion). No YUV, no matrix, no range remap.
//  2. NO TRANSFER-FUNCTION DRIFT. Every palette entry has channels that are
//     only 0 or 255 -- fixed points of every gamma curve, sRGB step and 8-bit
//     rounding a render target can apply. So even if some leg of the pipeline
//     were sRGB-encoded, these sixteen colours survive it.
//  3. NO FILTERING ERROR IN THE INTERIOR. The sampler is built from the port's
//     render-target spec, whose filters default to QRhiSampler::Linear -- so
//     this is bilinear magnification, NOT nearest. That is fine, and it is
//     exactly the property MovingPattern's header claims: a bilinear tap taken
//     at least one texel inside a flat block reads four identical texels, and
//     the weighted mean of four identical 0/255 values is that value in float
//     and again after the 8-bit round trip. The blocks are 40x30 source texels
//     (160x120 / 4x4) and only the inner half is compared, so every sample sits
//     >= 10 texels from a block edge in x and >= 7 in y. Block EDGES are not
//     compared and are not claimed exact.
...
//     flag. The RenderPipeline path does not request them at all.
...
//     flip on SPIRV for exactly this reason. Which orientation THIS path yields
//     on THIS backend is unverified, so the test requires exactness in one
//     orientation and requires that orientation to be the SAME for every grab,
//     and only CHECKs (non-fatally) that it is top-left. The pattern is
...
// grabFrame(n, path) is renderFrames(n) then grabTo(path), and renderFrames
// drives the whole gfx context, so all three readbacks are separated only by a
// handful of milliseconds of rendering. All consumers of one video read the
// SAME decoded frame: VideoNode::update() calls reader.readNextFrame() once per
// gfx frame into a shared VideoFrameShare, and each per-RenderList
// VideoNodeRenderer merely notices that reader.m_currentFrameIdx advanced. So
// the bracket min(refA, refB) <= geo <= max(refA, refB) is a true statement
// about one instant, and at 10 fps (kClipFps) the bracket can only be one frame
// wide unless a snap triple took longer than 100 ms to render -- which the test
// reports when it happens (refA != refB) rather than hiding it.
//
// ---------------------------------------------------------- negative controls
//
// Freezing the texture upload after the first frame (VideoNodeRenderer, the
// `reader_frame > this->m_currentFrameIdx` guard) must redden "geometry frame
// index strictly increasing" and "reference frame index strictly increasing"
// and nothing else: a frozen frame is still a perfectly exact frame of this
// pattern, and a constant index still brackets itself. The same signature is
// available from the producer side with no product edit and no rebuild, by
// running with SCORE_TEST_PATTERN_NEGATIVE=frozen, which mutates the CLIP
// rather than the assertions.
//
// A geometry-leg-only control isolates this test from the decoder: making
// RenderedRawRasterPipelineNode skip the `sampl.texture != tex` rebind so the
// Render Pipeline never binds the video texture into its SRB must redden every
// WinGeo assertion (the readback goes uniform, so `uniform == true` and the run
// fails on "no picture on the geometry") and leave every WinRef assertion green
// -- which is the discrimination the two-window shape exists for.
//
// --------------------------------------------------------------- SKIP policy
//
...
// pixel verdict there would be vacuous (the live-edit-sweep.sh house rule: do
// not fall back to offscreen/Null for a case whose verdict is a pixel; exit
// 77 / SKIP instead). score_plugin_threedim is discovered dynamically at run time, so it
// cannot be gated at configure time: instead the build script exits 77 when
// Score.createProcess returns null for the Plane or the Render Pipeline, and 77
// becomes a Catch2 SKIP here.
//
// ------------------------------------------------------------- recipe notes
//
...
//     only syncs with is_start() and only the scenario's own initial sync is
//     ever marked one -- see GfxNestedIntervalTest.cpp;
...
//     Render Pipeline branch rather than the Model Display branch --
//     framing a Model Display camera means setting two vec3 controls, and that
//     is the API gap ThreedimRenderTest.cpp SKIPs on.
...
//! Gfx::RenderPipeline::Model -- RenderPipeline/Metadata.hpp.
const char* kUuidRenderPipeline = "dbfc2101-40d7-4807-8804-571e88992e7e";
//! Threedim::Plane -- Primitive.hpp.
...
//! of score's decode. Byte-for-byte the recipe JsGraphE2ETest.cpp uses,
...
bool writeClip(const QString& rawPath, const QString& clipPath)
{
  ...
  // `frozen` must turn the two monotonicity checks red and leave every
  // exactness check green.
...
//! and its own texcoord attribute is (x, y), so this maps the mesh onto the
//! whole viewport, axis-aligned and unscaled, with w = 1 (no perspective
//! divide). Kept comment-free: the committed corpus .vs files carry no comments
//! and the ISF vertex preprocessor has never been asked to survive one here.
...
//! basename in the .fs's own directory, so the pair has to live in the same
//! directory -- hence the copy rather than
...
Run runPhased(const QString& js, const std::vector<std::pair<int, QByteArray>>& phases)
{
  ...
  // context and every grab comes back blank (see JsGraphE2ETest.cpp).
#if defined(_WIN32)
...
//! got. MovingPattern::read() only calls top-left a match, by design, and when
//! the picture is flipped returns the top-left reading whose frame index was
//! decoded from the wrong row -- so the index cannot be recovered from it. detail::readAt is the same comparison,
...
//! exact line GfxContext emits under SCORE_GFX_TRACE, parsed the way
//! GfxNestedIntervalTest.cpp and GfxEdgeConsumeLatch.cpp parse it.
    ...
    "[integration][gfx][js][media][threedim]")
{
  ...
  src += "if (!vid) { console.log(\"GEO-SKIP: no Video process factory\"); "
         "Qt.exit(77); }\n";
  // Scale.hpp -- 3 = Stretch, so the 160x120 clip fills the video node's
  // output texture instead of sitting 1:1 in a corner of it.
  src += "vid.scaleMode = 3;\n";
  // Scale.hpp -- 2 = FrameQueue, so the queue renderer is used
  ...
  src += "if (!rp) { console.log(\"GEO-SKIP: no Render Pipeline process "
         "factory\"); Qt.exit(77); }\n";
  // Inlet 0 is Geometry In; the shader's INPUTS follow, so "tex" is inlet 1.
  // Ask by name first so a future extra port cannot silently shift the index.
  ...
  // defect. Two are tolerated; the run is 900 ms in
  ...
  // -- 1. The REFERENCE leg is the graph JsGraphE2ETest.cpp already pins
  // byte-exact, top-left. If this half is red the run says nothing about
