...
//
// All top-level identifiers use `var`, not const/let: QML's JS engine scopes
// const/let inside eval(), so the outer script could not see them.
...
// Deliberately not `function tick_final() {}`: each scenario eval()s this file
// as its first statement and declares its own tick_final() later, and that
// declaration is hoisted before any statement runs, so a function declaration
// here would be created by the eval afterwards and overwrite it. Assigning to a
// var instead runs at eval time and loses to the hoisted declaration.
