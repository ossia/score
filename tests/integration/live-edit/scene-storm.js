// Scenario 11 — scene-storm.
//
// Registered in tests/integration/live-edit-sweep.sh (CFG / EXPECT / ORDER),
// which enumerates scenarios explicitly rather than by glob. It runs 20 ticks
// = two full 10-step cycles; nticks MUST stay a multiple of 10 or the last
// cycle's teardown never runs and the magenta gate fails. The final tick
// leaves only the full-screen isf-solid-color base, so the >=99%-magenta gate
// applies -- not min_nonblack_coverage, which is for device-fed scenarios. No
// precondition() entry is needed: this scenario uses no camera and no NDI,
// only the Window device.
//
...
// the gap: SceneGraphOps/SceneGraphFilterTest/MaterialOverrideTest are all CPU
...
// retargeted at a nonexistent .fbx): per the asset-loader failure contract,
...
function step(n) {
    switch(n % 10) {
        case 0:
            ...
            g_asset = mk(UUID_ASSET, MISSING_GLB); // publishes nothing
            g_inst  = mk(UUID_INST, "");
        ...
        case 7:
            ...
            // path of the nothing-published contract, mid-playback.
