...
//
// PR #2179 "Survive a build that does not have every plug-in" owns this
// problem, and solves it far more broadly -- ports, layers, document plug-ins,
// device protocols, network peers and curve segments, where this covers the
// process and its cables. Its own pin is a UNIT test over
// deserialize_interface: it checks the uuid, the port COUNTS and a
// byte-identical payload round trip, each stronger than this in its own way --
// but it never builds a document, never attaches a cable, and so cannot see a
// cable dropped because its endpoint vanished, which is the reported symptom.
// An independent check written against someone else's implementation is
// worth more than one written alongside it.
//
// So: this goes GREEN when #2179 lands, and the [!shouldfail] tag comes off in
// the same commit. Until then loadMissing is `SCORE_TODO; return nullptr;` and
// the case fails by DETECTING THE LOSS rather than by crashing --
// `CHECK(int(interval.processes.size()) == process_count)` reads 2 == 3.
// =============================================================================
// A document that names a process this build does not have must come back
// WITH that process, its ports and its cables.
//
// THE DEFECT. `Process::ProcessFactoryList::loadMissing` is
//     SCORE_TODO; return nullptr;
// and its caller (IntervalModelSerialization, both the DataStream and the JSON
// path) is `if(proc) ... else SCORE_TODO;`. So a document saved with an add-on
// the current build does not have comes back with the process GONE — and, since
// its ports go with it, every cable attached to it gone too — while the
// document still reports as having loaded successfully. The next save makes the
