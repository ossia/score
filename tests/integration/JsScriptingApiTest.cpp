...
// did.
//
// Six calls get a case of their own below, so a failure names the call
// directly. Everything else is in the sweep, a plain regression guard: it
...
// Hand EditJsContext::createDevice settings that are not an object -- a
// number, a string -- and neither the canConvert<Device::DeviceSettings> nor
// the canConvert<QVariantMap> branch runs. The missing-protocol guard must
// therefore reject the call before LoadDevice is submitted, or
// DeviceDocumentPlugin::createDeviceFromNode calls makeDevice() on a null
// factory.
//
// The same call with `{}` returns cleanly: it is the argument SHAPE, not the
// unknown uuid, that this case is about.
TEST_CASE(
...
// Device::DeviceList::device(), whose only failure mode is a SCORE_ASSERT --
// a throwing assert nothing catches, so the process terminates. Naming a
// device that does not exist is an ordinary scripting
...
// EditJsContext::zoom reaches doc->document.view()->viewDelegate(). With
// --no-gui there is no DocumentView at all, so the view has to be tested
...
// EditJsContext::scroll, the same shape as zoom().
TEST_CASE(
...
// EditJsContext::libraryEntries reads the library out of a
// Library::ProcessPanel, and --no-gui instantiates no panels: the lookup has
// to be the null-returning findPanel<>(), not ApplicationComponents::panel<>(),
// which ends in SCORE_ABORT. Score.availableProcesses(), which reads the
...
// EditJsContext::availableProcessesAndPresets, the same panel lookup.
TEST_CASE(
