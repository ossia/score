...
//
//     Score.startState(scenario)
//     Score.createIntervalAfter(state, dur_flicks, y)
//     Score.createProcess(box, UUID_ISF, shader)
//
// A FLOATING BOX NEVER EXECUTES -- the reason this test does NOT use
// Score.createBox. `Macro::createBox` mints a brand new start TimeSync at the
// requested date via CreateTimeSync_Event_State. Only the scenario's own
// initial sync is ever marked a start point (ScenarioModel's
// setStartPoint(true); nothing else in src/ calls it), and
// `ossia::scenario::get_roots()` collects ONLY syncs with `is_start()`. A box
// whose start sync is not a start point is unreachable and never executes.
// Editing only `StartState` in the saved document so the box hangs off the
// scenario's start state (id 0) is what makes it publish "GFX-EDGES publish 1"
// and render. Corpus documents that hold a gfx process inside a sub-interval
// chain that interval from the scenario start, the topology this test
// reproduces. (The same floating-box construction comes
// out of DropLayerInScenario, DropPresetInScenario, DropPortInScenario and
// ScenarioEditor, so a file dropped into empty scenario space is silently
// non-playing; a product question, recorded, not asserted here.)
//
...
//              baseline for an offscreen window is "renders nothing":
...
//              "grabTo: nothing rendered into" (WindowDevice.cpp).
...
//              PNG, the warning again. A sink that kept compositing the
//              stopped process would happily write a magenta after.png.
//
...
// at the decision point, inside the edges_changed.exchange(false) block.
// full=1 routes through recompute_connections(), full=0 through
// incrementalEdgeUpdate. The child's merged log is split at MARK-BEFORE /
// MARK-DURING / MARK-AFTER lines the script prints right after each grab;
// both stderr streams interleave in wall-clock order, so the t1 consume lands
// in the BEFORE..DURING segment and the t2 consume in the DURING..AFTER
// segment. Each transition segment must contain at least one consume line,
// every one of them full=0, and the edge-set sizes must grow at t1 (new >
// old) and shrink at t2 (new < old).
//
// Determinism: renderFrames()/grabFrame() step the RENDER side on demand
// (FrameDeterminismTest.cpp proves the step clock), so every grab reads back
// a frame that was synchronously rendered by the grab itself -- no vsync
// race. The TIMELINE side (interval start/stop) is driven by the execution
// engine on the dummy audio backend's own thread, so the transitions happen
// in wall time; the grabs sit >= 1 s away from t1 and t2 on either side, and
// the colours are constants, so timeline jitter cannot change any pixel
// value, only which phase a grab lands in.
//
// WHY THE PHASES ARE INJECTED OVER OSC and not busy-waited in one --script: a
// JS busy-wait (`while (Date.now() - t0 < ms) {}`) blocks the application's
...
// main-thread queues. With the loop blocked, the box starts on the audio
// thread's clock but its gfx edge never materialises: every grab (which DOES
// pump the render side synchronously) reports "no process is connected to
...
// with in-script busy-waits. So this test drives the phases the way
...
// SCORE_GFX_TRACE fprintf, inside the `if(edges_changed.exchange(false))`
// block. Both transition consumes then print full=1 and take
// recompute_connections(); a full rebuild still renders the correct picture,
// so the three colour phases stay green and only the trace assertions go red.
//
    ...
    const QString& js, const std::vector<std::pair<int, QByteArray>>& phases)
{
  ...
  // Makes GfxContext print its edge-consume decisions.
...
//! in order -- the line GfxContext::updateGraph emits, parsed the way
    ...
    "[integration][gfx][js][scenario]")
{
  ...
  // that holds the process. Required, not cosmetic -- see the "floating box"
  // note in the header.
  ...
  // stale magenta after.png is the "did not stop cleanly" failure.
  ...
  // set grew at t1 and shrank at t2. A full rebuild (full=1) is the regression
