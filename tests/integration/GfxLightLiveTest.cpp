...
// see tests/integration/GfxNestedIntervalTest.cpp:12-34.
...
//   `.name = "scene_counts"`.
// * The layout is SceneCountsUBO field-for-field, with a static_assert on
//   sizeof == 16.
// * "storage" is mandated by the product's own comment in
//   ScenePreprocessorNode.cpp: QRhi forbids Dynamic+StorageBuffer and
...
//   documented mechanism, per RenderedRawRasterPipelineNode.cpp --
...
//   `SceneCountsUBO sc{(uint32_t)m_cachedLightIndices.size(), ...}` in
//   ScenePreprocessorNode.cpp, fed from freshLightIndices.
//
...
// shader's own LAYOUT (`aux_declared_size` in
// RenderedRawRasterPipelineNode.cpp), and a zero placeholder has
// draw_count == 0 as well as
...
// a zeroed vector, documented at ThreedimRenderTest.cpp:644-654.
//
...
// (WindowDevice.cpp), i.e. the render side is stepped synchronously by the
// grab itself (FrameDeterminismTest.cpp proves the step clock), so no grab
// races vsync. The probe reads no TIME and no
...
// inside its `if(edges_changed.exchange(false))` block; full=1 routes through
// recompute_connections() and full=0 through incrementalEdgeUpdate. The
// child's merged log is
...
// count: it sizes the buffer, is diff-uploaded on the mesh-unchanged fast path,
// uploaded on the structural path, moved into m_cachedLightIndices, and
// `light_count` is literally `(uint32_t)m_cachedLightIndices.size()`. Clearing
// it pins light_count at 0 on both paths.
//
...
//           incrementally.
//
// AN OBVIOUS-LOOKING ALTERNATIVE CONTROL IS WRONG AND POINTS THE OTHER WAY.
// Making `scene_light_indices` include the sentinel `0xFFFFFFFF` slot looks
// like it should make B stop differing from A.
// Read against the code, it cannot do that:
//   1. The sentinel is pushed ONLY for producer-less lights.
//      In SceneGPUState.cpp the arena slot is
...
//   2. A Threedim::Light ALWAYS owns a slot. Light::init allocates from
//      Arena::RawLight unconditionally on first init and stamps the resulting
//      ref onto the emitted light_component's raw_slot. So this test's light is
//      never the sentinel, and in this scene `fs.lightArenaSlots` contains no
//      sentinel at all -- dropping the sentinel filter is a literal NO-OP here.
//      A control that cannot change the measured value is not a control.
//   3. Even in a scene that did have a producer-less light, admitting the
//      sentinel ADDS an entry and INCREMENTS light_count -- it makes B differ
//      from A more, not less, and additionally makes shaders index
//      `scene_lights.entries[0xFFFFFFFF]` out of the arena. It is a corruption
//      knob, not an erasure knob.
...
// ThreedimRenderTest.cpp was in ModelDisplayNode, which this chain does not
// use. The Vulkan leg is reasoned from that, not measured here.
//
...
//! LAYOUT is SceneCountsUBO field-for-field and the name is what the
//! preprocessor publishes. "storage" is required, not stylistic -- see
//! ScenePreprocessorNode.cpp.
constexpr const char* kProbeFS = R"__(/*{
  "DESCRIPTION": "probe: encodes the ScenePreprocessor's scene_counts into flat colour. R = fract(gl_FragCoord.x/256) coverage/identity plane, G = draw_count > 0 (proves scene_counts resolved to real data rather than a zero placeholder), B = light_count > 0 (the quantity under test). Reads no TIME, so two renders of the same scene state are byte-identical.",
  "CREDIT": "test",
  "ISFVSN": "2.0",
    ...
    const std::vector<std::pair<int, QByteArray>>& phases)
{
  ...
  // Per-run shader/PSO cache. A negative control can stay green because the app
  // replays the previously compiled pipeline out of XDG_CACHE_HOME (see
  // ThreedimRenderTest.cpp:360-365); isolate it so a product edit is actually
  // recompiled.
...
//! in order -- the exact line GfxContext.cpp emits, parsed the way
...
//! thread and the MARK lines from the main thread's console.log, so which of
//! the two reaches the shared stderr pipe first is a coin flip -- the drop's
//! "publish 3" can land one line BEFORE "MARK-DROP", which makes a
//! segment-local shrink test read old=3 new=3 while the edge set has in fact
//! gone 3 -> 4 -> 3. The published SEQUENCE carries the same information and
//! does not depend on log interleaving at all.
    ...
    "[integration][gfx][js][threedim][scene]")
{
  ...
  // grabTo refused ("nothing rendered into", WindowDevice.cpp), i.e.
  ...
  // (RenderedRawRasterPipelineNode.cpp): draw_count > 0, so G == 255
  ...
  // one -- which is why it is separate from the pixel half above.
  {
    ...
    // (see publishedSizes), so a segment-local test can see only "old=3 new=3".
    // The sequence of distinct published edge-set sizes over the whole run must
    // be exactly
