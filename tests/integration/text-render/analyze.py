...
be created -- most of the assertions below passed. A uniform frame has a
...
is treated as a failed grab for every assertion downstream:
  ...
  * text cannot fill the frame. The ceiling is 25 % coverage, well above what
    even the heaviest case draws.
...
# Structural contract (see the module docstring): every frame must contain
# background black, and no case may cover more than a small part of the frame.
BG_LUMA_MAX = 8.0
MAX_COVER = 0.25
...
def main():
    ...
    # Hard assertion: an untouched Text process must render visible text. The
    # UBO default position is {0,0} = screen centre (Gfx/Graph/TextNode.hpp); a
    # non-zero default shifts the quad in clip space and pushes the default text
    ...
    # size: it must be visible, which proves the default text/font/size do
    # propagate and that the position default alone causes a blank frame.
    ...
    # merely bright in red and green: checking only those two channels certifies
    # a (255,255,0) fill as white.
    ...
    # "present" means the structural contract held: tofu boxes and a blank
