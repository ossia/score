...
#
# Qt's offscreen integration exposes OpenGL only through GLX, so with DISPLAY
# unset there is no GL at all: QRhi falls back to the Null backend, which fills
# every texture with Qt::yellow (qrhinull.cpp), and every "rendered" frame is a
# solid (255,255,0) rectangle. So: a real X server (our own Xvfb when we can
# have one), QT_QPA_PLATFORM=xcb, and __GLX_VENDOR_LIBRARY_NAME=mesa alongside
# LIBGL_ALWAYS_SOFTWARE=1 -- without the vendor pin libglvnd hands us the NVIDIA
# GPU and LIBGL_ALWAYS_SOFTWARE is silently ignored.
#
# The backend is asserted, not assumed: the run is failed outright if QRhi
...
# process-default string and only repositions it on screen, isolating the
# position default). tofu/empty precede base-again so the last case proves
# clean recovery.
...
# "default" is not a golden of its own: its frame is byte-identical to
# default-pos0 (both are the process defaults at position (0,0)), so a second
# copy would be graded twice and mean once. The hard default-visible assertion
# in analyze.py is what guards it.
...
# all we SKIP rather than degrade quietly into Null-backend frames.
XVFB_PID=""
DISP=""
...
# single OUT_DIR constant. Point it at the directory this script uses, or under
# ctest -- which sets OUT into the build tree -- the readiness marker lands
# somewhere nobody is watching and the run dies at the readiness gate.
