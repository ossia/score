...
// answers.
//
// One TEST_CASE, deliberately: JS::ProcessModel::rootPath() caches the Library
// settings model in a function-local `static const auto&`, so the second score
// application built in a process reads the first one's freed settings. See
// JsRootPathStaticTest.cpp -- until that is fixed, a JS process may only be
// created under the first run_in_gui_app of a binary.
...
TEST_CASE("The UI script of a Javascript process", "[integration][js][gui][ui]")
{
  score::test::run_in_gui_app([](const score::GUIApplicationContext& ctx) {
    ...
    // invalid Connection. Signals inherited from Process::ProcessModel
    // (score_lib_process, one copy) do connect, so the error paths below are
    // asserted on state rather than on signals.
    int flags_changed = 0;
    QObject::connect(
