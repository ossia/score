...
//
// Everything below the device layer is covered elsewhere: PipewireRoundtrip's
...
// in the application, and the interesting case is the one that harness cannot
// express -- score asking for the geometry its own enumerator writes into every
// device path (1920x1080 RGBA 30fps) from a source that publishes something
// else. Advertise that single triple and nothing more and the link goes to
// `state error` / "no more input formats": no frame ever arrives while the
// device still reports itself connected.
//
