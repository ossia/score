// Opening the custom UI of a Javascript process must report that it is open.
//
// Process::setupExternalUI emits Process::ProcessModel::externalUIVisible(true)
// once it has created and placed the window, and false once closeProcessUI()
// has torn it down; the LV2, VST and VST3 windows also emit true from their own
// show path.
//
// What reads that signal is the state of the two controls that open the UI: the
// process-header button in EffectLayer.cpp and the inspector toggle in
// ProcessInspectorWidgetDelegateFactory.cpp. With no `true` they stay in the
// closed state while the window is open, so the next click tries to open it
