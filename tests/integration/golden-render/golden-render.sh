...
# Copying it used to leak the developer's live settings into every reference
# image: Samples=4 in particular made every run log
#   "buildPipeline: RT sampleCount=1 differs from renderer.samples()=4"
# so the refs depended on whatever the developer last clicked. Only the library
# root is inherited, because the corpus resolves "<LIBRARY>:/packages/...".
CFG="$OUT/config-home"
