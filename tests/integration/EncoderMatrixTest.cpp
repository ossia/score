// GPU video encoder matrix test. Instantiates every encoder in
// Gfx/Graph/encoders on a real QRhi backend (SCORE_TEST_API=opengl|vulkan,
// default OpenGL) and checks the readback bytes against hand-computed
// expectations:
//   - UYVY / YUY2 packed byte order (U,Y0,V,Y1 vs Y0,U,Y1,V)
//   - BGRA swizzles including ARGB
...
//   - v210 field placement and the padded wire row at width % 6 != 0
//   - I420 / NV12 plane sizes and U/V placement
...
//   - WireEncoderFactory mapping and wireComputeSupports width gating
//   - ColorSpaceOut shader generation branches and BT.709 limited-range
...
// 8-bit: Y_red=54, U_red=98, V_red=255, chroma at 0.5 = 127|128.
// 10-bit: Y_red=217, U_red=394, V_red=1023, chroma at 0.5 = 511|512.
//
// The encoders' flip_y converts the render graph's Y-up content to video
// top-down order, and a texture uploaded through QRhi goes through the same
// flip, so readback row r is uploaded row (H-1-r) on OpenGL and Vulkan alike.
