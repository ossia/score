...
// it -- but a --script run, the console panel and a .mjs module import nothing
// at all.
//
// Two halves, and the second is the control. "headless" is the case under
// test. "with QtQuick imported" runs the SAME script in a process where a QML
// file has imported QtQuick first -- the state a JS process leaves behind --
// and has to stay green: red there would mean the registration path that
// already works had been broken instead.
//
// Not covered by the value types: Qt.rect(), Qt.point() and Qt.size() return
// QRectF/QPointF/QSizeF directly and QtQml registers those itself; they are
// asserted below to keep that distinction honest. Qt.font() and the array form
// of Qt.matrix4x4() check the result and throw rather than returning a
...
//
// v3str prints the vector the way a script would: an unregistered value type
// shows up as "QVariant(QVector3D, QVector3D(0, 0, 0))".
