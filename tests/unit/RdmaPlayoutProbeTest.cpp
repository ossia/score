...
//
// The AJA RDMA-GL / RDMA-Vulkan output shims probed the playout P2P path with
// `card->DMAWriteFrame(...)` and believed its return code. A dropped
// peer-to-peer read returns success, so the rung engages, the card plays out
// whatever its framestore already held, and the pump reports a full frame count
// with zero drops. Only comparing the delivered bytes against the bytes that
// were handed over catches it.
//
