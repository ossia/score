...
//
// Probing the playout P2P path with `card->DMAWriteFrame(...)` and believing
// its return code is not enough: a dropped peer-to-peer read returns success,
// so the rung engages, the card plays out whatever its framestore already held,
// and the pump reports a full frame count with zero drops. Only comparing the
// delivered bytes against the bytes that were handed over catches it.
//
