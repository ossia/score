...
// Adopting whatever buffer the upstream published makes vkUpdateDescriptorSets
// reject the write with VUID-VkWriteDescriptorSet-descriptorType-00330 and the
// next setShaderResources() segfault; on OpenGL there are no descriptor sets,
// so nothing catches the mismatch and the shader reads whatever the binding
// holds.
//
    ...
    "[gfx][isf][binding][regression]")
{
  ...
  SECTION("a storage-only upstream keeps the placeholder")
  {
    ...
    // Binding ssbo.get() here would write a storage buffer into a
