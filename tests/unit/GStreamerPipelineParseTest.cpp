...
// string.
//
// A live source has no negotiated caps when the pipeline is built, so the
// classification has to come from the pipeline string: defaulting every
// unnegotiated appsink to 640x480 RGBA video turns an audio appsink into a
// video one.
