...
//     (Gfx/Graph/TextureLoader), the central CPU decode used by material
//     textures and AssetTable consumers
//   - Threedim::ImageLoader (LDR), the file-port process() decode contract;
//     its GPU-side init/update needs a full RenderList and is covered by
//     AssetTableTest's Null-RHI TextureLoader tests
//   - Threedim::ArrayToTexture, float-array to texture-bytes for every format
//
// Format matrix asserted by value: PNG8 / PNG16 / JPEG / BMP / TIFF8 / TIFF16 /
// WebP lossless / GIF / TGA 2.0 both origins / PPM / PGM / SVG / ICO. Formats
// whose Qt plugin may be absent are SKIP-guarded on
// QImageReader::supportedImageFormats.
//
// Behaviours asserted:
//  * decode canonicalizes to QImage::Format_RGBA8888: R,G,B,A in memory
//    regardless of endianness, straight alpha, top-left origin, no flip
//  * 16-bit sources are decoded at RGBA64 by Qt and then quantized to 8 bits,
//    which is the LDR contract; the quantization must round, not truncate
//  * sRGB is metadata-only here: decode never alters pixel bytes, and the sRGB
//    decision is taken at upload time via the QRhiTexture flag
//  * corrupt, truncated and garbage inputs fail as std::nullopt without
//    crashing, with two deliberate exceptions: a truncated BMP decodes to a
//    full-size image because Qt pads missing rows, and a PNG payload with a
//    lying ".jpg" extension still loads from a path because QImage sniffs
//    content, while decodeImageFromMemory with an explicit wrong hint fails
//  * Qt's TGA handler only accepts files bearing the "TRUEVISION-XFILE."
//    footer; both origins normalize to top-left and BGRA swizzles to RGBA
...
TEST_CASE("ImageLoader: decode does not linearize sRGB pixel bytes", "[gfx][imageloader]")
{
  ...
  // bytes untouched. A mid-gray 128 would come back ~55 if the loader
  // linearized on load.
  ensureApp();
          ...
          "[gfx][imageloader]")
{
  ...
  for(const char* fmt : fmts)
  {
    DYNAMIC_SECTION("format " << fmt)
    {
      ...
      // silently lost before score's own code runs.
