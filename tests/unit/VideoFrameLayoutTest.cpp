...
//
// Every layout below is owned by av_image_fill_arrays() rather than a
// hand-written switch, so:
...
// No format is mis-laid-out at any of the geometries below, so these exception
// lists are empty. They stay so that a format that starts failing is named by
// "no format outside the known-broken set is mis-laid-out" rather than only
...
// A 2x2 frame is "aligned" for 4:2:0 but not for 4:1:0 / 4:1:1, the smallest
// geometry at which a round-down stride comes out as ZERO.
    ...
    "[video][layout]")
{
  ...
  // nothing here depends on rounding.
...
TEST_CASE(
    ...
    "[video][layout][odd]")
{
  // FINDINGS 4-6. A subsampled stride or plane offset that rounds down leaves a
  // 65x33 or 63x31 frame a column short in chroma and, for 4:2:0 and 4:1:0,
  // starts plane 2 inside plane 1.
