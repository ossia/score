// Unit tests for the threedim 3D-asset loading layer: Threedim::ObjFromString
// (TinyObj.cpp), sceneStateFromMeshes, GltfParser, FbxParser and AssetLoader
// dispatch with AssetLoaderRegistry. The parsers are synchronous static entry
// points, so nothing here needs a Qt event loop, an RHI or an application
// context.
//
// glTF / GLB / FBX fixtures are generated in a temp dir by the test itself: a
// single triangle with known positions, normals, UVs and indices, base64-
// embedded for .gltf and a binary chunk for .glb. OBJ fixtures come from
// ~/Documents/ModelsOBJ (cube.obj 8 verts / 12 tris, sphere20.obj 252 / 500,
// both without vn normals) and those sections SKIP when the corpus is absent.
//
// Loader behaviours asserted below:
//  * OBJ expands to an unindexed triangle soup -- 12 tris become 36 vertices,
//    index_format::none -- so the vertex count is faces*3, not the "v" count.
//  * An OBJ without "vn" yields no normal attribute; the OBJ path generates
//    none.
//  * glTF keeps indexed geometry, widening indices to uint32.
//  * A glTF with normals and UVs but no TANGENT gets tangents from mikktspace;
//    without normals or UVs nothing is generated.
//  * FBX generates missing normals.
//  * Corrupt, truncated or OOB-index inputs return a null result without
//    crashing.
...
// No HOME: an empty path, so the corpus cases SKIP. Never guess a home
// directory -- a hardcoded one sends every other machine looking in somebody
// else's tree.
static const fs::path models_dir = [] {
  const char* home = ::getenv("HOME");
  return home ? fs::path{home} / "Documents" / "ModelsOBJ" : fs::path{};
}();
...
// A hostile .gltf whose accessor declares `count` far beyond its bufferView's
// byteLength drives a heap-buffer-overflow read in fastgltf::iterateAccessor.
// fastgltf::validate() checks the accessor's bufferView index, alignment and
// count >= 1, but never
//   accessor.byteOffset + count * elementSize <= bufferView.byteLength
// GltfParser.cpp sweeps every accessor's data range against its bufferView and
// the buffer's loaded bytes before iterating anything. This case must load-fail
// cleanly with no ASAN report.
...
// ===========================================================================
// AssetLoader point-cloud dispatch and the format-override stage.
//
// `.ply` sniffs ply_is_splat_shaped() and forks between the miniply mesh path
// and the PrimitiveCloud path; `.splat` and `.spz` are new suffixes; and a
// rebuild_format_state() stage sits between parse and wrap, splitting the
// loader's state into m_parsed_state -> m_overridden_state -> m_wrapped_state.
// The parsers underneath are covered by PrimitiveCloudTest.
// ===========================================================================
