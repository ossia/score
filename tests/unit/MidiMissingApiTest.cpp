// =============================================================================
// Loading a document whose MIDI device names an API this build does not have
// must NOT abort the application.
//
// THE HAZARD. In Protocols/MIDI/MIDIDevice.cpp's makeInputConfiguration, a
//     case libremidi::API::ALSA_SEQ: {
//       auto ptr = get_if<libremidi::alsa_seq::input_configuration>(&api_conf);
//       SCORE_ASSERT(ptr);
// cannot hold: libremidi::midi_in_configuration_for(api) only fills the variant
// for a backend that was COMPILED IN (libremidi.cpp -> midi_any::for_backend,
// which walks available_backends). On a build without ALSA — macOS, Windows,
// WASM, or any Linux build made without it — the variant holds
// unspecified_configuration, get_if returns null, and opening a Linux-authored
// document ABORTS. The KEYBOARD case has the same shape.
//
...
//
// THE BEHAVIOUR CHOSEN, and why. reconnect() checks
    ...
    "[unit][midi][protocols]")
{
  ...
  // The direct pin: given settings naming a backend that is not compiled in,
  // building the configuration must come back, not abort. Driven directly
  // because whether
  ...
  // reachable in the field but not reliably in a test rig.
  {
    ...
    // The generic configuration must come back rather than the ALSA one.
  ...
  score::test::run_in_app([&](const score::GUIApplicationContext& ctx) {
    ...
    // DeviceDocumentPlugin::loadDevice -> MIDIDevice::reconnect); it must not
    // abort the process.
