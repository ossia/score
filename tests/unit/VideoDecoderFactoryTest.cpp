...
TEST_CASE("Packed RGB decoder parameterisation", "[gfx][video][decoderfactory]")
{
  ...
  {
    auto* p = as<PackedDecoder>(make(AV_PIX_FMT_RGB0, "processed.r *= 2.0;"));
    REQUIRE(p != nullptr);
...
{
  {
    auto* p = as<PlanarDecoder>(make(AV_PIX_FMT_GBRP));
    REQUIRE(p != nullptr);
  ...
  {
    auto* p = as<PlanarDecoder>(make(AV_PIX_FMT_GBRAP));
    REQUIRE(p != nullptr);
