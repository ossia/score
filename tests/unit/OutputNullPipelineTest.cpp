...
// ScaledRenderer::finishFrame must not carry that null into
//
//     cb.setGraphicsPipeline(m_p.pipeline);
//
// QRhiCommandBuffer::setGraphicsPipeline is
...
//
// It also must not be a function-local static: that defers ~QGuiApplication to
// __run_exit_handlers, by which time the thread-local storage
// QFontCache::cleanup() reaches for is gone, and the process segfaults at exit
// *after* Catch2 has printed a green report. A listener owns it for exactly the
// length of the run and destroys it while Qt is still whole.
