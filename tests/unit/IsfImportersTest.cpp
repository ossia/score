// Unit tests for the libisf importer branches:
...
//
// The cases marked REGRESSION below pin importer defects that produced
// shaders which do not compile; each names the failure mode it guards.
...
TEST_CASE("shadertoy: autodetected from mainImage signature", "[isf][shadertoy]")
{
  ...
  CHECK(contains(frag, "#version 460"));
  ...
  CHECK(contains(vert, "#version 460"));
...
{
  // REGRESSION: parse_shadertoy_json()'s compat block references TIME /
  // RENDERSIZE / isf_process_uniforms / isf_FragCoord / isf_FragColor, so it
  // must prepend GLSL45.versionPrelude / fragmentPrelude / defaultUniforms the
  // way parse_shadertoy() does, or the emitted fragment does not compile
  // stand-alone.
  parser p{{}, shadertoy_json, 450, parser::ShaderType::Autodetect};
  const auto frag = p.fragment();
...
TEST_CASE(
    ...
    "[isf][glslsandbox]")
{
  // REGRESSION: the compat header prepends `uniform float TIME;`, so a
  // textual replacement that also rewrites the source's own
  // `uniform float time;` yields a second `uniform float TIME;` -- a
  // redeclaration error at GLSL compile time.
...
{
  // REGRESSION: a plain replace_all("time" -> "TIME") also rewrites
  // identifiers *containing* the word, e.g. `lifetime` -> `lifeTIME` and
...
{
  // REGRESSION: a trailing-comma cleanup in the PASSES emission loop that
  // does `oss.str(fixed_string)` on a plain std::ostringstream resets the
  // write position to the *beginning* of the buffer, so every subsequent
  // write overwrites the start of the document and any descriptor with a
  // non-empty PASSES array comes out corrupted.
  const std::string src = R"_(/*{
  "ISFVSN": "2",
  "INPUTS": [ { "NAME": "inputImage", "TYPE": "image" } ],
  "PASSES": [
...
TEST_CASE(
    ...
    "[isf][errors]")
{
  // REGRESSION: a constructor that moves `vert` into m_sourceVertex and
  // *then* evaluates `vert.empty()` reads the moved-from string, so for any
  // vertex source longer than the SSO buffer the flag comes out true even
  // though a custom vertex shader was provided.
    ...
    "[isf][rawraster][clipcull]")
{
  ...
  // Never as bare globals: gl_ClipDistance / gl_CullDistance are already
  // declared in gl_PerVertex, so a bare global is a redeclaration that changes
  // their qualification and glslang rejects it outright.
