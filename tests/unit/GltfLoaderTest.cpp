// glTF and GLB load — CPU loader half only (the render half lives
// elsewhere). Covers glTF/GLB against a real-world asset.
//
...
fs::path assets_dir()
{
  // No HOME: return an empty path so the corpus cases SKIP. Never guess a
  // home directory -- a hardcoded one sends every other machine looking in
  // somebody else's tree.
  const char* home = ::getenv("HOME");
  if(!home)
    return {};
  return fs::path{home} / "ossia" / "threedim-assets";
}
