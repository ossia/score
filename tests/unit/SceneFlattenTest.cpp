...
TEST_CASE("flattenScene: hasCamera is the only reliable signal on the legacy mirror", "[scene][flatten]")
{
  ...
  // clear() resets the containers and the two flags but NOT viewMatrix /
  // projectionMatrix / cameraPosition / cameraFov / cameraNear / cameraFar,
  // and the empty-scene path returns before the fallback block would rewrite
  // them. A reused FlatScene therefore keeps last scene's camera on it.
  flattenScene(ossia::scene_spec{}, out, 1.f);
  CHECK_FALSE(out.hasCamera);
  CHECK(out.activeCameraIndex == -1);
  CHECK(out.cameraPosition.z() == Approx(50.f));
  CHECK(out.cameraNear == Approx(2.f));
}
