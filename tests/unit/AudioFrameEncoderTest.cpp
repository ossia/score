...
{
  // -1.0 is the endpoint every integer form converts exactly; pinned here
  // alongside the positive-endpoint cases below.
  auto v = channels({{-1.f}});
...
TEST_CASE(
    ...
    "[gfx][libav][audioenc]")
{
  // The scale must not be (float)INT32_MAX: that rounds UP to 2147483648.0f
  // -- one past the largest representable int32 -- so converting 1.0f is out of
  // range and cvttss2si returns INT32_MIN, turning a full-scale positive peak
  // into a full-scale NEGATIVE one in every AV_SAMPLE_FMT_S32 / S32P stream,
  // which is what libav's 24- and 32-bit PCM encoders take.
  auto v = channels({{1.f}});
...
TEST_CASE(
    ...
    "[gfx][libav][audioenc]")
{
  // int24_max must not be INT32_MAX / 256.0 == 8388607.99609375: as a float
  // that rounds UP to 2^23, so 1.0 quantises to 8388608 -- one past the largest
  // signed 24-bit value, reading back as -8388608. The same polarity flip as
  // the S32 case above, one bit depth down.
  auto v = channels({{1.f}});
  TestFrame fr{AV_SAMPLE_FMT_S32, 1, 1};
  S24IAudioFrameEncoder{1}.add_frame(*fr.f, v);
