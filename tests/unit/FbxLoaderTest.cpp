// `FBX loads` — the ufbx path yields
...
//   (b) a byte-for-byte synthesized FBX 7400 *binary* container, asserted to
//       decode to the same streams as the equivalent ASCII document;
...
//
// Entry point under test is Threedim::FbxParser::ins::fbx_t::process(), which
// AssetLoader dispatches `.fbx` to. It sets generate_missing_normals and
// normalize_normals, target axes +X/+Y/+Z with target_unit_meters = 1.0 and
// space_conversion = ADJUST_TRANSFORMS, then walks the ufbx scene. The FBX path
// is UNINDEXED — vertex_count = num_triangles * 3, no index buffer at all;
// texcoords go through verbatim with no V flip; tangents come from mikktspace
// when normals and UVs are both present and no TANGENT channel is; the
// attribute order is position, normal, texcoord0, color0, tangent, with absent
// streams skipped. The ufbx synthetic root is skipped and its children become
// scene_state::roots, each node emitting its local transform payload before its
// mesh_component.
//
...
// optional and SKIPs with the exact command to satisfy it.
//
// Unverified: this file assumes a little-endian host when writing the binary
// FBX container. Not exercised on a big-endian target.
...
// apply-lambda (empty on rejection); applying it to a fresh parser stores the
// scene in m_raw_state.
//
// FbxParser hands `tv.filename.data()` straight to `ufbx_load_file`, which
// needs NUL termination — GltfParser copies the view into a
// std::filesystem::path first. Every caller here (and AssetLoader's) backs the
// view with a std::string, so the byte after the view is the string's NUL.
// Flagged, not exercised: a caller passing a substring view would read past the
// end.
// ---------------------------------------------------------------------------
...
// FbxParser emits one root scene_node per ufbx root child. Look them up by name
// so no assertion depends on the order ufbx happens to walk the Connections
// block in.
...
// dedicated, tightly-packed buffer, so this is also the buffer's byte_size
// divided by vertex_count.
...
{
  // Triangle list, never indexed: the extraction emits three unshared vertices
  // per triangle, so the vertex count is always a multiple of 3 and there is no
  // index buffer at all — the structural difference from the glTF path, which
  // does index.
  ...
  // One buffer per present attribute, appended in lockstep, byte_offset always
  // 0, buffer_index the running counter.
  ...
  // Position is unconditional (ScenePart in FbxParser.hpp: "always present").
  ...
  if(nrm)
  {
    // FbxParser divides by the length, so every emitted normal is unit — or
    // the (0,1,0) fallback, which is also unit.
  ...
  // Normals + UVs and no TANGENT channel in the file means mikktspace runs, so
  // a primitive that has both must have tangents.
  if(nrm && uv)
    CHECK(tan != nullptr);
  if(tan)
  {
    ...
    for(uint32_t v = 0; v < prim.vertex_count; v++)
    {
      ...
      // (m_setTSpaceBasic, wired in TangentUtils.hpp), so this is an exact
...
//    opts.target_unit_meters = 1.0 with
//    space_conversion = ADJUST_TRANSFORMS, fold a 0.01
...
//    opts.target_axes, so no axis conversion is
...
//    exactly representable in binary64 AND binary32, so the double -> float
//    narrowing is lossless and the comparisons below are `==`.
//
// The three meshes:
    ...
    "[threedim][fbx][loader]")
{
  ...
  // ufbx's synthetic root has three children, each becoming a root scene_node.
  REQUIRE(state.roots);
  CHECK(state.roots->size() == 3u);
  // No material is declared or connected, so register_material is only ever
  // called with nullptr and the published list is empty rather than absent.
  REQUIRE(state.materials);
  CHECK(state.materials->empty());
  // No skin deformer anywhere -> no skeleton published.
  ...
  SECTION("TriMesh: one triangular face -> 3 vertices, all four streams")
  {
    ...
    // FbxParser pushes the local transform before the mesh. The file gives Lcl
    // Translation 0,0,0 and no rotation or scaling; UnitScaleFactor 100 makes
    // the unit conversion factor exactly 1 (see the fixture note), and the
    // target axes match the file's, so the TRS is exactly identity. This pins
    // the target_axes / target_unit_meters / space_conversion options.
    ...
    // position + normal + texcoord0 + tangent. Tangent is not in the file; it
    // is synthesized because normals and UVs are both present. No colours are
    // declared, so color0 is absent and the tangent lands at buffer_index 3,
    // not 4.
    ...
    // AABB over exactly those nine floats, via
    // ossia::compute_aabb_from_positions.
    ...
    // re-normalization multiplies by exactly 1.
    const auto* nrm = find_attr(prim, ossia::attribute_semantic::normal);
    REQUIRE(nrm);
    const float* n = attr_floats(prim, *nrm);
    for(int v = 0; v < 3; v++)
    ...
    // UVs: the file's UV array verbatim, no V flip — uv.x / uv.y are copied
    // straight through.
    const auto* uv = find_attr(prim, ossia::attribute_semantic::texcoord0);
    REQUIRE(uv);
  ...
  SECTION("QuadMesh: one quad face triangulates to 2 triangles -> 6 vertices")
  {
    ...
    // A convex n-gon triangulates to n - 2 triangles: 4 - 2 = 2, times 3
    // vertices each.
    ...
    for(int v = 0; v < 6; v++)
    {
      ...
      // through the SAME triangulated index, i.e. that the two streams are not
      // desynchronized.
  ...
  SECTION("BareMesh: no normals in the file, so ufbx generates the face normal")
  {
    ...
    // Positions only in the file, and no UVs means the mikktspace branch
    // cannot run, so exactly two attributes.
...
// std::abort() is the "contract violated" signal the forked parent reads off
// waitpid().
    ...
    "[threedim][fbx][malformed][truncation]")
{
  ...
  // tests/unit/AssetLoaderFailure.cpp and GltfLoaderTest.cpp's
  // glb_truncation_matrix.
  ...
  SECTION("ASCII 7500 scene: prefixes may parse, but never publish a lie")
  {
    ...
    // ufbx is right to accept it. The contract asserted is therefore the honest
    // one: no crash, and any scene that does come out must have every buffer it
    // advertises. This fixture has LayerElementNormal and LayerElementUV
    // arrays, so prefixes land in the middle of the normal and UV payloads.
    truncation_matrix(
...
{
  // A small .fbx in the corpus would be ideal; there is none, so this
  // leg is optional and SKIPs with the exact command. The named file is a
  // Blender 2.72 export of the default cube that ships inside the vendored ufbx
  // submodule's own sample set, so satisfying this needs no network.
  ...
  // This export carries no UV set, so the mikktspace branch does not fire:
  // position + normal only. The invariant
