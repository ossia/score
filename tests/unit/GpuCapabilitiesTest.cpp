...
TEST_CASE("NVIDIA path exclusion", "[gfx][interop][gpucaps]")
{
  ...
  // Load-bearing: an unidentified GPU must still attempt the NVIDIA paths and
  // degrade on init failure. Flipping this to `true` would silently disable DVP
  // wherever the vendor probe comes back empty -- Mesa's GL driver among them.
