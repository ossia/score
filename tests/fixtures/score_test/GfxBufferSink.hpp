...
// =============================================================================
// L3 GPU buffer-readback sink for score's gfx render engine.
// =============================================================================
//
// WHY THIS EXISTS
//
...
// `input->type == Types::Buffer || Geometry || Scene` branch) —
// runInitialPasses is where RenderedCSFNode dispatches its compute passes and
// pushes its output geometry downstream. The only other headless-constructible
// sink in <score_test/Gfx.hpp>, BackgroundNode, consumes a Types::Image, so
// without this one a geometry-only CSF never enters a RenderList at all: the
// gap csf_geometry_readback_skip_reason() describes and tests/gfx/
// CsfGeometry.cpp skips on.
//
...
public:
  ...
  {
    std::string name;
    // NOT QRhiReadbackResult: on the Qt CI builds against (6.4.2) the buffer
    // readback result is its own type, and readBackBuffer takes a
    // QRhiBufferReadbackResult*; only later Qt folds it into
    // QRhiReadbackResult. Gfx/Graph/RenderState.hpp carries the project's
    // compatibility alias for the newer Qt, and EncoderMatrixTest.cpp /
    // ExtractComputeSrb.cpp spell it this way too.
