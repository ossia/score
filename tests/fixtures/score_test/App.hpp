...
inline void prepare_test_environment(bool headless)
{
  ...
  // a static in the app that no test can reach. Without this, on macOS every
  // test touching the OpenGL backend runs on Apple's LEGACY profile -- GL 2.1 /
  // GLSL 1.20 -- instead of the 4.1 core the app itself runs on. There QRhi has
  // no texture arrays and no 3D textures, RenderList creation throws, and no
  // window ever presents a frame.
  //
