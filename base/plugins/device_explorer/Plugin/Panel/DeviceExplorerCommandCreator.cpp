...
QModelIndex DeviceExplorerCommandCreator::cut(const QModelIndex &index)
{
    ...
    cmd->set(m_model->pathFromIndex(index.parent()), index.row(),
...
QModelIndex DeviceExplorerCommandCreator::moveUp(const QModelIndex &index)
{
    ...
    Move* cmd = new Move;
    cmd->set(m_model->pathFromNode(*parent), oldRow, 1,
             m_model->pathFromNode(*parent), newRow,
    ...
    Path path = m_model->pathFromNode(*n);
    path.back()--;
...
QModelIndex DeviceExplorerCommandCreator::moveDown(const QModelIndex &index)
{
    ...
    Move* cmd = new Move;
    cmd->set(m_model->pathFromNode(*parent), oldRow, 1,
             m_model->pathFromNode(*parent), newRow + 1,
...
QModelIndex DeviceExplorerCommandCreator::promote(const QModelIndex &index)
{
    ...
    Move* cmd = new Move;
    Path parentPath = m_model->pathFromNode(*parent);
    cmd->set(parentPath, row, 1,
             m_model->pathFromNode(*grandParent), rowParent + 1,
...
QModelIndex DeviceExplorerCommandCreator::demote(const QModelIndex &index)
{
    ...
    Move* cmd = new Move;
    Path newPath = m_model->pathFromNode(*sibling);
    cmd->set(m_model->pathFromNode(*parent), row, 1,
