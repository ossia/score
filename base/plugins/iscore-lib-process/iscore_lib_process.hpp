 ...
 * Examples of processes are Automation::ProcessModel, Scenario::ProcessModel, Mapping::ProcessModel, etc.
 *
 * The corresponding class in the OSSIA API is ossia::time_process.
 * It is only used for execution.
 *
 * The main class is Process::ProcessModel : it contains all the data necessary to create, save, reload
 ...
 * except for very specific cases : for instance, the execution percentage, since
 * it is not part of the save.
 *
 * See for instance the addon tutorial (<https://github.com/OSSIA/iscore-addon-tutorial>).
 *
 * The UI works as follows :
 * * Process::ProcessModel is the base class for processes.
 * * Process::LayerModel is the "view model" : a process may have multiple different view;
 ...
 *
 *
 * The class Process::StateProcessModel is similar, but instead of being temporal,
 * it is instantaneous and goes into instances of Scenario::StateModel.
 *
 * This plug-in also contains :
 * * Styling information for the central view of i-score, with the Process::ScenarioStyle class.
