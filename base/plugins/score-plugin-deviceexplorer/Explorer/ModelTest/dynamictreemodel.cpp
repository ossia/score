...
QModelIndex
DynamicTreeModel::index(int row, int column, const QModelIndex& parent) const
{
  //   if (column != 0)
  //     return QModelIndex();
  ...
  QList<QList<qint64>> childIdColumns
      = m_childItems.value(parent.internalId());
  ...
  if (grandParent >= 0)
  {
    ...
    {
      qFatal(
          "%s: parent.column() must be less than parentTable.size()",
          Q_FUNC_INFO);
    }
    ...
    {
      qFatal(
          "%s: parent.row() must be less than parentSiblings.size()",
          Q_FUNC_INFO);
    }
  }
...
ModelChangeCommand::ModelChangeCommand(
    DynamicTreeModel* model, QObject* parent)
    : QObject(parent)
    , m_model(model)
    , m_numCols(1)
    , m_startRow(-1)
...
ModelInsertCommand::ModelInsertCommand(
    DynamicTreeModel* model, QObject* parent)
...
void ModelMoveCommand::doCommand()
{
  ...
  {
    QList<qint64> l
        = m_model->m_childItems.value(srcParent.internalId())[column].mid(
            m_startRow, m_endRow - m_startRow + 1);

    for (int i = m_startRow; i <= m_endRow; i++)
    {
      m_model->m_childItems[srcParent.internalId()][column].removeAt(
          m_startRow);
    }
...
ModelChangeChildrenLayoutsCommand::ModelChangeChildrenLayoutsCommand(
    DynamicTreeModel* model, QObject* parent)
    : ModelChangeCommand(model, parent)
{
}
...
void ModelChangeChildrenLayoutsCommand::doCommand()
{
  ...
  // If we're changing one of the parent indexes, we need to ensure that we do
  // that before
  // changing any children of that parent. The reason is that we're keeping
  // parent1 and parent2
  ...
  Q_FOREACH (const QModelIndex& idx, persistent)
  {
    if (idx.parent() == parent1)
    {
      ...
      {
        m_model->changePersistentIndex(
            idx, m_model->createIndex(0, idx.column(), idx.internalPointer()));
      }
      else
      {
        m_model->changePersistentIndex(
            idx, m_model->createIndex(
                     idx.row() + 1, idx.column(), idx.internalPointer()));
      }
    }
    else if (idx.parent() == parent2)
    {
      if (idx.row() == 0)
      {
        m_model->changePersistentIndex(
            idx, m_model->createIndex(
                     rowSize2 - 1, idx.column(), idx.internalPointer()));
      }
      else
      {
        m_model->changePersistentIndex(
