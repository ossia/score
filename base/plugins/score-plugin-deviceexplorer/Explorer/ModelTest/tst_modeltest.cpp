...
void tst_ModelTest::treeWidgetModel()
{
  ...
  QTreeWidgetItem* parent
      = new QTreeWidgetItem(&widget, QStringList("parent"));
...
class AccessibleProxyModel : public QSortFilterProxyModel
{
  Q_OBJECT
public:
  AccessibleProxyModel(QObject* parent = nullptr)
...
public Q_SLOTS:
  ...
  void storePersistent()
  {
    ...
    Q_FOREACH (const QModelIndex& idx, m_persistentProxyIndexes)
      ...
      {
        qWarning(
            "%s: persistentProxyIndexes contains invalid index", Q_FUNC_INFO);
        ++storePersistentFailureCount;
      }
    ...
    {
      qWarning(
          "%s: proxy should have no persistent indexes when storePersistent "
          "called",
          Q_FUNC_INFO);
      ++storePersistentFailureCount;
    }
    ...
    {
      qWarning(
          "%s: proxy should have persistent index after storePersistent "
          "called",
          Q_FUNC_INFO);
      ++storePersistentFailureCount;
    }
