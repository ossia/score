    ...
    : QObject(parent), model(_model), fetchingMore(false)
{
  ...
  connect(
      ...
      SLOT(runAllTests()));
  connect(
      model, SIGNAL(columnsInserted(QModelIndex, int, int)), this,
      SLOT(runAllTests()));
  connect(
      model, SIGNAL(columnsRemoved(QModelIndex, int, int)), this,
      SLOT(runAllTests()));
  connect(
  ...
  connect(
      ...
      SLOT(runAllTests()));
  connect(
      model, SIGNAL(rowsInserted(QModelIndex, int, int)), this,
      SLOT(runAllTests()));
  connect(
      model, SIGNAL(rowsRemoved(QModelIndex, int, int)), this,
      SLOT(runAllTests()));
...
/*!
   ...
   all)
    to make sure the model doesn't outright segfault, testing the functions
   that makes sense.
*/
...
/*!
   ...
   work.
    If they have a bug it will point it out, but the above tests should have
   already
...
void ModelTest::checkChildren(const QModelIndex& parent, int currentDepth)
{
  ...
  for (int r = 0; r < rows; ++r)
  {
    ...
    for (int c = 0; c < columns; ++c)
    {
      ...
      // If the next test fails here is some somewhat useful debug you play
      // with.
    ...
    Qt::Orientation orientation, int start, int end)
{
  ...
  QVERIFY(start <= end);
  int itemCount
      = orientation == Qt::Vertical ? model->rowCount() : model->columnCount();
