...
using SetControlValue = Process::SetControlValue;
struct FloatSlider
{
    template <typename T>
...
struct LogFloatSlider
{
        ...
        QObject* context)
    {
      ...
      QObject::connect(
          sl, &score::DoubleSlider::sliderReleased, context, [=, &inlet, &ctx] {
            ctx.dispatcher.submitCommand<SetControlValue>(
              inlet, from01(min, max, ((QSlider*)sl)->value() / score::DoubleSlider::max));
            ctx.dispatcher.commit();
            sl->moving = false;
          });
...
struct IntSlider
{
        ...
        QObject* context)
    {
      ...
      QObject::connect(sl, &QSlider::sliderReleased, context, [sl, &inlet, &ctx] {
...
struct Button
{
        ...
        QObject* context)
    {
      ...
      QObject::connect(
          sl, &QPushButton::clicked, context, [&inlet] {
