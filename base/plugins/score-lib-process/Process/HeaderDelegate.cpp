...
{
  static const QImage gl{makeGlyphs("I:", ScenarioStyle::instance().GrayTextPen)};
  return gl;
}
static const QImage& toGlyphGray()
{
  static const QImage gl{makeGlyphs("O:", ScenarioStyle::instance().GrayTextPen)};
  return gl;
}
static const QImage& fromGlyphWhite()
{
  static const QImage gl{makeGlyphs("I:", ScenarioStyle::instance().IntervalHeaderTextPen)};
  return gl;
}
static const QImage& toGlyphWhite()
{
  static const QImage gl{makeGlyphs("O:", ScenarioStyle::instance().IntervalHeaderTextPen)};
  return gl;
}
...
void DefaultHeaderDelegate::paint(QPainter* painter, const QStyleOptionGraphicsItem* option, QWidget* widget)
{
  ...
  if(w > minPortWidth())
  {
    if(m_inPorts.empty())
    {
      ...
      if(m_sel)
      {
        painter->drawImage(QPointF{ 8. + m_line.width(), textY() }, toGlyphWhite());
      }
      else
      {
        painter->drawImage(QPointF{ 8. + m_line.width(), textY() }, toGlyphGray());
      }
    }
    else
    {
      ...
      if(m_sel)
      {
        ...
        painter->drawImage(QPointF{startText + 8. + m_line.width(), textY() }, toGlyphWhite());
      }
      else
      {
        painter->drawImage(QPointF{4., textY()}, fromGlyphGray());
        painter->drawImage(QPointF{startText + 8. + m_line.width(), textY() }, toGlyphGray());
      }
    }
  }
}
