    ...
    const ProcessModel& scenar)
{
  ...
  {
    auto create_itv
        = new CreateInterval{scenar, startState->id(), endState->id()};
    disp.submitCommand(create_itv);
    model = &scenar.interval(create_itv->createdInterval());
  }
  else if (startState)
  {
    auto create_itv = new CreateInterval_State_Event_TimeSync{
        scenar, startState->id(), endDate, topY};
    disp.submitCommand(create_itv);
    model = &scenar.interval(create_itv->createdInterval());
  }
  else if (endState)
  {
    ...
    auto create_itv = new CreateInterval{scenar, create_start->createdState(),
                                         endState->id()};
    disp.submitCommand(create_itv);
    model = &scenar.interval(create_itv->createdInterval());
  }
  else
  {
    ...
    disp.submitCommand(create_start);
    auto create_itv = new CreateInterval_State_Event_TimeSync{
        scenar, create_start->createdState(), endDate, topY};
    disp.submitCommand(create_itv);
    model = &scenar.interval(create_itv->createdInterval());
  }
