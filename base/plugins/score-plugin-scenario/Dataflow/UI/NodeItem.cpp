...
PortItem::PortItem(const score::DocumentContext& ctx, Type t, std::size_t idx,  NodeItem& node)
...
void PortItem::mouseReleaseEvent(QMouseEvent* ev)
{
  ...
  if(item && item != this)
  {
    SCORE_ASSERT(&item->m_node.node != &this->m_node.node);
    ...
    {
      SCORE_ASSERT(cd.sink != cd.source);
...
NodeItem::NodeItem(const score::DocumentContext& ctx, Process::Node& p)
  : node{p}
  , m_ctx{ctx}
  ...
  , m_depOut{ctx, PortItem::DependencyOutlet, 0, *this}
{
  ...
  QObject::connect(&node, &Process::Node::outletsChanged,
                   this, &NodeItem::recreate);
  QObject::connect(&node.metadata(), &score::ModelMetadata::NameChanged,
                   this, &NodeItem::recreate);
