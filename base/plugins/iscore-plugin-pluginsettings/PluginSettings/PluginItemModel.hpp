...
struct RemoteAddon
{
  ...
  UuidKey<iscore::Addon> key; // Can be the same as plug-in's
...
struct AddonVectorWrapper
{
  ...
  {
    bool operator()(const iscore::Addon& e)
    {
      return !e.corePlugin;
    }
  };
  ...
  using iterator = boost::
      filter_iterator<not_core_addon, std::vector<iscore::Addon>::iterator>;
  using const_iterator = boost::
      filter_iterator<not_core_addon, std::vector<iscore::Addon>::const_iterator>;
...
public:
  LocalPluginItemModel(const std::vector<iscore::Addon>& vec);
...
public:
  ...
  void updateAddon(UuidKey<iscore::Addon> k, Fun f)
  {
    if (auto add = addon(k))
    {
      beginResetModel();
