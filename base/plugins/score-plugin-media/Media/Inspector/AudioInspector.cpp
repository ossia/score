    ...
    , m_count{this}
{
  ...
  con(process(), &Input::ProcessModel::startChannelChanged, this,
      [&] { m_start.setValue(object.startChannel()); });
  con(process(), &Input::ProcessModel::numChannelChanged, this,
      [&] { m_count.setValue(object.numChannel()); });

  con(m_start, &QSpinBox::editingFinished, this, [&]() {
    m_dispatcher.submitCommand(
        new Commands::ChangeInputStart(object, m_start.value()));
  });
  con(m_count, &QSpinBox::editingFinished, this, [&]() {
    m_dispatcher.submitCommand(
        new Commands::ChangeInputNum(object, m_count.value()));
  });
