...
EffectComponent::~EffectComponent()
{
    ...
    {
        std::get<2>(tpl)->about_to_be_deleted.disconnect<EffectComponent,
&EffectComponent::on_nodeDeleted>(this);
    }
...
void EffectComponent::recreate()
{
    ...
    for(EffectParameter parameter :
MediaEffectParameterAdaptor<InParameter>{fx})
    {
        ...
        param_node->about_to_be_deleted.connect<EffectComponent,
&EffectComponent::on_nodeDeleted>(this); auto param_addr =
param_node->create_parameter(ossia::val_type::FLOAT);
        param_addr->set_access(ossia::access_mode::BI);
        param_addr->set_domain(ossia::make_domain(float{parameter.min},
float{parameter.max})); if(!str_label.empty())
...
{ if(val.getType() != ossia::val_type::FLOAT) return; auto fx =
this->effect().effect(); if(!fx) return;
        ...
        m_inAddresses.push_back(std::make_tuple(parameter.id, param_addr,
param_node));
    ...
    {
        for(EffectParameter parameter :
MediaEffectParameterAdaptor<OutParameter>{fx})
        {
            auto idx = parameter.label.lastIndexOf('/');
            if(idx != -1)
            {
...
float{parameter.max})); if(!str_label.empty())
            ...
            param_addr->push_value(float{GetControlOutValue(fx,
parameter.id)}); m_outAddresses.push_back(std::make_tuple(parameter.id,
param_addr, param_node));
        }
        ...
        if(GetControlOutCount(fx) > 0)
        {
            lv2_fx->effectContext.on_outControlsChanged = [&] {
                ...
                {
                    std::get<1>(p)->push_value(float{GetControlOutValue(fx,
std::get<0>(p))});
                }
            };
        }
