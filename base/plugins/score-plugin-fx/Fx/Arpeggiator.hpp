...
struct Node
{
    ...
    static void run(
        ...
        float spacing, float tempo, int quantif, int duration,
        ossia::midi_port& out,
        ...
        State& self)
    {
      ...
      if(msgs.size() > 0)
      {
        ...
        {
          self.notesMap[i] = self.notes.container[i % self.notes.size()] + std::floor(i / self.notes.size()) * 12;
        }
      }
      ...
      auto whole_samples = (240.f / tempo) * st.sampleRate();
      {
        ...
        auto next = period * std::floor(1 + tk.prev_date / period); // date de la prochaine note qui doit être envoyée
        if(next < tk.date)
        {
        ...
        else
        {
          ...
          if(next < tk.date)
          {
            ...
            out.messages.push_back(rtmidi::message::note_off(1, self.lastnote, 64));
          }
        }
      }
    }
