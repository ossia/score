...
{
    // getSegmentId *has* to be prior to filterSegments for middleBegin / middleEnd
...
void PenCommandObject::release_n(seg_tuple&& segts_tpl)
{
    ...
    { // Put the first one
        ...
        if(lin_segments.size() > 1) lin_segments[1].previous = lin.id;
...
std::tuple<
    ...
    optional<SegmentData>,
    std::vector<SegmentData>> PenCommandObject::filterSegments()
{
    auto x = m_state->currentPoint.x();
    if(x < m_minPress.x()) m_minPress = m_state->currentPoint;
    if(x > m_maxPress.x()) m_maxPress = m_state->currentPoint;
