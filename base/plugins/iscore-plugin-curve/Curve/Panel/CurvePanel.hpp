    ...
    public:
            ...
            m_sm{m_context, *m_curvePresenter}
        {
            ...
            connect(m_view, &ProcessGraphicsView::sizeChanged,
                    this,   &CurvePanelProxy::on_sizeChanged);
            ...
            connect(m_zoomSlider, &iscore::DoubleSlider::valueChanged,
                    this,         &CurvePanelProxy::on_zoomChanged);
        ...
        void on_curveChanged()
        {
            ...
            {
                auto x = point->pos().x();
                if(x > m_currentCurveMax) m_currentCurveMax = x;
            }
        }
        ...
        void on_zoomChanged(ZoomRatio newzoom)
        {
            ...
            m_zoomRatio = mapZoom(1.0 - newzoom,
                                  1.5,
                                  std::max(5., 2 * duration / m_width));

            recompute();
        }

        auto& toolPalette() { return m_sm; }
    private:
        void recompute()
        {
