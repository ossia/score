...
{
  return "application/x-iscore-nodelist";
}
inline constexpr const char* addressettings()
{
  return "application/x-iscore-address-settings";
}
}
}
    ...
    : public MimeDataWriter
{
  ...
  {
    auto obj
        = QJsonDocument::fromJson(m_mime.data(iscore::mime::addressettings()))
              .object();
...
struct Visitor<Writer<Mime<Device::FreeNodeList>>> : public MimeDataWriter
{
  ...
  auto deserialize()
  {
    ...
    auto arr = QJsonDocument::fromJson(m_mime.data(iscore::mime::nodelist()))
                   .array();
