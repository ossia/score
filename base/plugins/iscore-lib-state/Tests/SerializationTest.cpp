...
class SerializationTest: public QObject
{
    ...
    private slots:
        ...
        void ossia_domain_serialization_test()
        {
            ...
            {
                ossia::net::domain d = ossia::net::make_domain(ossia::Vec2f{0, 0}, ossia::Vec2f{1, 1});
                QCOMPARE(unmarshall<ossia::net::domain>(marshall<JSONObject>(d)), d);
                QCOMPARE(unmarshall<ossia::net::domain>(marshall<DataStream>(d)), d);
            }

            {
                ossia::net::domain d = ossia::net::make_domain(ossia::Vec3f{0, 0, 0}, ossia::Vec3f{1, 1, 1});
                QCOMPARE(unmarshall<ossia::net::domain>(marshall<JSONObject>(d)), d);
                QCOMPARE(unmarshall<ossia::net::domain>(marshall<DataStream>(d)), d);
            }

            {
                ossia::net::domain d = ossia::net::make_domain(ossia::Vec4f{0, 0, 0, 0}, ossia::Vec4f{1, 1, 1, 1});
                QCOMPARE(unmarshall<ossia::net::domain>(marshall<JSONObject>(d)), d);
                QCOMPARE(unmarshall<ossia::net::domain>(marshall<DataStream>(d)), d);
            }
