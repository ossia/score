...
TypeAndValueWidget::TypeAndValueWidget(State::Value init, QWidget* parent)
    : QWidget{parent}
...
void TypeAndValueWidget::on_typeChanged(State::Value val, State::ValueType t)
{
  ...
  switch (t)
  {
    ...
    case State::ValueType::Int:
      m_val->setWidget(
          new NumericValueWidget<int>(State::convert::value<int>(val), this));
      break;
    case State::ValueType::Float:
      m_val->setWidget(new NumericValueWidget<float>(
          State::convert::value<float>(val), this));
      break;
    case State::ValueType::Bool:
      m_val->setWidget(
          new BoolValueWidget(State::convert::value<bool>(val), this));
      break;
    case State::ValueType::String:
      m_val->setWidget(new StringValueWidget(
          State::convert::value<std::string>(val), this));
      break;
    case State::ValueType::Char:
      // TODO here a bug might be introduced : everywhere the char are utf8
      // while here it's latin1.
      m_val->setWidget(new CharValueWidget(
          State::convert::value<QChar>(val).toLatin1(), this));
      break;
