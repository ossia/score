...
convert(const State::Value& orig, State::Value& toConvert);
...
textualType(const State::Value& val); // For JSONValue serialization
...
ISCORE_LIB_STATE_EXPORT QString
prettyType(const State::Value& val); // For display to the user, translated
ISCORE_LIB_STATE_EXPORT QString
    prettyType(State::ValueType); // For display to the user, translated
...
ISCORE_LIB_STATE_EXPORT const
    std::array<std::pair<QString, State::ValueType>, 10>&
    ValuePrettyTypesMap();
}
}
