...
namespace Scenario
{
MoveSlotTool::MoveSlotTool(const ToolPalette& sm) : m_sm{sm}
{
  ...
  // Two states : one for moving the content of the slot, one for resizing with
  // the handle.
  {
    auto dragSlot = new DragSlotState<ToolPalette>{
        m_sm.context().context.commandStack, m_sm, m_sm.scene(), &m_localSM};
    // Enter the state
    iscore::make_transition<ClickOnSlotOverlay_Transition>(
        m_waitState, dragSlot, *dragSlot);

    dragSlot->addTransition(dragSlot, finishedState(), m_waitState);
  }

  {
    auto resizeSlot = new ResizeSlotState<Scenario::ProcessModel, ToolPalette>{
        m_sm.context().context.commandStack, m_sm, &m_localSM};
    iscore::make_transition<ClickOnSlotHandle_Transition>(
        m_waitState, resizeSlot, *resizeSlot);
