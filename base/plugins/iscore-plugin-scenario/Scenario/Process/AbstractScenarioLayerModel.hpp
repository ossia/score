...
class AbstractScenarioLayer : public Process::LayerModel, public Nano::Observer
{
  Q_OBJECT
public:
  ...
  // Access to elements
  // A given constraint can be represented only once in a given scenario VM,
  // hence...
  ConstraintViewModel&
  constraint(const Id<ConstraintModel>& constraintModelid) const;
  ConstraintViewModel&
  constraint(const Id<ConstraintViewModel>& constraintViewModelid) const;
...
protected:
  AbstractScenarioLayer(
      ...
      QObject* parent)
      : LayerModel{viewModelId, name, sharedProcess, parent}
  {
  }

  // Copy
  AbstractScenarioLayer(
      const AbstractScenarioLayer& source,
      const Id<LayerModel>& viewModelId,
      const QString& name,
      Process::ProcessModel& sharedProcess,
      QObject* parent)
  ...
  {
    // No data to save (the constraints vector will be rebuilt by the subclass
    // accordingly.)
  }
...
typename T::constraint_layer_type& constraintViewModel(
    const T& scenarioViewModel, const Id<ConstraintViewModel>& cvm_id)
{
  return static_cast<typename T::constraint_layer_type&>(
      scenarioViewModel.constraint(cvm_id));
}

template <typename T>
QVector<typename T::constraint_layer_type*>
constraintsViewModels(const T& scenarioViewModel)
{
...
// Note : the view models can also be more easily accessed using the viewModels
// methods of ConstraintModel
