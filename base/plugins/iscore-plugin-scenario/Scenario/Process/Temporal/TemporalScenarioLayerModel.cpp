...
TemporalScenarioLayer::TemporalScenarioLayer(
    ...
    QObject* parent)
    : AbstractScenarioLayer{viewModelId, "TemporalScenarioLayer", model,
                            parent}
{
...
TemporalScenarioLayer::TemporalScenarioLayer(
    ...
    QObject* parent)
    : AbstractScenarioLayer{source, id, "TemporalScenarioLayer", newScenario,
                            parent}
{
  for (TemporalConstraintViewModel* src_constraint :
       constraintsViewModels(source))
  {
    // OPTMIZEME (inside addConstraintViewModel)
    addConstraintViewModel(src_constraint->clone(
        src_constraint->id(),
        newScenario.constraints.at(
            Id<ConstraintModel>{src_constraint->model().id()}),
        this));
  }
}
    ...
    const Id<ConstraintViewModel>& constraintViewModelId)
{
  ...
  auto constraint_view_model
      = constraint_model.makeConstraintViewModel<constraint_layer_type>(
          constraintViewModelId, this);

  addConstraintViewModel(constraint_view_model);
}

void TemporalScenarioLayer::addConstraintViewModel(
    constraint_layer_type* constraint_view_model)
{
