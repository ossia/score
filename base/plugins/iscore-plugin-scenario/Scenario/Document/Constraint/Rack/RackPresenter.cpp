...
RackPresenter::RackPresenter(
    ...
    QObject* parent)
    : QObject{parent}, m_model{model}, m_view{view}, m_context{ctx}
{
  ...
  m_model.slotmodels.added
      .connect<RackPresenter, &RackPresenter::on_slotCreated>(this);
  m_model.slotmodels.removed
      .connect<RackPresenter, &RackPresenter::on_slotRemoved>(this);

  con(m_model, &RackModel::slotPositionsChanged, this,
      &RackPresenter::on_slotPositionsChanged);

  con(m_model, &RackModel::on_durationChanged, this,
      &RackPresenter::on_durationChanged);
}
...
{
  qreal totalHeight = 0; // No slot -> not visible ? or just "add a process"
                         // button ? Bottom bar ? How to make it visible ?
...
void RackPresenter::on_slotCreated_impl(const SlotModel& slotModel)
{
  ...
  connect(
      slotPres, &SlotPresenter::askUpdate, this, &RackPresenter::on_askUpdate);
  ...
  auto scenario
      = dynamic_cast<TemporalScenarioPresenter*>(this->parent()->parent());
  if (!scenario)
    return;
...
void RackPresenter::updateShape()
{
  ...
  for (const auto& slotId : m_model.slotsPositions())
  {
    ...
    currentSlotY
        += slotPres.height() + slotSpacing; // Separation between slots
  }
