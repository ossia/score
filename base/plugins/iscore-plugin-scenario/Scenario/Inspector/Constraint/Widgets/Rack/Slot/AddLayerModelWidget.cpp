    ...
    : QWidget{parentSlot}
{
  ...
  connect(addButton, &QToolButton::pressed, [=]() {
    ...
    const auto& shared_process_list
        = parentSlot->model().parentConstraint().processes;
    ...
    for (const auto& process : shared_process_list)
    {
      ...
      auto it = std::find_if(
          beg_it, end_it, [&process](const Process::LayerModel& lm) {
    ...
    if (!available_models.empty())
    {
      ...
      if (ok)
        parentSlot->createLayerModel(
            Id<Process::ProcessModel>{available_models[process_name]});
    }
  });
}
}
