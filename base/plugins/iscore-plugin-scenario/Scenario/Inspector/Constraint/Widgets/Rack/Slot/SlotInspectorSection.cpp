...
SlotInspectorSection::SlotInspectorSection(
    ...
    , m_sb{slot,
           parentRack->constraintInspector().commandDispatcher()->stack(),
           nullptr}
{
  ...
  m_model.layers.added
      .connect<SlotInspectorSection, &SlotInspectorSection::on_layerModelCreated>(
          this);
  m_model.layers.removed
      .connect<SlotInspectorSection, &SlotInspectorSection::on_layerModelRemoved>(
          this);
  ...
  connect(
      this, &InspectorSectionWidget::nameChanged, this,
      &SlotInspectorSection::ask_changeName);
    ...
    const Id<Process::ProcessModel>& sharedProcessModelId)
{
  auto cmd = new Command::AddLayerModelToSlot{
      m_model, m_model.parentConstraint().processes.at(sharedProcessModelId)};
...
void SlotInspectorSection::on_layerModelCreated(const Process::LayerModel& lm)
{
  displayLayerModel(lm);
}

void SlotInspectorSection::on_layerModelRemoved(
    const Process::LayerModel& removed)
{
  // OPTIMIZEME
