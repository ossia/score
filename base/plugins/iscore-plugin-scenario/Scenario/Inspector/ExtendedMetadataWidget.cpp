    ...
    QWidget{parent}
{
    ...
    connect(m_addButton, &QPushButton::clicked,
            this, [=] () {
        QString txt = QInputDialog::getText(this, tr("Property name?"), tr("Enter a name for the new property:"));
        if(!txt.isEmpty())
        {
...
std::pair<QLabel *, QLineEdit *> ExtendedMetadataWidget::makeRow(
        const QString &l,
        const QString &r,
        int row)
{
    ...
    connect(remove_act, &QAction::triggered,
            this, [=] { on_rowRemoved(row); },
    Qt::QueuedConnection);
    left->addAction(remove_act);

    auto right = new QLineEdit{r};
    connect(right, &QLineEdit::editingFinished,
            this, [=] { on_rowChanged(row); },
    Qt::QueuedConnection);
