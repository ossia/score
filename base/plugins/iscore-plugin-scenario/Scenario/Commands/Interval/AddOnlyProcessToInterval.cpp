...
void AddOnlyProcessToInterval::undo(const iscore::DocumentContext& ctx) const
{
  undo(m_path.find(ctx));
}

void AddOnlyProcessToInterval::redo(const iscore::DocumentContext& ctx) const
{
...
AddOnlyProcessToInterval::redo(IntervalModel& interval, const iscore::DocumentContext& ctx) const
{
