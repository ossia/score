...
public:
  ...
  AddMultipleProcessesToConstraintMacro(const Path<ConstraintModel>& cstpath)
  {
    ...
    auto cmd_rack = new Scenario::Command::AddRackToConstraint{
        Path<ConstraintModel>{cstpath}};
    addCommand(cmd_rack);
    ...
    addCommand(new ShowRackInAllViewModels{Path<ConstraintModel>{cstpath},
  ...
  AddMultipleProcessesToConstraintMacro(const ConstraintModel& constraint)
  {
    ...
    else
    {
      ...
      auto cmd_rack = new Scenario::Command::AddRackToConstraint{constraint};
      bool cmd_rack_used = std::any_of(
          constraint.viewModels().begin(),
  ...
  std::vector<std::pair<Path<SlotModel>, std::vector<Id<Process::LayerModel>>>>
      slotsToUse; // No need to save this, it is useful only for construction.
};

inline std::
    tuple<AddMultipleProcessesToConstraintMacro*, std::vector<std::vector<std::pair<Path<SlotModel>, Id<Process::LayerModel>>>>>
    makeAddProcessMacro(const ConstraintModel& constraint, int num_processes)
{
  auto macro = new AddMultipleProcessesToConstraintMacro{
      constraint}; // The constraint already exists
  ...
  for (const auto& elt : macro->slotsToUse)
  {
    ...
    {
      slotVec.push_back({elt.first, getStrongIdRange<Process::LayerModel>(
                                        num_processes, slot->layers)});
    }
    else
    {
      slotVec.push_back(
          {elt.first, getStrongIdRange<Process::LayerModel>(num_processes)});
    }
  }
  ...
  for (int i = 0; i < num_processes; ++i)
  {
    ...
    std::transform(
        slotVec.begin(), slotVec.end(), std::back_inserter(layerVec),
