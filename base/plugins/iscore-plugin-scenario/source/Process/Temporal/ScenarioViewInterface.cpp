...
void ScenarioViewInterface::on_eventMoved(const EventPresenter& ev)
{
    ...
    {
        auto state_it = m_presenter->m_displayedStates.find(state);
        if(state_it != m_presenter->m_displayedStates.end())
        {
...
{
    auto rect = m_presenter->m_view->boundingRect();
    const auto& ev = static_cast<const ScenarioModel&>(m_presenter->layerModel().sharedProcessModel()).event(state.model().eventId());
