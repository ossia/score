...
void ScenarioGlobalCommandManager::clearContentFromSelection(const ScenarioModel &scenario)
{
    ...
    {
        cleaner.submitCommand(new ClearConstraint(safe_path(*constraint)));
    }

    for(auto& state : statesToRemove)
    {
        cleaner.submitCommand(new ClearState(safe_path(*state)));
    }
...
void ScenarioGlobalCommandManager::removeSelection(const ScenarioModel &scenario)
{
    ...
    if(!sel.empty())
    {
        ...
        dispatcher.submitCommand(new RemoveSelection(safe_path(scenario), sel));
    }
}
