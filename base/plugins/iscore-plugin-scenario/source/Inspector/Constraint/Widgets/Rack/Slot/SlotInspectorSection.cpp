    ...
    m_parent{parentRack->m_parent}
{
    ...
    {
        auto cmd = new RemoveSlotFromRack{iscore::IDocument::safe_path(m_model)};
        ...
        const id_type<Process>& sharedProcessModelId)
{
    auto cmd = new AddLayerModelToSlot(
                   ...
                   iscore::IDocument::safe_path(m_model.parentConstraint().process(sharedProcessModelId)));
...
void SlotInspectorSection::displayLayerModel(const LayerModel& lm)
{
    ...
    connect(pb, &QPushButton::clicked,
            [=]() {
        PutLayerModelToFront cmd(iscore::IDocument::safe_path(m_model), lm_id);
        cmd.redo();
    });
    ...
    connect(deleteButton, &QPushButton::pressed, this, [=] ()
    {
        auto cmd = new RemoveLayerModelFromSlot{iscore::IDocument::safe_path(m_model), lm_id};
        emit m_parent->commandDispatcher()->submitCommand(cmd);
    });
