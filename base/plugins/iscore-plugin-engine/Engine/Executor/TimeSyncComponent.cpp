...
{
  con(m_iscore_node, &Scenario::TimeSyncModel::triggeredByGui,
      this, &TimeSyncComponent::on_GUITrigger);

  con(m_iscore_node, &Scenario::TimeSyncModel::activeChanged,
      this, &TimeSyncComponent::updateTrigger);
  con(m_iscore_node, &Scenario::TimeSyncModel::triggerChanged,
...
const Scenario::TimeSyncModel& TimeSyncComponent::iscoreTimeSync() const
{
...
{
  if(dynamic_cast<Scenario::ProcessModel*>(this->iscoreTimeSync().parent()))
  {
    this->system().executionQueue.enqueue(
          [this,e = m_ossia_node]
    {
