...
class observable_device_roots final : public QObject
{
  ...
  {
    con(d, &Device::DeviceInterface::deviceChanged, this,
...
ossia::net::parameter_base* find_parameter(
    ...
    const std::vector<ossia::net::node_base*>& roots, const QString& str)
{
  auto d = str.indexOf(':');
  if (d == -1)
  {
...
struct mapper_parameter_data_base
{
  ...
  = delete;
  mapper_parameter_data_base& operator=(mapper_parameter_data_base&&)
  = default;
...
struct mapper_parameter_data final : public parameter_data,
    public mapper_parameter_data_base
{
  ...
  {
    return !bind.isNull() || !write.isNull();
  }
};
...
public:
  ...
  struct callback_stopper
  {
    ...
    callback_stopper(mapper_parameter& self) : self{self}
    {
      self.m_stop_callbacks = true;
    }
    ~callback_stopper()
    {
      self.m_stop_callbacks = false;
    }
  };
...
public:
    ...
    connect(
      ...
      switch (status)
      {
        case QQmlComponent::Status::Ready:
        {
          ...
          qt::create_device<
              ossia::net::device_base, mapper_node, mapper_protocol>(
  ...
  void sig_recv(mapper_parameter* p, ossia::net::parameter_base* s, const ossia::value& v) W_SIGNAL(sig_recv, p, s, v);
  ...
  void slot_recv(mapper_parameter* p, ossia::net::parameter_base* s, const ossia::value& v)
  {
    ...
    {
      auto res = p->data().read.call({QString::fromStdString(s->get_node().osc_address()), qt::value_to_js_value(v, *m_engine)});
      p->set_value(qt::value_from_js(std::move(res)));
    }
  }

  static mapper_parameter_data read_data(const QJSValue& js)
  {
    return js;
  }
private:
  void reset_tree()
  {
    ...
    ossia::net::visit_parameters(
      ...
      p.data().source = setup_sources(
            p.data().bind, m_device->get_root_node(), m_roots);
...
void mapper_parameter::connect(parameter_base& s, mapper_protocol& proto)
{
  set_value(s.value());
  s.get_node()
      .about_to_be_deleted.connect<&mapper_parameter::on_sourceRemoved>(
        *this);
  // TODO handle parameter removal from device -> some hash_map
  callbacks[&s.get_node()] = s.add_callback(
        [this, param=&s, &proto] (const ossia::value& v) {
...
Device::DeviceSettings MapperProtocolSettingsWidget::getSettings() const
{
  ...
  s.deviceSpecificSettings = QVariant::fromValue(
        MapperSpecificSettings {m_codeEdit->toPlainText()});
  return s;
}
