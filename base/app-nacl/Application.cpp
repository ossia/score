...
static QApplication* make_application(int& argc, char** argv)
{
...
Application::Application(
    const ApplicationSettings& appSettings, int& argc, char** argv)
    : NamedObject{"Application", nullptr}
    , m_app{new SafeQApplication{argc, argv}}
...
void Application::init()
{
  ...
  m_presenter
      = new score::Presenter{m_applicationSettings, m_settings, m_view, this};
...
void Application::initDocuments()
{
  ...
  {
    if (!m_presenter->applicationComponents()
  ...
  connect(
      m_app, &SafeQApplication::fileOpened, this, [&](const QString& file) {
...
{
  auto& ctx = m_presenter->applicationContext();
  score::ApplicationRegistrar registrar{m_presenter->components(),
                                        ctx,
                                        *m_view,
  ...
  registrar.registerApplicationContextPlugin(
      new score::CoreApplicationPlugin{ctx, *m_presenter});
  registrar.registerApplicationContextPlugin(
  ...
  for (auto& panel_fac :
       context().interfaces<score::PanelDelegateFactoryList>())
  {
