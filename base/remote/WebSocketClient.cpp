...
WebSocketClient::WebSocketClient() : m_ws{"remote-ui"}
{
  // Websocket setup
  connect(
      &m_ws, &QWebSocket::binaryMessageReceived, this,
  ...
  connect(
      &m_ws, &QWebSocket::textMessageReceived, this, [=](const QString& mess) {
  ...
  connect(&m_ws, &QWebSocket::connected, this, [] { qDebug("Connected"); });
  connect(
      &m_ws,
      static_cast<void (QWebSocket::*)(QAbstractSocket::SocketError)>(
          &QWebSocket::error),
      this,
      [=](QAbstractSocket::SocketError) { qDebug() << m_ws.errorString(); });
...
void WebSocketClient::handleObject(const QJsonObject& obj)
{
  ...
  auto it = actions.find(
      obj[score::StringConstant().Message].toString().toStdString());
  if (it != actions.end())
  {
    ((*it).second)(obj);
  }
