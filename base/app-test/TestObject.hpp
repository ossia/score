...
class TestObject : public QObject
{
  ...
  Q_OBJECT
public:
  TestObject(const score::GUIApplicationContext& ctx) : m_context{ctx}
  {
...
public:
  void appStarted()
  {
    ...
    while (it.hasNext())
    {
      ...
      auto ba_doc
          = m_context.docManager.loadDocument(m_context, byte_arr, doctype);
      QApplication::processEvents();
      m_context.docManager.forceCloseDocument(m_context, *ba_doc);
      QApplication::processEvents();

      auto json_doc
          = m_context.docManager.loadDocument(m_context, json_arr, doctype);
      QApplication::processEvents();
    ...
    {
      auto doc = m_context.docManager.loadFile(
          m_context, "testdata/execution.scorejson");
      if (!doc)
        qApp->exit(1);
      for (int i = 0; i < 10; i++)
      {
