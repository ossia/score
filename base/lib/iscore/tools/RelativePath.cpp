...
QObject* RelativePath::find_impl(QObject *source) const
{
    ...
    {
        auto found_children =
                source->findChildren<IdentifiedObjectAbstract*> (
                    currentObjIdentifier.objectName(),
                    Qt::FindDirectChildrenOnly);

        source = findById_weak_safe(
                    found_children,
...
template<>
ISCORE_LIB_BASE_EXPORT void Visitor<Reader<DataStream>>::readFrom(const iscore::RelativePath& path)
{
...
template<>
ISCORE_LIB_BASE_EXPORT void Visitor<Writer<DataStream>>::writeTo(iscore::RelativePath& path)
{
    m_stream >> path.m_parents;
...
template<>
ISCORE_LIB_BASE_EXPORT void Visitor<Reader<JSONObject>>::readFrom(const iscore::RelativePath& path)
{
...
template<>
ISCORE_LIB_BASE_EXPORT void Visitor<Writer<JSONObject>>::writeTo(iscore::RelativePath& path)
{
