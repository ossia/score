...
#define return_uuid(text)                                   \
  ...
  {                                                         \
    Q_DECL_RELAXED_CONSTEXPR const auto t                   \
        = iscore::uuids::string_generator::compute((text)); \
    return t;                                               \
  } while (0)
...
class UuidKey : iscore::uuid_t
{
  ...
  {
    return static_cast<const iscore::uuid_t&>(lhs)
           == static_cast<const iscore::uuid_t&>(rhs);
  }
  friend Q_DECL_RELAXED_CONSTEXPR bool
  operator!=(const this_type& lhs, const this_type& rhs)
  {
...
public:
  ...
  Q_DECL_RELAXED_CONSTEXPR UuidKey(Iterator beg_it, Iterator end_it)
      : iscore::uuid_t(
            iscore::uuids::string_generator::compute(beg_it, end_it))
  {
  }
  ...
  Q_DECL_RELAXED_CONSTEXPR const iscore::uuid_t& impl() const
  {
    return *this;
  }
  Q_DECL_RELAXED_CONSTEXPR iscore::uuid_t& impl()
  {
    return *this;
  }
};
...
struct hash<UuidKey<T>>
{
  Q_DECL_CONSTEXPR std::size_t operator()(const UuidKey<T>& kagi) const
      noexcept
  {
    return boost::hash<iscore::uuid_t>()(
        static_cast<const iscore::uuid_t&>(kagi));
  }
};
}
