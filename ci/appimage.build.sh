...
# The container runs as root, so under rootful docker the build output is
# root-owned. Doing the chown from a container needs no sudo -- being in the
# docker group is already root-equivalent -- and works the same under rootless
# docker, where the files are ours to begin with. This must not be allowed to
# fail quietly: everything below writes into the AppDir, and a swallowed
# failure here surfaces much later as an unrelated appimagetool error.
